-- PATCH 10.6 — Q4 Eventos por decisão
...
select decision_request_id, count(distinct event_name)::int as tipos, count(*)::bigint as eventos from phase10 where decision_request_id is not null group by 1 order by eventos desc limit 50;
