...
} from "./miaPriceAlertLifecycleAnalytics.js";
...
export async function runPriceAlertsSend(options = {}) {
  ...
  for (const alert of loaded.alerts) {
    try {
      ...
      instrumentPriceAlertLifecycleFromCheck(options.supabase, {
        alert,
        evaluation,
        checkSource: "send_gate",
        dryRun: false,
      });
      ...
      instrumentPriceAlertLifecycleFromNotification(options.supabase, {
        alert,
        evaluation,
        stage: MIA_ALERT_LIFECYCLE_STAGE.NOTIFICATION_PREPARED,
        success: true,
        attempt: (Number.parseInt(String(alert.email_send_count ?? "0"), 10) || 0) + 1,
      });
      ...
      if (sendResult?.ok === true) {
        ...
        instrumentPriceAlertLifecycleFromNotification(options.supabase, {
          alert,
          evaluation,
          stage: MIA_ALERT_LIFECYCLE_STAGE.NOTIFICATION_FAILED,
          success: false,
