...
export function createCommercialRefinementMultiExtractor(deps = {}) {
  ...
  function probeBudgetRelaxation(message, q, sessionContext) {
    ...
    if (budgetFlexMatch) {
      ...
      const priorBudget =
        ...
        extractBudget(sessionContext?.lastQuery || sessionContext?.lastTopic || "") ??
        null;
      ...
      if (flexBudget && !priorBudget) {
        return {
          detected: true,
          refinementType: REFINEMENT_TYPES.BUDGET_REFINEMENT,
          operation: REFINEMENT_OPERATIONS.REPLACE,
          value: Math.round(flexBudget * 1.15),
          confidence: 0.88,
          requiresClarification: false,
          sourceMessage: message,
        };
  ...
  function probePriorityAdd(message, q) {
    const ATTR = "(bateria|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)";
    const patterns = [
      PRIORITY_ADD_PATTERN,
      new RegExp(`\\b${ATTR}\\s+agora\\s+(?:é|e)\\s+(?:o\\s+)?mais\\s+importante\\b`, "i"),
      new RegExp(`\\b(?:quero\\s+)?(?:dar\\s+)?mais\\s+import[aâ]ncia\\s+(?:para|em|a)\\s+${ATTR}\\b`, "i"),
      new RegExp(`\\b${ATTR}\\s+(?:é|e)\\s+prioridade\\b`, "i"),
      new RegExp(`\\bquero\\s+focar\\s+mais\\s+em\\s+${ATTR}\\b`, "i"),
      new RegExp(`\\b${ATTR}\\s+virou\\s+prioridade\\b`, "i"),
    ];
    for (const pattern of patterns) {
      const match = q.match(pattern);
      const rawAttr = match?.[1] || match?.[2];
      if (!rawAttr) continue;
      let attribute = normalizeText(rawAttr);
  ...
  function collectRefinementProbes(message, sessionContext) {
    ...
    const negativeBrand = SOCIAL_SEM_EXCLUSION_PATTERN.test(q) ? null : q.match(NEGATIVE_BRAND_PATTERN);
    if (negativeBrand) {
  ...
  function extractCommercialRefinements(message = "", sessionContext = {}) {
    ...
    if (isInitialCommercialEntryMessage(message, sessionContext)) {
      return [];
    }
