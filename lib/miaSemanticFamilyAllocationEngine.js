...
import { validateSemanticDecisionUnit } from "./miaSemanticDecisionContract.js";
import { toLegacyGainString } from "./miaSemanticDecisionLegacyAdapter.js";
 ...
 * @param {{ primaryAxis?: string, maxGains?: number, category?: string, productName?: string, userPriorityPhrase?: string }} context
 */
export function selectTradeoffGainsWithSemantics(state, candidateData, context = {}) {
  ...
  const tryAddGain = (item, allowPriorRepeat = false) => {
    ...
    const unit = buildSemanticDecisionUnitFromPoolItem(item, {
      primaryAxis,
      category: context.category,
      productName: context.productName,
      userPriorityPhrase: context.userPriorityPhrase,
      decisionRole: gains.length === 0 ? "primary_gain" : "secondary_gain",
    });
    const validation = validateSemanticDecisionUnit(unit);
    if (!validation.valid) return false;

    const text = toLegacyGainString(unit, compactConsequence);
  ...
  if (!gains.length) {
    ...
    if (impact) {
      ...
      const unit = buildSemanticDecisionUnitFromPoolItem(fallbackItem, {
        primaryAxis,
        category: context.category,
        productName: context.productName,
        userPriorityPhrase: context.userPriorityPhrase,
      ...
      if (validateSemanticDecisionUnit(unit).valid) {
        semanticUnits.push(unit);
        gains.push(toLegacyGainString(unit, compactConsequence));
      }
    }
  }

  state.semanticUnits = semanticUnits.slice(0, maxGains);

  return {
    gains: gains.slice(0, maxGains),
...
export function selectTradeoffGains(state, candidateData, context = {}) {
  return selectTradeoffGainsWithSemantics(state, candidateData, context).gains;
}

/**
 * @param {ReturnType<typeof createSemanticAllocationState>} state
 * @param {{ pool: Array }} candidateData
 * @param {{ primaryAxis?: string, maxSacrifices?: number, category?: string, productName?: string }} context
 */
export function selectTradeoffSacrificesWithSemantics(state, candidateData, context = {}) {
  ...
  for (const item of weaknesses) {
    ...
    const enrichedItem = { ...item, family };
    const unit = buildSemanticDecisionUnitFromWeaknessPoolItem(enrichedItem, {
      primaryAxis: context.primaryAxis,
      category: context.category,
      productName: context.productName,
    });
    if (!validateSemanticDecisionUnit(unit).valid) continue;

    const text = toLegacyGainString(unit, compactSacrifice);
    if (!text) continue;

    semanticUnits.push(unit);
    sacrifices.push(text);
    usedFamilies.add(family);
    markFamilyUsed(state, family, "tradeoff_sacrifice", { text: item.text, token: item.token });
  }

  state.semanticSacrificeUnits = semanticUnits.slice(0, maxSacrifices);

  return {
    sacrifices: sacrifices.slice(0, maxSacrifices),
    semanticUnits: state.semanticSacrificeUnits,
  };
}
...
export function selectTradeoffSacrifices(state, candidateData, context = {}) {
  return selectTradeoffSacrificesWithSemantics(state, candidateData, context).sacrifices;
}
