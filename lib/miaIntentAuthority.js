...
import { buildCommercialAuthorityContextPatch } from "./miaCommercialEntryReconciliation.js";
import {
  applyClarificationGateToContextResolution,
  resolveClarificationDecision,
} from "./miaClarificationGates.js";
import { isGovernedSocialContractBlocksClarification } from "./miaSemanticAuthority.js";

export const INTENT_AUTHORITY_SOURCE = "mia_intent_recognition";
export const INTENT_AUTHORITY_VERSION = "11A.3";
 ...
 */
export function resolveCommercialPermissionFromRecognition(
  recognition = null,
  { hasActiveAnchor = false, sessionContext = {}, conversationMessages = [] } = {}
) {
  if (!recognition) {
  ...
  const message = recognition.resolvedQuery || recognition.rawMessage || "";
  const activeCommercialAsk = detectActiveCommercialAsk(message);
  const explicitCommercialDenial = detectExplicitCommercialDenial(message);
  ...
  if (
    ...
    // PATCH 3.1 — vague mixed still opens commercial when intent signal is present.
    if (
  ...
  if (contextualFollowUp.requiresClarification && !activeCommercialAsk) {
    if (contextualFollowUp.contextualCommercialAuthorized) {
      return hasActiveAnchor || interactionMode === MIA_INTERACTION_MODES.MIXED
        ? COMMERCIAL_PERMISSION.MIXED
        : COMMERCIAL_PERMISSION.ALLOW;
    }
    if (
      contextualFollowUp.detected &&
      Array.isArray(contextualFollowUp.comparisonProducts) &&
      contextualFollowUp.comparisonProducts.length >= 2
    ) {
      return COMMERCIAL_PERMISSION.ALLOW;
    }
    return COMMERCIAL_PERMISSION.DENY;
...
export function applyIntentAuthorityToPipeline({
  ...
  if (authority.commercialPermission === COMMERCIAL_PERMISSION.DENY) {
    ...
    const governedContractBlocksClarification =
      socialBehaviorContract &&
    ...
    result.contextResolutionPatch = {
      ...
      needsClarification:
        !governedContractBlocksClarification &&
        (mappedIntent === "clarification" || clarificationDecision.needsClarification),
      clarificationMessage:
        clarificationDecision.clarificationMessage ||
    ...
    if (
      ...
      authority.commercialPermission === COMMERCIAL_PERMISSION.ALLOW &&
      (intent === "general_answer" || intent === "social_conversation")
    ) {
      result.intent = "search";
      result.contextAction = "search";
    } else if (authority.commercialPermission === COMMERCIAL_PERMISSION.MIXED) {
      result.contextAction = contextAction || "conversation";
    }

    const commercialPatch = buildCommercialAuthorityContextPatch({
      authority,
      contextResolution,
      intentRecognition,
      query,
    ...
    if (commercialPatch) {
      ...
      result.divergences.push({
        ...
        proposed: contextResolution?.mode || null,
        preserved: commercialPatch.mode || "commercial_authority_open",
        reason: "commercial_entry_reconciliation_3_1",
      });
    }
  }
