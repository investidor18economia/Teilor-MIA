...
export function instrumentPriceIntelligenceAnalyticsFromOfferSet(supabase, input = {}) {
  ...
  // Domain gate is enforced when offer_set tracking activates (initializeOfferSetAnalyticsTracking).
  // This hook runs only after offer_set delivery emission; re-checking sharedState permission here
  // would block valid commercial events because permission is not persisted on shared state.
