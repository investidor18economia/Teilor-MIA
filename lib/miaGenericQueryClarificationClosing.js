...
export const GENERIC_QUERY_CLARIFICATION_CLOSING_VERSION = "3.4b.0";
...
function resolveMissingContextAxis(input = {}) {
  ...
  const categoryKnown =
    ...
    (isGenericProductSearchQuery(query) &&
  ...
  if (
    categoryKnown &&
    resolveDetectedBudget(query, input.budget) == null &&
    !hasKnownPrimaryUseContext(input)
  ) {
    return {
      axis: "constraint",
      reason: "orçamento ainda não foi informado",
      confidence: "high",
    };
  }
...
export function buildGenericQueryClarificationQuestion(input = {}) {
  ...
  const templates =
    missingAxis === "constraint" && budget == null
      ? [
          "Consigo recomendar algumas opções, mas se você me disser seu orçamento consigo ser bem mais precisa.",
          "Posso te sugerir caminhos iniciais, mas com sua faixa de preço eu fecho uma recomendação bem mais certeira.",
          "Se você me passar o orçamento, eu ajusto a recomendação com bem mais precisão.",
        ]
