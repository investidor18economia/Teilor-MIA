...
export const SOCIAL_CONVERSATION_CONTINUITY_VERSION = "5.8.7";
...
export const SOCIAL_CONTINUITY_BEHAVIOR = Object.freeze({
  ...
  RESUME_WITHOUT_ANCHOR: "resume_without_anchor",
...
const RESUMPTION_SIGNAL =
  /\b(como\s+(?:eu|a\s+gente)\s+(?:estava|estavamos|estávamos)\s+(?:dizendo|falando|comentando)|voltando\s+(?:ao|naquele|pro|para\s+o)\s+(?:assunto|papo|tema|que\s+eu)|volta\s+(?:pro|para\s+o)\s+papo|lembra\s+(?:do|da|de\s+que|que\s+eu)|(?:retomando|continuando)\s*(?:o|a|o\s+papo|b\b)?|continuando\b|retomando\b|naquele\s+assunto|sobre\s+(?:aquilo|o\s+que)|sobre\s+o\s+que\s+(?:eu|a\s+gente)\s+(?:falei|disse|comentei)|como\s+(?:eu\s+)?(?:disse|falei)\s+antes|como\s+eu\s+disse|como\s+falamos|ent[aã]o\s+voc[eê]\s+lembra)\b/i;
...
const FAREWELL = /\b(tchau|at[eé]\s+(?:logo|mais|amanh[aã])|flw|falou|fui|vou\s+nessa)\b/i;

/** Social departure — not product intent; governs natural closing without commercial bleed */
const SOCIAL_DEPARTURE =
  /\b(preciso\s+ir|preciso\s+sair|vou\s+indo|t[oô]\s+indo|vou\s+dormir|falamos\s+depois|tenho\s+que\s+trabalhar|preciso\s+trabalhar|vou\s+trabalhar|at[eé]\s+amanh[aã]|depois\s+a\s+gente\s+conversa|at[eé]\s+depois)\b/i;
...
export function detectSocialDeparture(message = "") {
  const n = normalizeText(message);
  return SOCIAL_DEPARTURE.test(n) || FAREWELL.test(n);
}
...
function scanConversationDiscourse(conversationMessages = []) {
  ...
  let lastSubstantiveUserMessage = null;
  ...
  for (const msg of messages) {
    ...
    if (isInitialGreetingMessage(text)) {
    ...
    } else if (FAREWELL.test(n) || SOCIAL_DEPARTURE.test(n)) {
      ...
      lastSubstantiveUserMessage = text;
      lastSocialTurn = turnIndex;
...
export function resolveSocialConversationContinuity({
  ...
  const isSocialDeparture = detectSocialDeparture(userMessage);
  ...
  if (isSocialDeparture) {
    socialContinuityBehavior = SOCIAL_CONTINUITY_BEHAVIOR.SOCIAL_CLOSING;
    continuityStrength = CONTINUITY_STRENGTH.MODERATE;
    conversationPhase = CONVERSATION_PHASE.CLOSING;
  } else if (isResumption && activeSocialTopic) {
    ...
    isResumption &&
    !activeSocialTopic &&
    (historyScan.turnCount >= 1 || historyScan.lastSubstantiveUserMessage)
  ) {
    socialContinuityBehavior = SOCIAL_CONTINUITY_BEHAVIOR.RESUME_WITHOUT_ANCHOR;
    continuityStrength = CONTINUITY_STRENGTH.MODERATE;
    resumptionRequested = true;
    conversationPhase = CONVERSATION_PHASE.SOCIAL_ACTIVE;
  } else if (MEMORY_CHECK.test(normalized) && activeSocialTopic) {
    socialContinuityBehavior = SOCIAL_CONTINUITY_BEHAVIOR.CONFIRM_MEMORY;
    continuityStrength = CONTINUITY_STRENGTH.MODERATE;
    resumptionRequested = true;
  } else if (
    MEMORY_CHECK.test(normalized) &&
    !activeSocialTopic &&
    (historyScan.turnCount >= 1 || historyScan.lastMetaTurn >= 0)
  ) {
    socialContinuityBehavior = SOCIAL_CONTINUITY_BEHAVIOR.RESUME_WITHOUT_ANCHOR;
    continuityStrength = CONTINUITY_STRENGTH.MODERATE;
    resumptionRequested = true;
  ...
  const discourse = {
    ...
    activeSocialTopic,
    lastSubstantiveUserMessage: historyScan.lastSubstantiveUserMessage || null,
    lastUserEmotion,
    greetingExchanged,
    ...
    resumptionRequested,
    reciprocityState,
    socialDeparture: isSocialDeparture,
...
export function buildContinueGreetingThreadReply(contract = {}) {
  ...
  return pickRhythmGovernedVariant(pools[key] || pools.warm_balanced, seedFromContract(contract, "continue-greeting"));
}
...
export function buildResumeSocialDiscourseReply(contract = {}) {
  ...
  return pickRhythmGovernedVariant(variants, seedFromContract(contract, "resume-discourse"));
}
...
export function buildAcknowledgeActiveTopicReply(contract = {}) {
  ...
  const variants = (pools[key] || pools.warm_balanced).map((fn) => fn());
  return pickRhythmGovernedVariant(variants, seedFromContract(contract, "ack-topic"));
}
...
export function buildReturnToSocialThreadReply(contract = {}) {
  ...
  return pickRhythmGovernedVariant(pools[key] || pools.warm_balanced, seedFromContract(contract, "return-social"));
}
...
export function buildConfirmShortTermMemoryReply(contract = {}) {
  ...
  if (topic) {
    ...
    const variants = (pools[key] || pools.warm_balanced).map((fn) => fn());
    return pickRhythmGovernedVariant(variants, seedFromContract(contract, "confirm-memory"));
  }
  const pools = {
    warm_balanced: [
  ...
  return pickRhythmGovernedVariant(pools[key] || pools.warm_balanced, seedFromContract(contract, "confirm-memory-generic"));
}

export function buildResumeWithoutAnchorReply(contract = {}) {
  const key = warmthKey(contract);
  const prior = contract.socialConversationContinuity?.lastSubstantiveUserMessage;
  const pools = prior
    ? {
        warm_balanced: [
          () => `Lembro sim — você tinha comentado ${extractTopicLabel(prior) || "algo importante"}.`,
          () => `Sim — ainda estou acompanhando: ${extractTopicLabel(prior) || prior.slice(0, 48)}.`,
        ],
        warm_light: [() => `Lembro — ${extractTopicLabel(prior) || "isso"}.`],
        warm_reserved: [() => `Sim — ${extractTopicLabel(prior) || "continuamos"}.`],
      }
    : {
        warm_balanced: [
          "Ainda estou acompanhando o papo — me conta um pouco do que quer retomar?",
          "Claro — estou por aqui. Sobre o que você quer continuar?",
          "Sim — podemos retomar. Me lembra rapidinho o assunto?",
        ],
        warm_light: ["Claro — me conta do que quer falar.", "Estou aqui — retoma quando quiser."],
        warm_reserved: ["Certo — pode continuar.", "Sim — pode retomar."],
      };
  const variants = (pools[key] || pools.warm_balanced).map((fn) => (typeof fn === "function" ? fn() : fn));
  return pickRhythmGovernedVariant(variants, seedFromContract(contract, "resume-no-anchor"));
}

export function buildSocialClosingReply(contract = {}) {
  const key = warmthKey(contract);
  const pools = {
    warm_balanced: [
      ...
      "Vai tranquilo — até mais!",
    ],
    warm_light: ["Até! Foi bom o papo.", "Até mais — tchau!"],
    warm_reserved: ["Até logo.", "Até mais."],
  };
  return pickRhythmGovernedVariant(pools[key] || pools.warm_balanced, seedFromContract(contract, "social-closing"));
}
...
export function buildContinuityGovernedReply(contract = {}) {
  ...
  switch (behavior) {
    ...
    case SOCIAL_CONTINUITY_BEHAVIOR.RESUME_WITHOUT_ANCHOR:
      return buildResumeWithoutAnchorReply(contract);
    ...
    case SOCIAL_CONTINUITY_BEHAVIOR.SOCIAL_CLOSING:
      return buildSocialClosingReply(contract);
    default:
      return "";
  }
}
...
export function enrichContractWithSocialConversationContinuity(
  ...
  if (
    ...
    (!protectedBehaviors.has(expectedHumanBehavior) ||
      ...
      discourse.socialContinuityBehavior === SOCIAL_CONTINUITY_BEHAVIOR.RESUME_WITHOUT_ANCHOR ||
      discourse.socialContinuityBehavior === SOCIAL_CONTINUITY_BEHAVIOR.SOCIAL_CLOSING)
  ) {
  ...
  return {
    ...
    farewellMode: contract.farewellMode || discourse.socialDeparture || !!recognition?.socialFamilies?.farewell,
    socialDepartureMode: discourse.socialDeparture || false,
