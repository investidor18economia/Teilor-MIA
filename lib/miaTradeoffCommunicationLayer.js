...
export function resolveTradeoffCommunicationSources(input = {}) {
  ...
  if (input.semanticAllocationState && input.semanticCandidateData) {
    const gains = selectTradeoffGains(input.semanticAllocationState, input.semanticCandidateData, {
      primaryAxis,
      maxGains: 3,
      searchCognition,
    });
    const sacrifices = selectTradeoffSacrifices(
      input.semanticAllocationState,
      input.semanticCandidateData,
      { primaryAxis, maxSacrifices: 3 }
    );

    if (gains.length && sacrifices.length) {
      return { gains, sacrifices, primaryAxis };
    }
  }
