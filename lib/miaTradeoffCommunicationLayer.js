...
import {
  ...
  selectTradeoffSacrifices,
  selectTradeoffSacrificesWithSemantics,
} from "./miaSemanticFamilyAllocationEngine.js";
import { buildSemanticDecisionTrace } from "./miaSemanticDecisionContract.js";
import { buildStructuredDecisionFacts } from "./miaStructuredDecisionFacts.js";
import { finalizeTradeoffSourcesWithSynthesis } from "./miaContextualDecisionSynthesis.js";

export const TRADEOFF_COMMUNICATION_LAYER_VERSION = "9.2M.1";
...
function formatGainLine(text = "", seed = "") {
  let body = humanizeIdealForGain(stripTrailingPeriod(text));
  if (!body) return "";

  if (isArtificialAttributeChain(body)) {
    const controlled = sanitizeConsequenceForSpecialistUse(body);
    if (!controlled) return "";
    const listBody = controlled
      .replace(/^Os principais pontos positivos são:\s*/i, "")
      .replace(/^Ele se destaca principalmente[^:]*:\s*/i, "")
      .replace(/\.$/, "");
    return `✅ Você ganha pontos fortes importantes para esse perfil, como ${listBody}.`;
  }
...
export function resolveTradeoffCommunicationSources(input = {}) {
  ...
  if (input.semanticAllocationState && input.semanticCandidateData) {
    const semanticResult = selectTradeoffGainsWithSemantics(
      input.semanticAllocationState,
      input.semanticCandidateData,
      {
        primaryAxis,
        maxGains: 3,
        searchCognition,
    ...
    const gains = semanticResult.gains;
    const sacrificeResult = selectTradeoffSacrificesWithSemantics(
      input.semanticAllocationState,
      input.semanticCandidateData,
      {
        primaryAxis,
    ...
    const sacrifices = sacrificeResult.sacrifices;

    if (gains.length && sacrifices.length) {
      const structuredDecisionFacts = buildStructuredDecisionFacts({
        gainUnits: semanticResult.semanticUnits,
        sacrificeUnits: sacrificeResult.semanticUnits,
        productName: input.productName,
        category: input.category,
        primaryAxis,
      });

      return finalizeTradeoffSourcesWithSynthesis(
        {
          gains,
          sacrifices,
          primaryAxis,
          semanticUnits: semanticResult.semanticUnits,
          semanticSacrificeUnits: sacrificeResult.semanticUnits,
          semanticTrace: buildSemanticDecisionTrace([
            ...semanticResult.semanticUnits,
            ...sacrificeResult.semanticUnits,
          ]),
        ...
        },
        {
          ...
          category: input.category,
          primaryAxis,
          trustedSpecs: input.structuredFacts?.trustedSpecs || input.trustedSpecs || null,
  ...
  const gains = filterUsefulConsequences(structuredFacts.strengthConsequences, 2)
    .map((entry) => sanitizeConsequenceForSpecialistUse(entry) || humanizeStrengthGain(entry))
    .filter((entry) => entry && !isArtificialAttributeChain(entry));
  ...
  return finalizeTradeoffSourcesWithSynthesis(
    {
      ...
      sacrifices: dedupedSacrifices.slice(0, 3),
      primaryAxis,
    },
    {
      productName: input.productName,
      category: input.category,
      primaryAxis,
      trustedSpecs: structuredFacts.trustedSpecs || input.trustedSpecs || null,
      sourceOrigin: input.sourceOrigin || "fallback",
    }
  );
}
