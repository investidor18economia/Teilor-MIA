...
import { isStructuredSpecialistReply } from "./miaSpecialistPresentationContract.js";
...
function applyPhraseRewrites(text = "", options = {}) {
  const preserveParagraphBreaks = !!options.preserveParagraphBreaks;

  if (preserveParagraphBreaks) {
    const paragraphs = String(text || "").split(/\n\s*\n/);
    return paragraphs
      .map((paragraph) => applyPhraseRewrites(paragraph, { preserveParagraphBreaks: false }))
      .filter(Boolean)
      .join("\n\n")
      .trim();
  }
...
export function cleanupMiaHumanLanguage(text = "", context = {}) {
  ...
  const preserveSpecialistStructure =
    context.preserveSpecialistStructure === true || isStructuredSpecialistReply(original);

  let result = cleanedParagraphs.join("\n\n").trim();
  result = applyPhraseRewrites(result, { preserveParagraphBreaks: preserveSpecialistStructure });
