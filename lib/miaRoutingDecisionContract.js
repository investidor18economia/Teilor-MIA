...
/**
 * PATCH 7.9X-H.3 — COMPREHENSION_FAILURE routing hold authority.
 * Must run before cognitive_explanation_anchored / generic explanation intercept.
 * FAILURE only — SUCCESS stays on acknowledgement hold (ACK design).
 */
...
export function buildRoutingDecision({
  ...
  // PATCH 7.7L / 7.9X-H.3 — COMPREHENSION_FAILURE fallback hold (cold / non-explanation paths)
  if (
    applyComprehensionRoutingHoldIfEligible(rd, {
      hasAnchor,
      signals,
