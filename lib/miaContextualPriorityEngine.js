...
export const CONTEXTUAL_PRIORITY_ENGINE_VERSION = "4A.8.0";
...
export function buildContextualPriorityModel(input = {}) {
  ...
  if (signals.gaming || signals.heavyUse) {
    applyBoosts(
      criteriaMap,
      { processor: 0.14, screen: 0.06 },
      PRIORITY_ORIGIN.QUERY_SIGNAL,
      "sinal de uso intenso/jogos",
      [`signal:gaming=${!!signals.gaming}`, `signal:heavyUse=${!!signals.heavyUse}`]
    );
    lockedIntentionCriterion = PRIORITY_CRITERION.PROCESSOR;
  }
  if (signals.batteryPriority) {
    applyBoosts(
      criteriaMap,
      { battery: 0.16 },
      PRIORITY_ORIGIN.QUERY_SIGNAL,
  ...
  const dominantCriterion = criteria[0]?.criterion || null;
