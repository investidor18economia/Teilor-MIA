...
export const VERBALIZATION_STYLE_GOVERNOR_VERSION = "4A.6.2";

const MAX_RECENT_PATTERNS = 6;

const CONSEQUENCE_SPEECH_REWRITES = [
  {
    pattern: /^menos dependência do carregador(?: no dia a dia)?/i,
    rewrite:
      "a autonomia costuma ser um ponto forte, com menos idas ao carregador no dia a dia",
  },
  {
    pattern: /^menos barreira inicial(?: para fechar a compra sem estourar o orçamento)?/i,
    rewrite: "o custo de entrada tende a ser mais acessível, sem estourar o orçamento",
  },
  {
    pattern: /^menos ansiedade com recarga(?: ao longo do dia)?/i,
    rewrite:
      "a bateria costuma dar mais folga, com menos preocupação com recarga ao longo do dia",
  },
  {
    pattern: /^mais sensação de fluidez na navegação/i,
    rewrite: "a tela tende a deixar a navegação mais fluida no uso diário",
  },
  {
    pattern: /^a navegação pode parecer menos fluida/i,
    rewrite:
      "a fluidez da tela pode ficar um pouco abaixo do que quem vem de aparelhos mais rápidos espera",
  },
];

const ARTIFICIAL_BECAUSE_PATTERN =
  /\bporque\s+(?:menos|mais)\s+[a-záàâãéêíóôõúç]/i;

const ARTIFICIAL_NOMINAL_BECAUSE_PATTERN =
  /\bporque\s+(?:câmera|tela|bateria|desempenho|carregamento)\s+(?:boa|bom|fluida|forte|alta|lento|melhor)\b/i;

const DOMINANT_OPENING_PATTERN =
  /^entre as opções,\s*eu colocaria o .+ em primeiro porque /i;
...
/**
 * Rewrites internal consequence phrases into speakable clauses (PATCH 4A.6V).
 * @param {string} text
 */
export function rewriteConsequenceForSpeech(text = "") {
  const normalized = cleanText(text);
  if (!normalized) return normalized;

  for (const entry of CONSEQUENCE_SPEECH_REWRITES) {
    if (entry.pattern.test(normalized)) return entry.rewrite;
  }

  if (/^(?:menos|mais)\s+/i.test(normalized) && !hasFiniteVerb(normalized)) {
    if (/^menos /i.test(normalized)) {
      const tail = normalized.replace(/^menos\s+/i, "");
      return `no dia a dia, isso se traduz em menos ${tail}`;
    }
    const tail = normalized.replace(/^mais\s+/i, "");
    return `no uso real, isso se traduz em mais ${tail}`;
  }

  return genericSurfaceRewrite(normalized, "");
}

/**
 * @param {string} text
 */
export function detectArtificialBecauseFragment(text = "") {
  const normalized = cleanText(text);
  if (!normalized) return { detected: false, reasons: [] };
  const reasons = [];
  if (ARTIFICIAL_BECAUSE_PATTERN.test(normalized)) reasons.push("porque_menos_mais");
  if (ARTIFICIAL_NOMINAL_BECAUSE_PATTERN.test(normalized)) reasons.push("porque_nominal");
 ...
 * @param {string} text
 */
export function detectDominantOpeningTemplate(text = "") {
  const firstLine = cleanText(String(text || "").split(/\n/)[0] || "");
  return {
    detected: DOMINANT_OPENING_PATTERN.test(firstLine),
    text: firstLine,
  };
}
...
export function surfaceRewriteFragment(text = "", options = {}) {
  ...
  if (!normalized) return normalized;
  if (/^(?:menos|mais)\s+/i.test(normalized) && !hasFiniteVerb(normalized)) {
    return rewriteConsequenceForSpeech(normalized);
  }
  return genericSurfaceRewrite(normalized, options.effectKey || "");
}
