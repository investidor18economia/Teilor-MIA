...
import {
  ...
  FILLER_TYPES,
} from "./miaConversationalFillerGovernance.js";
...
function inferCommercialAnchorValidity(sessionContext = {}, options = {}) {
  ...
  if (!anchor) {
    ...
    if (
      ...
      (options.commercialFollowUpReuse || options.allowCompletedDecisionReuse || sessionContext.comparisonContextLocked)
    ) {
      return { valid: true, reason: "comparison_set_anchor" };
    }
...
export function resolveSemanticContinuationEligibility({
  ...
  const fillerResolution = classifyConversationalFiller(message, {
    conversationMessages: signals.conversationMessages || [],
    sessionContext: state.legacy || {},
    hasActiveAnchor: commercialAnchorValid || commercialAnchorPresent,
  });
  if (fillerResolution.detected && fillerResolution.preserveCommercialAnchor) {
    reasonCodes.push(fillerResolution.reasonCode || "filler_does_not_clear_commercial_anchor");
    return buildContinuationResult({
      eligible: true,
      continuationType: fillerResolution.type === FILLER_TYPES.EXIT ? "social" : "social",
      socialContinuityEligible: true,
      commercialContinuationEligible: false,
      commercialExecutionFromContinuation: false,
      commercialAnchorPresent,
      ...
      anchorExecuted: false,
      stateUsed: commercialAnchorValid ? ["commercial.anchor"] : stateUsed,
      stateIgnored,
      reasonCodes,
      postPurchase: false,
      decisionCompleted: !!state.commercial?.decisionCompleted,
      fillerGovernance: fillerResolution,
    });
  }

  if (detectGenericDimensionFollowUpQuery(message).detected && commercialAnchorValid) {
    commercialContinuationEligible = true;
    commercialExecutionFromContinuation = true;
    reasonCodes.push("commercial_dimension_follow_up_preserves_target");
  }
  ...
  const contextualFollowUp = intentRecognition?.contextualFollowUp || null;
  const structuredCommercialFollowUp =
    !!contextualFollowUp?.contextualCommercialAuthorized ||
    (contextualFollowUp?.detected &&
      isCommercialFollowUpContinuationSignal(message) &&
      (comparisonValid || commercialAnchorValid));

  if (
    structuredCommercialFollowUp &&
    (comparisonValid || commercialAnchorValid || currentCommercialAsk) &&
    !commercialExecutionFromContinuation
  ) {
    commercialContinuationEligible = true;
    if (comparisonValid) comparisonContinuationEligible = true;
    commercialExecutionFromContinuation = true;
    reasonCodes.push("structured_commercial_follow_up_continuation");
  }
