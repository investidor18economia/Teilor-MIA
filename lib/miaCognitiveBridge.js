 ...
 *
 * EXPLANATION_REQUEST, VALUE_QUESTION, PRIORITY_SHIFT e OBJECTION mapeiam para "decision" porque:
 *   - Não podem virar nova busca
 *   - Devem manter o âncora
 *   - O branch "decision" é o que aciona o rich explanation path e o
 *     priority_shift_response_contract / objection_response_contract
 *
 * PRIORITY_SHIFT (PATCH 7.6P): usuário muda eixo decisório (segurança,
 *   confiabilidade, tranquilidade) sem pedir produto novo. Intent "decision"
 *   garante context block ativo, anchor protegido, sem re-search ou rerank.
 *
 * OBJECTION (PATCH 7.6S-B): objeção/hesitação/ansiedade com âncora ativa
 *   não é nova busca — é continuação decisória ancorada (purchase_anxiety,
 *   hesitation, risk_probe, not_convinced, lack_confidence).
 */
export const COGNITIVE_TO_LEGACY_INTENT_MAP = Object.freeze({
  EXPLANATION_REQUEST: "decision",
  VALUE_QUESTION:      "decision",
  PRIORITY_SHIFT:      "decision",
  OBJECTION:           "decision",
 ...
 *
 * PRIORITY_SHIFT foi adicionado em PATCH 7.6P (antes ficava com legacy).
 * OBJECTION foi adicionado em PATCH 7.6S-B (antes ficava com legacy).
 */
...
export function mapCognitiveTurnToLegacyIntent(cognitiveTurn) {
  ...
  // PATCH 7.9X-D.3 — ANTI_REGRET contract precedes generic OBJECTION→decision fallback
  if (_isAntiRegretBridgeSignal(cognitiveTurn)) {
    ...
    return {
      active: true,
      intent: "anti_regret",
      reason: "anti_regret_family_bridge_authority",
      cognitiveTurnType: turnType,
      confidence,
    };
  }
 ...
 * EXPLANATION_REQUEST e VALUE_QUESTION (originais) + PRIORITY_SHIFT (PATCH 7.6P)
 * + OBJECTION (PATCH 7.6S-B).
 */
const _GUARD_SCOPE_TURN_TYPES = new Set([
  "EXPLANATION_REQUEST",
  "VALUE_QUESTION",
  "PRIORITY_SHIFT",
  "OBJECTION",
...
/** PATCH 7.9X-D.3 — dedicated contract route for emotional risk-reduction family */
const _ANTI_REGRET_CONTEXT_ACTION = "anti_regret";

function _isAntiRegretBridgeSignal(cognitiveTurn) {
  return cognitiveTurn?.signals?.isAntiRegret === true;
}

function _expectedBridgedIntent(cognitiveTurn) {
  if (_isAntiRegretBridgeSignal(cognitiveTurn)) {
    return "anti_regret";
  }
  return COGNITIVE_TO_LEGACY_INTENT_MAP[cognitiveTurn?.turnType] ?? null;
}

function _expectedContextActionForCognitiveTurn(cognitiveTurn) {
  if (_isAntiRegretBridgeSignal(cognitiveTurn)) {
    return _ANTI_REGRET_CONTEXT_ACTION;
  }
  return _expectedContextActionForTurnType(cognitiveTurn?.turnType);
}
 ...
 *   - cognitiveTurnType é EXPLANATION_REQUEST, VALUE_QUESTION, PRIORITY_SHIFT ou OBJECTION
...
export function guardContextActionWithCognitiveBridge({
  ...
  // PATCH 7.9X-D.3 — align contextAction with anti_regret bridge contract
  if (bridgeApplied && bridgeAudit?.toIntent === "anti_regret") {
    if (contextAction !== _ANTI_REGRET_CONTEXT_ACTION) {
      return {
        contextAction: _ANTI_REGRET_CONTEXT_ACTION,
        applied: true,
        reason: "anti_regret_bridge_context_action_alignment",
        fromContextAction: contextAction,
        toContextAction: _ANTI_REGRET_CONTEXT_ACTION,
        cognitiveTurnType,
        bridgeApplied,
      };
    }
    ...
    return {
      contextAction,
      applied: false,
      reason: "anti_regret_context_action_already_aligned",
      fromContextAction: contextAction,
...
export function buildCognitiveBridgeImpactAudit({
  ...
  if (bridgeApplied) {
    ...
    const expectedIntent = _expectedBridgedIntent(cognitiveTurnEarly);
    if (expectedIntent && finalIntent !== expectedIntent) {
      remainingDivergenceFlags.push("FINAL_INTENT_MISMATCH");
    }

    // 2. contextAction final deve ser compatível com o turnType / família
    const expectedContextAction = _expectedContextActionForCognitiveTurn(cognitiveTurnEarly);
...
const _UNCHECKED_TURN_TYPES = new Set([
  "CONVERSATIONAL", "UNKNOWN", "REACTION",
  "FOLLOW_UP", "COMMERCIAL_QUESTION",
  // PRIORITY_SHIFT removido em PATCH 7.6P — agora coberto pela bridge
  // OBJECTION removido em PATCH 7.6S-B — agora coberto pela bridge
]);
