...
/**
 * Commercial delivery recovery — structural only; never apply social fallback pools.
 */
export function applyCommercialConversationRecovery({
  reply = "",
  universalContract = null,
} = {}) {
  if (!isReplyEmpty(reply)) {
    return {
      ...
      chain: { valid: true, approved: [VALIDATOR_IDS.STRUCTURAL], rejected: [] },
      universalContract,
      reasonCodes: [],
...
export function applyUniversalConversationRecovery({
  ...
  if (deliveryMode === "commercial") {
    return applyCommercialConversationRecovery({ reply, universalContract });
  }
