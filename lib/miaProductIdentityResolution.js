...
/** PATCH 4A.2VF — remove cauda de pergunta de avaliação antes de resolver produto. */
const PRODUCT_EVALUATION_TAIL_PATTERNS = [
  /\s+(?:vale a pena|compensa|presta|e bom|e boa|vale comprar|vale mesmo|devo comprar|faz sentido comprar|boa escolha|boa compra|e uma boa|recomendado|recomendada)\??\s*$/i,
  /\s+(?:vale|compensa|presta)\??\s*$/i,
  /\s+(?:nao seria|seria)\s+(?:uma\s+)?(?:escolha\s+)?(?:melhor|boa)\??\s*$/i,
  /^(?:e\s+)?(?:o|a)\s+/i,
  /^(?:vale comprar(?: o| a)?|quero saber se o|quero saber se a|me fala(?: do| da)?|fala(?: do| da)?|o que acha do|o que acha da)\s+/i,
];
...
const ATTRIBUTE_CONSTRAINT_PATTERN =
  /\b(com|com uma|com um|que tenha|que tenham|precisa ter|preciso de|de)\s+(?:boa|melhor|mais|bom|grande|menor|barat\w*)\s+(?:bateria|autonomia|camera|cam|câmera|desempenho|performance|tela|memoria|memória|armazenamento|nfc|5g)\b|\b(boa bateria|bateria boa|boa camera|boa câmera|boa autonomia|mais bateria|bateria melhor)\b/i;

function hasResolvedModelSignal(mention = "") {
  const compact = normalizeKey(mention).replace(/\s+/g, "");
  return (
    /\b[a-z]{1,6}\d{1,3}[a-z]{0,4}\b/i.test(compact) ||
    /\b(iphone|galaxy|moto|redmi|poco|rog|zenfone|note)\b/i.test(normalizeKey(mention))
  );
}
...
const VARIANT_RULES = [
  ...
  {
    test: /\b(?:samsung\s+)?a(\d{2,3})(?:\s*5g)?\b/i,
    display: (m) => `Galaxy A${m[1]}`,
    short: (m) => `A${m[1]}`,
  },
  {
    test: /\ba(\d{2,3})(?:\s*5g)?\b/i,
    display: (m) => `Galaxy A${m[1]}`,
    short: (m) => `A${m[1]}`,
  },
  {
...
export function extractProductMentionFromQuery(query = "") {
  ...
  for (const pattern of PRODUCT_EVALUATION_TAIL_PATTERNS) {
...
const SPECIFIC_PRODUCT_EVALUATION_PATTERN =
  /\b(vale a pena|compensa|e bom|e boa|presta|vale comprar|vale mesmo|devo comprar|faz sentido comprar|boa escolha|boa compra|e uma boa|recomendado|recomendada)\b/;

/**
 * PATCH 4A.2VF — produto específico + pergunta de avaliação ("Galaxy A55 vale a pena?").
 * Generaliza por modelo resolvido, não por nome fixo.
 */
export function isSpecificProductEvaluationQuery(query = "") {
  const raw = cleanText(query);
  if (!raw || !SPECIFIC_PRODUCT_EVALUATION_PATTERN.test(normalizeKey(raw))) return false;

  const mention = extractProductMentionFromQuery(raw);
  if (!mention || !hasResolvedModelSignal(mention)) return false;

  const identity = resolveProductIdentityFromQuery(mention);
  return !!identity.officialName;
}

/**
 * Ancora textual canônica para lock/busca em avaliação de produto específico.
 * Ignora prefixos comerciais concatenados ("celular até 2.500. o Galaxy A55...").
 */
export function resolveSpecificProductEvaluationAnchorQuery(query = "") {
  if (!isSpecificProductEvaluationQuery(query)) return "";
  const identity = resolveProductIdentityFromQuery(query);
  return cleanText(identity.officialName || extractProductMentionFromQuery(query));
}

/**
 * Verifica se uma oferta corresponde ao produto citado na query (avaliação ou menção).
 */
export function productOfferMatchesQueryIdentity(offer = {}, query = "") {
  const anchor =
    resolveSpecificProductEvaluationAnchorQuery(query) ||
    resolveProductIdentityFromQuery(query).officialName ||
    extractProductMentionFromQuery(query);
  if (!anchor) return false;

  const resolution = buildProductResolutionKeys(anchor);
  const offerName = cleanText(
    offer?.product_name || offer?.trustedSpecs?.official_name || ""
  );
  ...
  for (const key of resolution.keys) {
    ...
    if (offerKey.includes(normalized) || normalized.includes(offerKey)) {
      return true;
    }
  }
...
/**
 * Produto específico + constraint implícito ("S24 com boa bateria")
 * ou avaliação direta ("Galaxy A55 vale a pena?").
 * Deve abrir pipeline comercial sem verbos explícitos de compra.
 */
export function detectImplicitProductEvaluationQuery(query = "") {
  if (isSpecificProductEvaluationQuery(query)) return true;

  const raw = cleanText(query);
  if (!raw || !ATTRIBUTE_CONSTRAINT_PATTERN.test(raw)) return false;
