...
import { instrumentPriceIntelligenceAnalyticsFromOfferSet, emitPriceIntelligenceAnalytics } from "./miaPriceIntelligenceAnalytics.js";
import { instrumentSavingsEstimationAnalyticsFromOfferSet, emitSavingsEstimationAnalytics } from "./miaSavingsEstimationAnalytics.js";
import { emitAntiRegretFoundationAnalytics } from "./miaAntiRegretFoundationAnalytics.js";
import { emitUserValueOutcomeAnalytics } from "./miaUserValueOutcomeAnalytics.js";
import { getRecommendationDecisionMetadataSnapshot } from "./miaRecommendationDecisionAnalytics.js";
import {
 ...
 * }} input
 */
export async function instrumentOfferSetAnalyticsForDelivery(supabase, input = {}) {
  const bucket = getOfferSetAnalyticsBucket();
  ...
  const decisionSnapshot = getRecommendationDecisionMetadataSnapshot();

  await emitOfferSetAnalytics(supabase, {
    requestId,
    analyticsContext,
  ...
  await emitPriceIntelligenceAnalytics(supabase, {
    requestId,
    analyticsContext,
  ...
  await emitSavingsEstimationAnalytics(supabase, {
    requestId,
    analyticsContext,
    offerSetMetadata: metadata,
    controlledTest: input.controlledTest,
  ...
  await emitAntiRegretFoundationAnalytics(supabase, {
    ...
    analyticsContext,
    offerSetMetadata: metadata,
    decisionMetadata: decisionSnapshot,
