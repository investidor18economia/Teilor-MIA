...
export const SOCIAL_HUMANIZATION_VERSION = "5.8.8.3";
...
export const SOCIAL_HUMANIZATION_BEHAVIOR = Object.freeze({
  ...
  CURIOSITY_ENGAGEMENT: "curiosity_engagement",
});
...
const CATEGORY_PATTERNS = [
  ...
  { category: EMOTIONAL_CATEGORY.GRATITUDE, pattern: /\b(obrigad\w*|valeu|vlw|agrade\w*|tmj|brigad\w*|thanks)\b/i },
  ...
  {
    category: EMOTIONAL_CATEGORY.NEUTRAL_SOCIAL,
    pattern:
      /\b(acredita|olha\s+isso|tenho\s+(?:uma\s+)?novidade|adivinha|aconteceu\s+(?:um\s+)?neg[oó]cio|preciso\s+te\s+contar)\b/i,
  },
];
...
const COLD_GRATITUDE_PATTERN = /^(disponha\.?!?|de\s+nada\.?!?|por\s+nada\.?!?)\s*$/i;
...
function resolveHumanizationBehavior(category, contract = {}, recognition = {}) {
  ...
  switch (category) {
    ...
    case EMOTIONAL_CATEGORY.NEUTRAL_SOCIAL:
      if (
        /\b(acredita|novidade|adivinha|olha\s+isso|preciso\s+te\s+contar)\b/i.test(
          normalizeText(contract.resolvedQuery || "")
        )
      ) {
        return SOCIAL_HUMANIZATION_BEHAVIOR.CURIOSITY_ENGAGEMENT;
      }
      if (
...
export function buildGratitudeWithPresenceReply(contract = {}) {
  const key = warmthKey(contract);
  const pools = {
    warm_light: ["Imagina!", "Por nada — tamo junto!", "Disponha — fico feliz!"],
    warm_balanced: [
...
export function buildCuriosityEngagementReply(contract = {}) {
  const key = warmthKey(contract);
  const pools = {
    warm_light: ["Conta! Tô curiosa.", "Opa — manda!", "Aí sim, fala!"],
    warm_balanced: [
      "Conta — fiquei curiosa!",
      "Opa, me fala — quero saber!",
      "Aí sim — manda ver, estou ouvindo.",
    ],
    warm_reserved: ["Me conta — fiquei curiosa.", "Pode falar — estou ouvindo."],
  };
  return pickRhythmGovernedVariant(pools[key] || pools.warm_balanced, contract, "curiosity-engage");
}
...
export function buildHumanizationGovernedReply(contract = {}) {
  const behavior =
    ...
    (contract.conversationalIntent === "gratitude"
      ? SOCIAL_HUMANIZATION_BEHAVIOR.GRATITUDE_WITH_PRESENCE
      : null);
  ...
  switch (behavior) {
    ...
    case SOCIAL_HUMANIZATION_BEHAVIOR.CURIOSITY_ENGAGEMENT:
      return buildCuriosityEngagementReply(contract);
    default:
      return "";
  }
}
...
export function detectHumanizationViolations(text = "", contract = {}) {
  ...
  if (
    (behavior === SOCIAL_HUMANIZATION_BEHAVIOR.GRATITUDE_WITH_PRESENCE ||
      contract.conversationalIntent === "gratitude" ||
      contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.ACKNOWLEDGE_GRATITUDE) &&
    COLD_GRATITUDE_PATTERN.test(raw)
  ) {
    violations.push("cold_gratitude_response");
  }

  if (
    contract.conversationalIntentPolicy?.forbidBareAcknowledgement &&
    COLD_GRATITUDE_PATTERN.test(raw)
  ) {
  ...
  if (
    (behavior === SOCIAL_HUMANIZATION_BEHAVIOR.LIGHT_HUMOR_REACT ||
      behavior === SOCIAL_HUMANIZATION_BEHAVIOR.CURIOSITY_ENGAGEMENT ||
      contract.conversationalIntent === "curiosity_hook" ||
      contract.conversationalIntent === "small_talk") &&
    (COLD_EMOTIONAL_PATTERN.test(raw) || /^(ok|claro|certo|entendi)\.?\s*$/i.test(raw))
  ) {
    violations.push("cold_social_reaction");
  }
