...
export function buildSpecialistPresentationContract(input = {}) {
  ...
  const narrativePlan = sources.narrativePlan || null;
  const verbalizationPlan = sources.verbalizationPlan || null;
  const orderedLegacy = hasVerbalizationPlan(verbalizationPlan)
  ...
  contract.tradeoff = {
    gains: orderedLegacy?.gains?.length
      ? orderedLegacy.gains
      : [...(sources.gains || [])].map((entry) => cleanText(entry)).filter(Boolean),
    sacrifices: orderedLegacy?.sacrifices?.length
      ? orderedLegacy.sacrifices
      : [...(sources.sacrifices || [])].map((entry) => cleanText(entry)).filter(Boolean),
    structuredDecisionFacts: sources.structuredDecisionFacts || null,
    narrativePlan,
    verbalizationPlan,
    verbalizationStyleGovernance: sources.verbalizationStyleGovernance || null,
    llmStyleContract: sources.llmStyleContract || null,
    semanticUnits: sources.semanticUnits || [],
    semanticSacrificeUnits: sources.semanticSacrificeUnits || [],
    synthesisSource: sources.synthesisSource || null,
  };
