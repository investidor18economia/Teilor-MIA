/**
 * PATCH 12D / 3.3A.2 — Server-issued user session tokens for write endpoints.
 */

import crypto from "crypto";
import { ENDPOINT_REASON_CODES } from "./miaEndpointAccessPolicy.js";
import { getUserSessionSecret, MIA_USER_SESSION_SECRET_ENV } from "./miaAuthSecrets.js";

export { MIA_USER_SESSION_SECRET_ENV };
...
export function verifyUserSessionToken(token, env = process.env, now = Date.now()) {
  let secret;
  try {
    secret = getUserSessionSecret(env);
  } catch {
