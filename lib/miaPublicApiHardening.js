...
import { logInfo } from "./miaLogger.js";

export const PUBLIC_API_ORIGIN_BLOCK_REASONS = Object.freeze({
...
function resolveOriginHost(origin = "") {
  try {
    return new URL(String(origin || "").trim()).host;
  } catch {
    return "unparseable";
  }
}

export function sendPublicApiOriginRejected(res, { endpoint, origin } = {}) {
  const blockReason = origin
    ? PUBLIC_API_ORIGIN_BLOCK_REASONS.INVALID_ORIGIN
    : PUBLIC_API_ORIGIN_BLOCK_REASONS.MISSING_ORIGIN;
  ...
  logInfo({
    ...
    blockReason,
    originHost: origin ? resolveOriginHost(origin) : null,
  });
...
export function applyPublicCorsHeaders(req, res, env = process.env) {
  ...
  const devLocalhostAllowed =
    !isProductionRuntime(env) && isLocalDevelopmentOrigin(origin);

  if (allowedOrigins.has(origin) || devLocalhostAllowed) {
    res.setHeader("Access-Control-Allow-Origin", origin);
    res.setHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
    res.setHeader("Access-Control-Allow-Headers", "Content-Type");
    applyPublicSecurityHeaders(res, { varyOrigin: true });
    return { originAllowed: true, crossOrigin: true, origin };
  }

  return { originAllowed: false, crossOrigin: true, origin };
}
