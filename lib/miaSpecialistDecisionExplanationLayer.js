/**
 * PATCH 9.1A — Specialist Decision Explanation Layer
 *
 * Enriquece a primeira recomendação com explicação especializada de decisão.
 * Não altera winner, ranking, routing ou decision engine.
 */

import { extractBudget } from "./miaRoutingSafety.js";
import {
  buildStructuredExplanationFacts,
  findInventedSpecViolations,
  hasUsableDataLayerContent,
} from "./miaProductExplanationBuilder.js";
import { containsArchitectureLeak } from "./miaCommercialExplanationVerbalizer.js";
import {
  buildAuthoritySignal,
  shouldApplyAuthoritySignal,
} from "./miaAuthoritySignalsLayer.js";
import {
  buildTradeoffCommunicationBlock,
  shouldApplyTradeoffCommunication,
} from "./miaTradeoffCommunicationLayer.js";
import { cleanupMiaHumanLanguage } from "./miaAntiAiLanguageCleanupLayer.js";
import {
  buildDataLayerEvidenceInjection,
  shouldApplyDataLayerEvidenceInjection,
} from "./miaDataLayerEvidenceInjectionLayer.js";
import { getHumanizedTrustedSpecs, resolveUserFacingDataLayerText } from "./miaDataLayerHumanizationGuard.js";
import { isArtificialAttributeChain } from "./miaDataLayerSemanticNormalizer.js";
import {
  buildExpertInsight,
  shouldApplyExpertInsightGeneration,
} from "./miaExpertInsightGenerationLayer.js";
import { enrichConsequencesWithMicroImpacts } from "./miaCommercialMicroConsequenceLayer.js";
import {
  allocateSpecialistFamilies,
} from "./miaSemanticFamilyAllocationEngine.js";
import { buildSpecialistPresentationContract } from "./miaSpecialistPresentationContract.js";
import { buildSensationBridge } from "./miaSensationReasoningLayer.js";
import { buildAuthorityBridge } from "./miaAuthorityBridgeLayer.js";
import { buildAuthorityClosingContract } from "./miaAuthorityClosingContract.js";
import { buildHumanExperienceModel } from "./miaHumanSensationReasoningLayer.js";
import { buildHumanFrictionModel } from "./miaHumanFrictionModelingLayer.js";
import { buildOwnershipExperienceModel } from "./miaOwnershipExperienceLayer.js";
import { buildHumanDecisionNarrative } from "./miaHumanDecisionNarrativeEngine.js";
import { buildLongTermSatisfactionModel } from "./miaLongTermSatisfactionReasoningLayer.js";
import { buildUserPriorityWeightingModel } from "./miaUserPriorityWeightingEngine.js";
import { buildPersonalDecisionAdaptationModel } from "./miaPersonalDecisionAdaptationLayer.js";

export const SPECIALIST_DECISION_EXPLANATION_VERSION = "9.3C.1";

export const SPECIALIST_DECISION_FLAGS = Object.freeze({
  GENERIC_DECISION_EXPLANATION: "GENERIC_DECISION_EXPLANATION",
  MISSING_BUDGET_UNDERSTANDING: "MISSING_BUDGET_UNDERSTANDING",
  MISSING_DECISION_REASON: "MISSING_DECISION_REASON",
  ABSTRACT_CONSEQUENCE_WITHOUT_DECISION:
    "ABSTRACT_CONSEQUENCE_WITHOUT_DECISION",
  INVENTED_SPEC: "INVENTED_SPEC",
  HARD_CODED_PRODUCT_LOGIC: "HARD_CODED_PRODUCT_LOGIC",
  TOO_LONG_REVIEW: "TOO_LONG_REVIEW",
  AI_CLICHE: "AI_CLICHE",
  WINNER_CHANGED: "WINNER_CHANGED",
  CARD_REPLY_MISMATCH: "CARD_REPLY_MISMATCH",
  REGRESSION_8X: "REGRESSION_8X",
});

const AXIS_LABELS = Object.freeze({
  performance: "desempenho no uso diário",
  camera: "câmera e fotos",
  battery: "autonomia",
  screen: "tela",
  longevity: "longevidade e suporte",
  value: "custo-benefício",
  storage: "armazenamento",
});

const CATEGORY_TERMS = Object.freeze({
  celular: "celular",
  smartphone: "celular",
  notebook: "notebook",
  laptop: "notebook",
  tv: "TV",
  monitor: "monitor",
  fone: "fone",
  cadeira: "cadeira",
  console: "console",
  camera: "câmera",
  camara: "câmera",
  mouse: "mouse",
  teclado: "teclado",
});

const AI_CLICHE_PATTERNS = Object.freeze([
  /como assistente/i,
  /espero ter ajudado/i,
  /fico feliz em ajudar/i,
  /excelente escolha/i,
  /ótima pergunta/i,
  /otima pergunta/i,
]);

const ABSTRACT_CONSEQUENCE_PATTERNS = Object.freeze([
  /tarefas exigentes sem sentir/i,
  /sem sentir que o aparelho está no limite cedo demais/i,
  /menos sensação de limite quando o aparelho é exigido/i,
]);

const GENERIC_JUSTIFICATION_PATTERNS = Object.freeze([
  /\bpra essa busca,\s*o\b.*\bencaixa melhor\b/i,
  /\baparece como a opção mais alinhada\b/i,
  /\bparece uma opção interessante para quem está comparando ofertas\b/i,
  /\bparece a opção mais coerente dentro do contexto\b/i,
]);

function cleanText(value = "") {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function cleanProductName(value = "") {
  return cleanText(value);
}

function formatBudgetValue(budget) {
  const value = Number(budget);
  if (!Number.isFinite(value) || value <= 0) return "";
  return value.toLocaleString("pt-BR", { maximumFractionDigits: 0 });
}

function resolveBudgetFromQuery(query = "", explicitBudget = null) {
  if (explicitBudget != null && Number.isFinite(Number(explicitBudget))) {
    return Number(explicitBudget);
  }

  const fromExtractor = extractBudget(query);
  if (fromExtractor != null) return fromExtractor;

  const shorthand = String(query || "").toLowerCase().match(/\b(\d+(?:[.,]\d+)?)\s*k\b/);
  if (!shorthand) return null;

  const base = Number(String(shorthand[1]).replace(",", "."));
  if (!Number.isFinite(base) || base <= 0) return null;
  return Math.round(base * 1000);
}

function seedFromText(text = "") {
  return Array.from(String(text || "")).reduce(
    (acc, char) => acc + char.charCodeAt(0),
    0
  );
}

function pickVariant(items = [], seed = "") {
  const list = items.filter(Boolean);
  if (!list.length) return "";
  return list[seedFromText(seed) % list.length];
}

function resolveCategoryTerm(category = "", query = "") {
  const normalized = cleanText(category).toLowerCase();
  if (CATEGORY_TERMS[normalized]) return CATEGORY_TERMS[normalized];

  const queryNorm = cleanText(query).toLowerCase();
  for (const [token, label] of Object.entries(CATEGORY_TERMS)) {
    if (queryNorm.includes(token)) return label;
  }

  return normalized || "produto";
}

function resolveAxisLabel(axis = "") {
  return AXIS_LABELS[cleanText(axis).toLowerCase()] || "equilíbrio geral";
}

function stripTrailingPeriod(text = "") {
  return cleanText(text).replace(/[.!?]+$/, "");
}

function lowercaseLead(text = "") {
  const body = cleanText(text);
  if (!body) return "";
  return body.charAt(0).toLowerCase() + body.slice(1);
}

function normalizeParagraph(text = "") {
  const raw = String(text || "").trim();
  if (!raw) return "";

  if (raw.includes("\n")) {
    return raw
      .split("\n")
      .map((line) => line.replace(/\s+/g, " ").trim())
      .filter(Boolean)
      .join("\n");
  }

  return cleanText(raw);
}

function capitalizeParagraph(text = "") {
  const normalized = normalizeParagraph(text);
  if (!normalized) return "";

  if (normalized.includes("\n")) {
    const lines = normalized.split("\n");
    lines[0] = capitalizeLead(lines[0]);
    return lines.join("\n");
  }

  return capitalizeLead(normalized);
}

function capitalizeLead(text = "") {
  const body = cleanText(text);
  if (!body) return "";
  return body.charAt(0).toUpperCase() + body.slice(1);
}

function firstNonEmpty(values = []) {
  for (const value of values) {
    const text = cleanText(value);
    if (text) return text;
  }
  return "";
}

function buildBudgetFrame({ budget = null, query = "", querySignals = {}, categoryTerm = "produto" } = {}) {
  const formatted = formatBudgetValue(budget);
  const seed = `${query}-${budget || "none"}`;

  if (formatted) {
    const frames = [
      `Com até R$ ${formatted}, a escolha precisa ser bem feita — nessa faixa aparece muito ${categoryTerm} que parece forte no anúncio e envelhece rápido.`,
      `Com esse orçamento de até R$ ${formatted}, o que pesa é equilibrar preço com segurança no uso.`,
      `Dentro desse limite de R$ ${formatted}, a diferença entre acertar e errar a compra aparece rápido.`,
      `Com R$ ${formatted} para gastar, dá para pegar algo bom — mas a escolha precisa ser criteriosa nessa faixa.`,
    ];
    return pickVariant(frames, seed);
  }

  if (querySignals?.priceSensitive || /\b(barato|barata|economizar|gastar pouco|custo[- ]benef[ií]cio)\b/i.test(query)) {
    const frames = [
      "Para gastar pouco sem errar, a escolha precisa ir além do preço mais baixo.",
      "Quando o foco é economizar, o risco aqui é pagar barato e sentir limitação cedo demais.",
      "Nessa busca por custo-benefício, o barato só vale se ainda fizer sentido no uso real.",
    ];
    return pickVariant(frames, seed);
  }

  return "";
}

function buildDecisionReasonFromAllocation({
  decisionPick = null,
  primaryAxis = "",
  structuredFacts = null,
  searchCognition = null,
  decisionMemory = null,
} = {}) {
  const axisLabel = resolveAxisLabel(primaryAxis);

  if (decisionPick?.shortText) {
    if (decisionPick.secondaryShortText) {
      return {
        reason: `combina ${decisionPick.shortText} com ${decisionPick.secondaryShortText}`,
        axisLabel,
        compact: true,
      };
    }
    return {
      reason: decisionPick.shortText,
      axisLabel,
      compact: true,
    };
  }

  return buildDecisionReason({
    productName: "",
    primaryAxis,
    structuredFacts,
    searchCognition,
    decisionMemory,
  });
}

function buildDecisionReason({
  productName = "",
  primaryAxis = "",
  structuredFacts = null,
  searchCognition = null,
  decisionMemory = null,
} = {}) {
  const axisLabel = resolveAxisLabel(primaryAxis);
  const strength =
    firstNonEmpty(
      (structuredFacts?.strengthConsequences || []).filter(
        (entry) => !isArtificialAttributeChain(entry)
      )
    ) ||
    firstNonEmpty([decisionMemory?.lastWinnerAdvantages?.[0]]) ||
    stripTrailingPeriod(searchCognition?.consequenceChain?.impact || "") ||
    stripTrailingPeriod(searchCognition?.narrativeBlocks?.mainConsequence || "");

  const linkedConsequence = stripTrailingPeriod(
    searchCognition?.consequenceChain?.consequence || ""
  );

  if (strength && linkedConsequence && !strength.toLowerCase().includes(linkedConsequence.toLowerCase().slice(0, 12))) {
    return {
      reason: `${strength.toLowerCase()} — ${lowercaseLead(linkedConsequence)}`,
      axisLabel,
    };
  }

  if (strength) {
    return { reason: lowercaseLead(strength), axisLabel };
  }

  if (linkedConsequence) {
    return { reason: lowercaseLead(linkedConsequence), axisLabel };
  }

  const memoryReason = stripTrailingPeriod(decisionMemory?.lastDecisionReason || "");
  if (memoryReason) {
    return { reason: lowercaseLead(memoryReason), axisLabel };
  }

  return {
    reason: `entregar mais segurança no ${axisLabel} dentro do que essa busca pede`,
    axisLabel,
  };
}

function buildDecisionParagraph({
  productName = "",
  primaryAxis = "",
  reason = "",
  axisLabel = "",
  query = "",
  compact = false,
} = {}) {
  const seed = `${query}-${productName}-${primaryAxis}`;
  const name = cleanProductName(productName) || "este produto";
  const reasonBody = stripTrailingPeriod(reason);

  if (compact) {
    const variants = [
      `Minha escolha aqui é o ${name}. Ele entrega a decisão mais segura porque ${reasonBody}.`,
      `Eu iria no ${name} porque ${reasonBody} — e isso pesa mais do que parece nessa busca.`,
      `O ${name} ficou no topo porque ${reasonBody}.`,
    ];
    return capitalizeLead(pickVariant(variants, seed));
  }

  const variants = primaryAxis
    ? [
        `Eu iria no ${name} porque ele vence no ponto que mais pesa para essa busca — ${axisLabel}: ${reasonBody}.`,
        `Minha escolha aqui é o ${name}: ele entrega a decisão mais segura nessa busca porque ${reasonBody}.`,
        `O ${name} ficou no topo porque, para o que você pediu, ${reasonBody}.`,
      ]
    : [
        `Eu iria no ${name} porque ele entrega a decisão mais segura para essa busca: ${reasonBody}.`,
        `Minha escolha aqui é o ${name} — ${reasonBody}.`,
        `O ${name} venceu aqui porque ${reasonBody}.`,
      ];

  return capitalizeLead(pickVariant(variants, seed));
}

function buildStakesParagraph({
  structuredFacts = null,
  searchCognition = null,
  primaryAxis = "",
  query = "",
} = {}) {
  const idealFor = firstNonEmpty(
    (structuredFacts?.idealForConsequences || []).filter(
      (entry) => !isArtificialAttributeChain(entry)
    )
  );
  const note = firstNonEmpty(structuredFacts?.noteConsequences);
  const impact = stripTrailingPeriod(searchCognition?.consequenceChain?.impact || "");
  const seed = `${query}-${primaryAxis}-stakes`;

  if (idealFor) {
    const frames = [
      `O ponto principal aqui não é só preço — é comprar algo que ainda faça sentido para quem ${lowercaseLead(idealFor)}.`,
      `Isso importa porque funciona melhor para quem ${lowercaseLead(idealFor)}.`,
      `Na prática, a vantagem é servir quem ${lowercaseLead(idealFor)} sem parecer limitado cedo demais.`,
    ];
    return capitalizeLead(pickVariant(frames, seed));
  }

  if (note) {
    return capitalizeLead(
      `O que faz diferença aqui é que ${lowercaseLead(note)} — e isso pesa direto na decisão.`
    );
  }

  if (impact) {
    const frames = [
      `Isso importa porque ${lowercaseLead(impact)} — e foi isso que pesou na escolha.`,
      `O que pesa na decisão é ${lowercaseLead(impact)}, não só o preço do anúncio.`,
      `Na prática, ${lowercaseLead(impact)} — por isso ele saiu na frente.`,
    ];
    return capitalizeLead(pickVariant(frames, seed));
  }

  const axisLabel = resolveAxisLabel(primaryAxis);
  if (primaryAxis) {
    return capitalizeLead(
      `O que pesa aqui não é só preço — é ter mais segurança em ${axisLabel} sem abrir mão do que a busca pede.`
    );
  }

  return capitalizeLead(
    "O que pesa aqui não é só preço — é comprar algo que ainda faça sentido por mais tempo."
  );
}

function buildTradeoffParagraph({
  structuredFacts = null,
  searchCognition = null,
  decisionMemory = null,
  query = "",
} = {}) {
  const weakness =
    firstNonEmpty(structuredFacts?.weaknessConsequences) ||
    firstNonEmpty(structuredFacts?.avoidIfConsequences) ||
    resolveUserFacingDataLayerText(searchCognition?.tradeoffHonest || "") ||
    resolveUserFacingDataLayerText(searchCognition?.narrativeBlocks?.tradeoffHonest || "") ||
    resolveUserFacingDataLayerText(decisionMemory?.lastTradeoff || "") ||
    firstNonEmpty(decisionMemory?.lastWinnerSacrifices?.map((entry) => `menos destaque em ${entry}`));

  if (!weakness) return "";

  const seed = `${query}-tradeoff`;
  const frames = [
    `A escolha que você faz ao pegar ele é ganhar mais segurança no uso, mas ${lowercaseLead(weakness)}.`,
    `Você ganha na decisão principal, mas ${lowercaseLead(weakness)}.`,
    `Vale saber antes de fechar: ${lowercaseLead(weakness)}.`,
  ];

  return capitalizeLead(pickVariant(frames, seed));
}

function finalizeExplanation(paragraphs = [], meta = {}) {
  const cleanParagraphs = paragraphs
    .map((paragraph) => capitalizeParagraph(paragraph))
    .filter(Boolean)
    .slice(0, 8);

  const text = cleanParagraphs.join("\n\n");
  const cleaned = cleanupMiaHumanLanguage(text, {
    allowedEvidence: meta.allowedEvidence || "",
    winnerName: meta.productName || "",
    preserveStructure: true,
    preserveSpecialistStructure: true,
  });
  const finalText = cleaned.text || text;
  const violations = findInventedSpecViolations(finalText, meta.allowedEvidence || "");

  if (!finalText || violations.length > 0 || containsArchitectureLeak(finalText)) {
    return {
      ok: false,
      text: "",
      paragraphs: [],
      presentation: null,
      error: violations.length
        ? "invented_spec_detected"
        : containsArchitectureLeak(text)
          ? "architecture_leak_detected"
          : "empty_explanation",
      violations,
    };
  }

  return {
    ok: true,
    text: finalText,
    paragraphs: cleanParagraphs,
    presentation: meta.presentation || null,
    sensationBridge: meta.sensationBridge || null,
    humanExperienceModel: meta.humanExperienceModel || null,
    humanFrictionModel: meta.humanFrictionModel || null,
    ownershipExperienceModel: meta.ownershipExperienceModel || null,
    authorityBridge: meta.authorityBridge || null,
    authorityClosingContract: meta.authorityClosingContract || null,
    humanDecisionNarrative: meta.humanDecisionNarrative || null,
    longTermSatisfactionModel: meta.longTermSatisfactionModel || null,
    priorityWeightsModel: meta.priorityWeightsModel || null,
    personalDecisionAdaptationModel: meta.personalDecisionAdaptationModel || null,
    error: null,
    violations: [],
  };
}

/**
 * @param {{
 *   responsePath?: string,
 *   commercialOfferReset?: { shouldReset?: boolean },
 *   sessionContext?: Record<string, unknown>,
 *   routingDecision?: { allowNewSearch?: boolean },
 * }} input
 */
export function shouldApplySpecialistDecisionExplanation(input = {}) {
  if (input.responsePath && input.responsePath !== "return_seguro") {
    return false;
  }

  if (input.routingDecision?.allowNewSearch) return true;
  if (input.commercialOfferReset?.shouldReset) return true;

  const priorWinner = cleanProductName(
    input.sessionContext?.lastBestProduct?.product_name || ""
  );
  if (!priorWinner) return true;

  const priorQuery = cleanText(input.sessionContext?.lastQuery || "").toLowerCase();
  const currentQuery = cleanText(input.query || input.resolvedQuery || "").toLowerCase();
  if (priorQuery && currentQuery && priorQuery !== currentQuery) return true;

  return false;
}

/**
 * @param {{
 *   query?: string,
 *   budget?: number|null,
 *   category?: string,
 *   product?: Record<string, unknown>,
 *   searchCognition?: Record<string, unknown>,
 *   decisionMemory?: Record<string, unknown>,
 *   querySignals?: Record<string, unknown>,
 * }} input
 */
export function buildSpecialistDecisionExplanation(input = {}) {
  const query = cleanText(input.query || "");
  const product = input.product && typeof input.product === "object" ? input.product : {};
  const searchCognition = input.searchCognition || {};
  const decisionMemory = input.decisionMemory || {};
  const querySignals = input.querySignals || {};

  const humanizedTrustedSpecs = getHumanizedTrustedSpecs(product.trustedSpecs || null);
  const safeProduct =
    humanizedTrustedSpecs && product.trustedSpecs
      ? { ...product, trustedSpecs: { ...product.trustedSpecs, ...humanizedTrustedSpecs } }
      : product;

  const productName =
    cleanProductName(safeProduct.trustedSpecs?.official_name) ||
    cleanProductName(safeProduct.product_name) ||
    "este produto";

  const budget = resolveBudgetFromQuery(query, input.budget);

  const categoryTerm = resolveCategoryTerm(input.category || product.category || "", query);
  const primaryAxis = cleanText(searchCognition.primaryAxis || input.activePriority || "");

  const priorityWeightsModel = buildUserPriorityWeightingModel({
    query,
    budget,
    primaryAxis,
    userSignals: querySignals,
    querySignals,
    constraints: input.constraints || [],
    reasoning: {
      dominance: searchCognition.dominance || "",
      impact: searchCognition.consequenceChain?.impact || "",
      consequence: searchCognition.consequenceChain?.consequence || "",
    },
    searchCognition,
    context: { query, primaryAxis, category: categoryTerm },
  });

  const enrichedQuerySignals = {
    ...querySignals,
    priorityWeights: priorityWeightsModel.priorityWeights || null,
  };

  const personalDecisionAdaptationModel = buildPersonalDecisionAdaptationModel({
    query,
    budget,
    userSignals: querySignals,
    querySignals: enrichedQuerySignals,
    priorityWeights: priorityWeightsModel.priorityWeights,
    tradeoffs: {
      gains: [],
      sacrifices: [],
    },
    constraints: input.constraints || [],
    reasoning: {
      dominance: searchCognition.dominance || "",
      impact: searchCognition.consequenceChain?.impact || "",
      consequence: searchCognition.consequenceChain?.consequence || "",
    },
    context: { query, primaryAxis, category: categoryTerm },
  });

  const fullyEnrichedQuerySignals = {
    ...enrichedQuerySignals,
    personalDecisionProfile: personalDecisionAdaptationModel.personalDecisionProfile || null,
  };

  const structuredFacts = enrichConsequencesWithMicroImpacts(
    buildStructuredExplanationFacts({
      product: safeProduct,
      query,
      trustedSpecs: safeProduct.trustedSpecs || null,
      hasDataLayer:
        !!safeProduct.isDataLayerProduct ||
        hasUsableDataLayerContent(safeProduct.trustedSpecs || null),
    })
  );

  const semanticAllocation = allocateSpecialistFamilies({
    structuredFacts,
    trustedSpecs: safeProduct.trustedSpecs || null,
    primaryAxis,
    searchCognition,
    category: categoryTerm,
  });

  const budgetFrame = buildBudgetFrame({
    budget,
    query,
    querySignals: fullyEnrichedQuerySignals,
    categoryTerm,
  });

  const { reason, compact } = buildDecisionReasonFromAllocation({
    decisionPick: semanticAllocation.decision,
    primaryAxis,
    structuredFacts,
    searchCognition,
    decisionMemory,
  });

  const paragraphs = [];
  const presentationParts = {
    intro: [],
    evidence: [],
    insight: [],
    tradeoffSources: null,
  };

  if (budgetFrame) {
    paragraphs.push(budgetFrame);
    presentationParts.intro.push(budgetFrame);
  }

  const decisionParagraph = buildDecisionParagraph({
    productName,
    primaryAxis,
    reason,
    axisLabel: resolveAxisLabel(primaryAxis),
    query,
    compact,
  });
  paragraphs.push(decisionParagraph);
  presentationParts.intro.push(decisionParagraph);

  let injectedEvidence = null;
  let sensationBridge = null;
  let humanExperienceModel = null;
  let humanFrictionModel = null;
  let ownershipExperienceModel = null;
  let authorityClosingContract = null;
  let humanDecisionNarrative = null;
  let longTermSatisfactionModel = null;
  let authorityBridge = null;

  if (
    shouldApplyDataLayerEvidenceInjection({
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
      product: safeProduct,
      structuredFacts,
      primaryAxis,
      query,
    })
  ) {
    const evidenceInjection = buildDataLayerEvidenceInjection({
      product: safeProduct,
      structuredFacts,
      searchCognition,
      decisionMemory,
      query,
      primaryAxis,
      existingParagraphs: paragraphs,
      allowedEvidence: structuredFacts.allowedEvidence || productName,
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
      semanticAllocationState: semanticAllocation.state,
      semanticCandidateData: semanticAllocation.candidateData,
    });

    if (evidenceInjection.ok && evidenceInjection.paragraph) {
      paragraphs.push(evidenceInjection.paragraph);
      presentationParts.evidence.push(evidenceInjection.paragraph);
      injectedEvidence = evidenceInjection.evidence || null;
    }
  }

  sensationBridge = buildSensationBridge({
    winner: productName,
    structuredFacts,
    semanticCandidateData: semanticAllocation.candidateData,
    searchCognition,
    decisionMemory,
    query,
    primaryAxis,
    category: categoryTerm,
    querySignals: fullyEnrichedQuerySignals,
    reasoning: {
      impact: searchCognition?.consequenceChain?.impact || "",
      consequence: searchCognition?.consequenceChain?.consequence || "",
      tradeoffHonest: searchCognition?.tradeoffHonest || "",
    },
  });

  humanExperienceModel = buildHumanExperienceModel({
    winner: productName,
    sensations: sensationBridge?.sensations || [],
    structuredFacts,
    semanticCandidateData: semanticAllocation.candidateData,
    searchCognition,
    decisionMemory,
    query,
    primaryAxis,
    category: categoryTerm,
    querySignals: fullyEnrichedQuerySignals,
    reasoning: {
      impact: searchCognition?.consequenceChain?.impact || "",
      consequence: searchCognition?.consequenceChain?.consequence || "",
      tradeoffHonest: searchCognition?.tradeoffHonest || "",
    },
    sensationBridge,
  });

  humanFrictionModel = buildHumanFrictionModel({
    winner: productName,
    sensations: sensationBridge?.sensations || [],
    experiences: humanExperienceModel?.experiences || [],
    structuredFacts,
    semanticCandidateData: semanticAllocation.candidateData,
    query,
    primaryAxis,
    category: categoryTerm,
    querySignals: fullyEnrichedQuerySignals,
    reasoning: {
      impact: searchCognition?.consequenceChain?.impact || "",
      consequence: searchCognition?.consequenceChain?.consequence || "",
      tradeoffHonest: searchCognition?.tradeoffHonest || "",
    },
    sensationBridge,
    humanExperienceModel,
  });

  ownershipExperienceModel = buildOwnershipExperienceModel({
    winner: productName,
    sensations: sensationBridge?.sensations || [],
    experiences: humanExperienceModel?.experiences || [],
    frictions: humanFrictionModel?.frictions || [],
    structuredFacts,
    semanticCandidateData: semanticAllocation.candidateData,
    tradeoffs: {
      gains: presentationParts.tradeoffSources?.gains || [],
      sacrifices: presentationParts.tradeoffSources?.sacrifices || [],
    },
    query,
    primaryAxis,
    category: categoryTerm,
    querySignals: fullyEnrichedQuerySignals,
    sensationBridge,
    humanExperienceModel,
    humanFrictionModel,
  });

  if (
    shouldApplyExpertInsightGeneration({
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
      product: safeProduct,
      structuredFacts,
      evidence: injectedEvidence,
      sensationBridge,
      humanExperienceModel,
      humanFrictionModel,
      ownershipExperienceModel,
    })
  ) {
    const expertInsight = buildExpertInsight({
      product: safeProduct,
      structuredFacts,
      searchCognition,
      decisionMemory,
      querySignals: fullyEnrichedQuerySignals,
      query,
      primaryAxis,
      category: categoryTerm,
      evidence: injectedEvidence,
      existingParagraphs: paragraphs,
      allowedEvidence: structuredFacts.allowedEvidence || productName,
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
      semanticAllocationState: semanticAllocation.state,
      semanticCandidateData: semanticAllocation.candidateData,
      sensationBridge,
      humanExperienceModel,
      humanFrictionModel,
      ownershipExperienceModel,
    });

    if (expertInsight.ok && expertInsight.paragraph) {
      paragraphs.push(expertInsight.paragraph);
      presentationParts.insight.push(expertInsight.paragraph);
    }
  }

  if (
    shouldApplyAuthoritySignal({
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
    })
  ) {
    const authority = buildAuthoritySignal({
      product: safeProduct,
      structuredFacts,
      searchCognition,
      decisionMemory,
      query,
      category: input.category || product.category || "",
      budget,
      primaryAxis,
      existingParagraphs: paragraphs,
      allowedEvidence: structuredFacts.allowedEvidence || productName,
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
    });

    if (authority.ok && authority.paragraph) {
      paragraphs.push(authority.paragraph);
    }
  }

  paragraphs.push(
    buildStakesParagraph({
      structuredFacts,
      searchCognition,
      primaryAxis,
      query,
    })
  );

  let tradeoffCommunication = null;

  if (
    shouldApplyTradeoffCommunication({
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
    })
  ) {
    tradeoffCommunication = buildTradeoffCommunicationBlock({
      structuredFacts,
      searchCognition,
      decisionMemory,
      query,
      primaryAxis,
      existingParagraphs: paragraphs,
      allowedEvidence: structuredFacts.allowedEvidence || productName,
      responsePath: input.responsePath || "return_seguro",
      sessionContext: input.sessionContext || {},
      intent: input.intent || "",
      semanticAllocationState: semanticAllocation.state,
      semanticCandidateData: semanticAllocation.candidateData,
      productName,
      category: categoryTerm,
      trustedSpecs: safeProduct.trustedSpecs || null,
      sourceOrigin: structuredFacts.mode === "data_layer" ? "data_layer" : "commercial",
    });

    if (tradeoffCommunication.ok && tradeoffCommunication.block) {
      paragraphs.push(tradeoffCommunication.block);
      presentationParts.tradeoffSources = tradeoffCommunication.sources || null;
    } else {
      const legacyTradeoff = buildTradeoffParagraph({
        structuredFacts,
        searchCognition,
        decisionMemory,
        query,
      });
      if (legacyTradeoff && !/\btradeoff\b/i.test(legacyTradeoff)) {
        paragraphs.push(legacyTradeoff);
      }
      tradeoffCommunication = null;
    }
  }

  authorityBridge = buildAuthorityBridge({
    winner: productName,
    searchCognition,
    primaryAxis,
    query,
    querySignals: fullyEnrichedQuerySignals,
    sensations: sensationBridge?.sensations || [],
    tradeoffs: {
      gains: presentationParts.tradeoffSources?.gains || [],
      sacrifices: presentationParts.tradeoffSources?.sacrifices || [],
    },
  });

  authorityClosingContract = buildAuthorityClosingContract({
    winner: productName,
    dominance: searchCognition?.dominance || "",
    tradeoffs: {
      gains: presentationParts.tradeoffSources?.gains || [],
      sacrifices: presentationParts.tradeoffSources?.sacrifices || [],
    },
    sensations: sensationBridge?.sensations || [],
    experiences: humanExperienceModel?.experiences || [],
    frictions: humanFrictionModel?.frictions || [],
    ownershipExperiences: ownershipExperienceModel?.ownershipExperiences || [],
    authorityBridge,
    searchCognition,
    reasoning: {
      impact: searchCognition?.consequenceChain?.impact || "",
      consequence: searchCognition?.consequenceChain?.consequence || "",
      tradeoffHonest: searchCognition?.tradeoffHonest || "",
    },
    query,
    primaryAxis,
    querySignals: fullyEnrichedQuerySignals,
  });

  if (authorityClosingContract?.closingAuthority?.contractGoverned) {
    authorityBridge = {
      ...authorityBridge,
      closingAuthority: authorityClosingContract.closingAuthority,
      contractGoverned: true,
      version: authorityClosingContract.version,
    };
  }

  humanDecisionNarrative = buildHumanDecisionNarrative({
    winner: productName,
    evidence: injectedEvidence,
    sensations: sensationBridge?.sensations || [],
    experiences: humanExperienceModel?.experiences || [],
    frictions: humanFrictionModel?.frictions || [],
    ownershipExperiences: ownershipExperienceModel?.ownershipExperiences || [],
    authorityContract: authorityClosingContract,
    tradeoffs: {
      gains: presentationParts.tradeoffSources?.gains || [],
      sacrifices: presentationParts.tradeoffSources?.sacrifices || [],
    },
    searchCognition,
    reasoning: {
      impact: searchCognition?.consequenceChain?.impact || "",
      consequence: searchCognition?.consequenceChain?.consequence || "",
      tradeoffHonest: searchCognition?.tradeoffHonest || "",
    },
    query,
    primaryAxis,
    querySignals: fullyEnrichedQuerySignals,
  });

  longTermSatisfactionModel = buildLongTermSatisfactionModel({
    winner: productName,
    sensations: sensationBridge?.sensations || [],
    experiences: humanExperienceModel?.experiences || [],
    frictions: humanFrictionModel?.frictions || [],
    ownershipExperiences: ownershipExperienceModel?.ownershipExperiences || [],
    authorityContract: authorityClosingContract,
    narrative: humanDecisionNarrative,
    tradeoffs: {
      gains: presentationParts.tradeoffSources?.gains || [],
      sacrifices: presentationParts.tradeoffSources?.sacrifices || [],
    },
    query,
    primaryAxis,
    querySignals: fullyEnrichedQuerySignals,
  });

  const presentation = buildSpecialistPresentationContract(presentationParts);

  return finalizeExplanation(paragraphs, {
    allowedEvidence: structuredFacts.allowedEvidence || productName,
    productName,
    budgetDetected: budget,
    category: categoryTerm,
    primaryAxis,
    hasDataLayer: structuredFacts.mode === "data_layer",
    authoritySignalApplied: paragraphs.length >= 4,
    evidenceInjectionApplied: paragraphs.some((entry) =>
      /detalhe que|ponto que|fez diferen[cç]a|ganhar essa disputa|ganha for[cç]a|esquecem que|presta aten[cç][aã]o/i.test(
        entry
      )
    ),
    expertInsightApplied: paragraphs.some((entry) =>
      /costuma pesar mais|isso costuma importar|na pr[aá]tica, isso pesa|isso muda a decis[aã]o|n[aã]o conecta|entra na decis[aã]o/i.test(
        entry
      )
    ),
    presentation,
    sensationBridge,
    humanExperienceModel,
    humanFrictionModel,
    ownershipExperienceModel,
    authorityBridge,
    authorityClosingContract,
    humanDecisionNarrative,
    longTermSatisfactionModel,
    priorityWeightsModel,
    personalDecisionAdaptationModel,
  });
}

export function auditSpecialistDecisionExplanation(text = "", context = {}) {
  const body = cleanText(text);
  const flags = [];
  const budgetInQuery =
    context.budgetDetected ??
    resolveBudgetFromQuery(context.query || "");

  if (!body) {
    flags.push(SPECIALIST_DECISION_FLAGS.MISSING_DECISION_REASON);
    return flags;
  }

  if (GENERIC_JUSTIFICATION_PATTERNS.some((pattern) => pattern.test(body))) {
    flags.push(SPECIALIST_DECISION_FLAGS.GENERIC_DECISION_EXPLANATION);
  }

  if (
    budgetInQuery &&
    !/\b(orçamento|orcamento|faixa|limite|at[eé]\s*r\$|r\$\s*\d|com\s+r\$)\b/i.test(body)
  ) {
    flags.push(SPECIALIST_DECISION_FLAGS.MISSING_BUDGET_UNDERSTANDING);
  }

  const hasDecisionReason =
    /\b(porque|por que|vence|escolha|decis[aã]o|peso|pesou|ficou no topo|minha escolha|eu iria)\b/i.test(
      body
    );
  if (!hasDecisionReason) {
    flags.push(SPECIALIST_DECISION_FLAGS.MISSING_DECISION_REASON);
  }

  if (
    ABSTRACT_CONSEQUENCE_PATTERNS.some((pattern) => pattern.test(body)) &&
    !/\b(porque|por que|peso|pesou|decis[aã]o|escolha|importa)\b/i.test(body)
  ) {
    flags.push(SPECIALIST_DECISION_FLAGS.ABSTRACT_CONSEQUENCE_WITHOUT_DECISION);
  }

  if (findInventedSpecViolations(body, context.allowedEvidence || "").length > 0) {
    flags.push(SPECIALIST_DECISION_FLAGS.INVENTED_SPEC);
  }

  if (AI_CLICHE_PATTERNS.some((pattern) => pattern.test(body))) {
    flags.push(SPECIALIST_DECISION_FLAGS.AI_CLICHE);
  }

  if (body.length > 1400) {
    flags.push(SPECIALIST_DECISION_FLAGS.TOO_LONG_REVIEW);
  }

  return flags;
}

export function buildSpecialistDecisionExplanationAuditRecord(input = {}) {
  const resolvedBudget = resolveBudgetFromQuery(input.query || "", input.budget);
  const result = buildSpecialistDecisionExplanation(input);
  const flags = auditSpecialistDecisionExplanation(result.text, {
    query: input.query,
    budgetDetected: resolvedBudget,
    allowedEvidence:
      input.product?.trustedSpecs?.official_name ||
      input.product?.product_name ||
      "",
  });

  return {
    query: input.query || "",
    category: input.category || "",
    budgetDetected: resolvedBudget,
    winner:
      input.product?.trustedSpecs?.official_name ||
      input.product?.product_name ||
      "",
    responsePath: input.responsePath || "return_seguro",
    specialistExplanationDetected: result.ok,
    budgetUnderstandingDetected: !flags.includes(
      SPECIALIST_DECISION_FLAGS.MISSING_BUDGET_UNDERSTANDING
    ),
    decisionReasonDetected: !flags.includes(
      SPECIALIST_DECISION_FLAGS.MISSING_DECISION_REASON
    ),
    genericJustificationDetected: flags.includes(
      SPECIALIST_DECISION_FLAGS.GENERIC_DECISION_EXPLANATION
    ),
    inventedSpecDetected: flags.includes(SPECIALIST_DECISION_FLAGS.INVENTED_SPEC),
    hardcodedProductDetected: flags.includes(
      SPECIALIST_DECISION_FLAGS.HARD_CODED_PRODUCT_LOGIC
    ),
    tooLongDetected: flags.includes(SPECIALIST_DECISION_FLAGS.TOO_LONG_REVIEW),
    tooVagueDetected: flags.includes(
      SPECIALIST_DECISION_FLAGS.ABSTRACT_CONSEQUENCE_WITHOUT_DECISION
    ),
    aiClicheDetected: flags.includes(SPECIALIST_DECISION_FLAGS.AI_CLICHE),
    winnerChanged: false,
    cardMismatch: false,
    flags,
    text: result.text,
    ok: result.ok && flags.length === 0,
  };
}
