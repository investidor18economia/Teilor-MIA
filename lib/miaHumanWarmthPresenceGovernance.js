...
export const HUMAN_WARMTH_PRESENCE_VERSION = "5.8.8.2";
...
export function measureResponseWarmthPresence(text = "", contract = {}) {
  ...
  const coldGratitude =
    ...
    /^(de nada!?|por nada\.?|disponha!?)\s*$/i.test(raw) &&
    !warmthMarkers;
  ...
  return {
    ...
    functionallyCold: functionallyCold || coldAck || coldGratitude,
    missingPresence,
    structure,
    tokens,
  };
...
function buildWarmthCorrectedReply(contract = {}, extraSeed = "warmth-presence") {
  ...
  const pools = {
    ...
    [PRESENCE_RESPONSE_MODE.GRATITUDE_PRESENCE]: {
      ...
      warm_reserved: ["Por nada.", "De nada — fico contente em ajudar."],
    },
