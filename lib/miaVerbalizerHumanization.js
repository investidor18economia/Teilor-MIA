...
export const VERBALIZER_HUMANIZATION_VERSION = "3.5b.0";
...
export function buildHumanizedFirstAnswerOpening({ winner = "", gainPhrase = "", seed = "" } = {}) {
  ...
  return pickHumanizedVariant(
    [
      ...
      `Pelo que mapeei, o ${w} combina melhor porque ${normalizedGain}.`,
      `Ficaria com o ${w} porque ${normalizedGain}.`,
    ],
    `${seed}|${w}|${normalizedGain.slice(0, 24)}`
  );
}
