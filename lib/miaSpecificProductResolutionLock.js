...
export const SPECIFIC_PRODUCT_RESOLUTION_LOCK_VERSION = "3.3.0";
...
function countWords(value = "") {
  return normalizeQuery(value).split(" ").filter(Boolean).length;
}

function hasModelSignalInText(value = "") {
...
export function isGenericProductSearchQuery(query = "") {
  ...
  const hasModelSignal = hasModelSignalInText(q);

  const genericIntent =
    /\b(ate|ate|abaixo de|menos de|barato|barata|bom|boa|melhor|melhores|custo beneficio|custo-beneficio|recomend|indica|qual comprar|vale a pena|economico|econômico|procurar|buscar|busco|buscando)\b/.test(
      q
    );
  ...
  const categoryOnlySearch =
    /\b(procurar|buscar|busco|preciso de|quero)\b/.test(q) &&
    /\b(notebook|laptop|celular|smartphone|tv|monitor|mouse|teclado|geladeira|fone|macbook)\b/.test(
      q
    ) &&
    !hasModelSignal;

  if (categoryOnlySearch) return true;

  if (countWords(q) > 5 && !hasModelSignal && genericIntent) return true;

  return false;
}

export function hasMinimalProductIdentity(product = {}, { rawQuery = "" } = {}) {
  const name = cleanText(
    product?.product_name || product?.trustedSpecs?.official_name || ""
  );
  if (!name || name.length < 3) return false;

  if (countWords(name) > 6 && !hasModelSignalInText(name)) return false;
  if (product?.specificProductQueryAnchor && !hasModelSignalInText(name)) return false;

  return true;
}

/**
 * PATCH 11A.7F — lock candidate validation before runtime application.
 */
export function validateSpecificProductLockCandidate({
  query = "",
  lock = {},
  winnerBefore = null,
} = {}) {
  ...
  if (!lock?.active || !lock?.lockedProduct) {
    return {
      eligible: false,
      product: null,
      source: lock?.matchSource || null,
      reasonCodes: ["lock_inactive"],
      fallbackMode: "category_search",
      winnerPresentBeforeLock: !!winnerBefore?.product_name,
      stateMutationBlocked: true,
    };
  }

  if (isGenericProductSearchQuery(query)) {
    reasonCodes.push("generic_category_query");
    return {
      eligible: false,
      product: null,
      source: lock.matchSource || null,
  ...
  if (!hasMinimalProductIdentity(lock.lockedProduct, { rawQuery: query })) {
    reasonCodes.push("missing_valid_specific_product");
    return {
      eligible: false,
      product: null,
      source: lock.matchSource || null,
  ...
  if (lock.matchSource === "query_identity_anchor") {
    const identityName = normalizeProductKey(lock.lockedProduct.product_name || "");
    const queryKey = normalizeProductKey(extractProductMentionFromQuery(query));
    if (
      identityName &&
      queryKey &&
      identityName === queryKey &&
      !hasModelSignalInText(identityName)
    ) {
      reasonCodes.push("raw_query_as_product");
      return {
        eligible: false,
        product: null,
        source: lock.matchSource,
        reasonCodes,
        fallbackMode: "category_search",
        winnerPresentBeforeLock: !!winnerBefore?.product_name,
        stateMutationBlocked: true,
  ...
  return {
    eligible: true,
    product: lock.lockedProduct,
    source: lock.matchSource || null,
    reasonCodes: ["lock_candidate_valid"],
    fallbackMode: null,
    winnerPresentBeforeLock: !!winnerBefore?.product_name,
    stateMutationBlocked: false,
  };
}

function applyLockCandidateValidation(query = "", lock = {}, winnerBefore = null) {
  const validation = validateSpecificProductLockCandidate({ query, lock, winnerBefore });
  if (validation.eligible) return lock;
  return {
    active: false,
    ...
    matchScore: lock.matchScore || 0,
    reason: validation.reasonCodes[0] || "lock_rejected",
    validation,
  };
}

function finalizeSpecificProductLock(query = "", lock = {}, winnerBefore = null) {
  if (!lock?.active) return lock;
  return applyLockCandidateValidation(query, lock, winnerBefore);
}
...
function scoreProductForQuery(product = {}, query = "") {
  const resolution = buildProductResolutionKeys(query);
  const candidates = [
    query,
    resolution.mention,
    resolution.identity?.officialName,
    resolution.identity?.displayName,
    resolution.identity?.shortName,
  ].filter(Boolean);

  let best = { score: 0, source: null };
  for (const candidate of candidates) {
    const scored = scoreStrongSpecificProductMatch(product, candidate);
...
function resolveEffectiveProductLockQuery(rawQuery = "") {
  const cleaned = cleanText(rawQuery);
  const evalAnchor = resolveSpecificProductEvaluationAnchorQuery(cleaned);
  if (evalAnchor) return evalAnchor;

  const mention = extractProductMentionFromQuery(cleaned) || cleaned;
  if (isSpecificProductEvaluationQuery(cleaned)) {
    return resolveProductIdentityFromQuery(cleaned).officialName || mention;
  }
...
export function resolveSpecificProductLock(input = {}) {
  ...
  const query = resolveEffectiveProductLockQuery(rawQuery);
  ...
  const ranked = products
    .map((product) => {
      const scored = scoreProductForQuery(product, query);
      return {
        product,
        score: scored.score,
  ...
  return finalizeSpecificProductLock(rawQuery, {
    active: true,
    version: SPECIFIC_PRODUCT_RESOLUTION_LOCK_VERSION,
    query: rawQuery,
...
export function bootstrapSpecificProductLock(input = {}) {
  ...
  const baseLock = resolveSpecificProductLock({ query: rawQuery, products });
  if (baseLock.active) return finalizeSpecificProductLock(rawQuery, baseLock);
  if (!query || isGenericProductSearchQuery(query)) return baseLock;

  const identityFromResolver =
    typeof input.resolveIdentity === "function" ? input.resolveIdentity(rawQuery) : null;
  const identityFromRules = resolveProductIdentityFromQuery(query);
  const anchorName = cleanText(
    identityFromResolver?.officialName || identityFromRules.officialName || ""
  );
  if (!anchorName) return baseLock;

  const anchored = buildQueryAnchoredProduct(anchorName, rawQuery);
  const anchorScore = scoreProductForQuery(anchored, rawQuery);
  if (anchorScore.score < LOCK_THRESHOLD) return baseLock;

  const existing = products.find(
    (product) => scoreProductForQuery(product, rawQuery).score >= LOCK_THRESHOLD
  );
  if (existing) {
    return finalizeSpecificProductLock(
      rawQuery,
  ...
  if (top && scoreProductForQuery(top, rawQuery).score >= LOCK_THRESHOLD) {
    return baseLock;
  }
