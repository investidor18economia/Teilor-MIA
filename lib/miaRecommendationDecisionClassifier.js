...
import { buildRunnerUpAlternativeDecisionEnrichment } from "./miaRecommendationAlternativeClassifier.js";
import {
...
export function buildRecommendationDecisionMetadata(input = {}) {
  ...
  const winnerFamily = resolveSafeProductFamilyKey(winner);
  const scoreGap = rankInfo.runnerUpPresent ? computeScoreGap(winnerScore, runnerUpScore) : null;

  const alternativeEnrichment = buildRunnerUpAlternativeDecisionEnrichment({
    ...
    winnerScore,
    runnerUpScore,
    scoresAvailable: winnerScore != null && runnerUpScore != null,
  });
