...
export const COMPARATIVE_RUNNER_UP_REASONING_VERSION = "10.1C.2";
...
const GENERIC_TEMPLATES = Object.freeze([
  ...
  ({ winner, runnerUp, winnerEdge }) =>
    `Quase te recomendaria o ${runnerUp}; no fim, fiquei com o ${winner} — ${winnerEdge}.`,
  ({ winner, runnerUp, winnerEdge }) =>
...
const SPECIFIC_SOFT_TEMPLATES = Object.freeze([
  ...
  ({ winner, runnerUp, winnerEdge }) =>
    `O ${runnerUp} também apareceu entre as opções reais; ainda assim, para esta busca eu manteria o ${winner} — ${winnerEdge}.`,
]);
...
function polishWinnerEdgeForSpeech(edge = "", excludeAnchors = []) {
  const raw = cleanText(edge);
  if (!raw) return raw;

  const normalizedEdge = raw.toLowerCase();
  const isExcluded = excludeAnchors.some(
    (anchor) =>
      normalizedEdge.includes(anchor.toLowerCase()) ||
      anchor.toLowerCase().includes(normalizedEdge.slice(0, Math.min(24, normalizedEdge.length)))
  );

  if (
    /^(?:o|a|ele|é)\s/i.test(raw) ||
    /leva vantagem|fecha melhor|encaixa melhor|exatamente o modelo/i.test(raw)
  ) {
    if (!isExcluded) return raw.charAt(0).toLowerCase() + raw.slice(1);
  }

  const rewritten = rewriteConsequenceForSpeech(raw);
  const polished = rewritten.charAt(0).toLowerCase() + rewritten.slice(1);
  if (!isExcluded) return polished;

  if (/leva vantagem|fecha melhor|encaixa melhor|exatamente o modelo/i.test(raw)) {
...
export function resolveComparativeRunnerUpReasoning(input = {}) {
  ...
  const excludeSemanticAnchors = Array.isArray(input.excludeSemanticAnchors)
    ? input.excludeSemanticAnchors.map((entry) => cleanText(entry)).filter(Boolean)
    : [];
  ...
  const winnerEdge = polishWinnerEdgeForSpeech(
    extractWinnerEdge({
      winner,
      runnerUp,
      primaryAxis,
