...
const AXIS_CONSEQUENCE_FRAMES = Object.freeze({
  ...
  generic: {
    strength: {
      default:
        "ganho perceptível no uso real",
    },
    weakness: {
      default:
        "uma renúncia perceptível que vale pesar antes de fechar a compra",
    },
    ideal_for: {
...
function humanizeIdealForSlugToken(normalized = "") {
  return String(normalized || "")
    .replace(/_/g, " ")
    .replace(/\bcamera\b/gi, "câmera")
    .replace(/\bvideo\b/gi, "vídeo")
    .replace(/\bios\b/gi, "iOS")
    .replace(/\bpreco\b/gi, "preço")
    .replace(/\s+/g, " ")
    .trim();
}
...
function humanizeProseFallback(value = "", fieldType = "strength") {
  ...
  if (type === "weakness" || type === "risk" || type === "avoid_if") {
    ...
    const normalizedLabel = raw.toLowerCase();
    if (internalLabelMap[normalizedLabel]) return internalLabelMap[normalizedLabel];
    if (wordCount <= 2) return raw.charAt(0).toLowerCase() + raw.slice(1);
  ...
  return `funciona bem para ${lowered}`;
}
...
export function translateTokenToStructuredConsequence(token = "", fieldType = "strength") {
  ...
  const wordCount = raw.split(/\s+/).filter(Boolean).length;
  if (
    (type === "weakness" || type === "avoid_if" || type === "strength") &&
    wordCount >= 5
  ) {
    return {
      type,
  ...
  if (type === "ideal_for" && normalized.includes("_")) {
    if (/^quem_/i.test(normalized)) {
      return {
        type,
        sourceToken: normalized,
        consequence: humanizeIdealForSlugToken(normalized.replace(/^quem_/, "")),
      };
    }
    if (/^quer_/i.test(normalized)) {
      return {
        type,
        sourceToken: normalized,
        consequence: `quer ${humanizeIdealForSlugToken(normalized.replace(/^quer_/, ""))}`,
      };
    }
    if (/^modelo_/i.test(normalized)) {
      return {
        type,
        sourceToken: normalized,
        consequence: humanizeIdealForSlugToken(normalized.replace(/^modelo_/, "")),
      };
    }
  }
