...
export const SEMANTIC_TARGET_RESOLUTION_VERSION = "4.1I.3";
...
const RESPONSE_APPROVAL_PATTERN =
  /\b(gostei\s+da\s+resposta|boa\s+resposta|resposta\s+(?:foi\s+)?(?:otim\w*|boa|show|massa)|mandou\s+bem\s+na\s+resposta)\b/i;
...
function scanConversationForProductContext(conversationMessages = [], sessionContext = {}) {
  ...
  let hasRecentProductDiscussion = false;
  ...
  for (const msg of recent) {
    ...
    if (msg?.role === "user") {
      ...
      if (PRODUCT_ENTITY_PATTERN.test(content) || PRODUCT_DEMONSTRATIVE.test(content)) {
        hasRecentProductDiscussion = true;
  ...
  if (sessionProduct) {
    hasRecentProductDiscussion = true;
    lastProductMention = sessionProduct;
    hasCommercialContext = true;
  }

  return {
    hasRecentProductDiscussion,
    hasCommercialContext,
    lastProductMention,
    hasRecentMiaGreeting,
    hasProduct: hasRecentProductDiscussion || !!sessionProduct,
  };
}
...
export function resolveSemanticTarget(input = {}) {
  ...
  if (shortAestheticOnly) {
    if (productCtx.hasRecentProductDiscussion && productCtx.hasCommercialContext) {
      return {
    ...
    if (productCtx.hasRecentProductDiscussion) {
      return {
        target: SEMANTIC_TARGETS.PRODUCT,
        confidence: 0.78,
    ...
    if (productCtx.hasRecentMiaGreeting && !productCtx.hasCommercialContext) {
      return {
        target: SEMANTIC_TARGETS.MIA,
        confidence: 0.8,
    ...
    if (!productCtx.hasCommercialContext) {
      return {
        target: SEMANTIC_TARGETS.MIA,
        confidence: 0.72,
