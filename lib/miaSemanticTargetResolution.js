...
export const SEMANTIC_TARGET_RESOLUTION_VERSION = "4.1I.3.V.2.4";
...
function resolveFromTaxonomySignals(recognition = {}) {
  ...
  if (
    [SOCIAL_INTENT_FAMILIES.IRONY, SOCIAL_INTENT_FAMILIES.SARCASM, SOCIAL_INTENT_FAMILIES.JOKE, SOCIAL_INTENT_FAMILIES.HUMOR].includes(
      primary
    )
  ) {
    return {
      target: SEMANTIC_TARGETS.CONVERSATION,
      confidence: 0.8,
      reasonCodes: ["taxonomy_play_family"],
    };
  }

  if (primary === SOCIAL_INTENT_FAMILIES.CORRECTION || primary === SOCIAL_INTENT_FAMILIES.CONTEXT_REPAIR) {
    if (signals.some((s) => ["product_target", "product_rejection"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PRODUCT,
        confidence: 0.86,
        reasonCodes: ["taxonomy_correction_product_target"],
      };
    }
    if (MIA_ADDRESS.test(normalizeText(recognition.resolvedQuery || ""))) {
      return {
        target: SEMANTIC_TARGETS.MIA,
        confidence: 0.84,
        reasonCodes: ["taxonomy_correction_mia_target"],
      };
    }
    return {
      target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
      confidence: 0.82,
      reasonCodes: ["taxonomy_correction_previous_answer"],
    };
  }

  if (primary === SOCIAL_INTENT_FAMILIES.DISAPPROVAL) {
    if (signals.some((s) => ["product_target", "product_rejection"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PRODUCT,
        confidence: 0.88,
        reasonCodes: ["taxonomy_disapproval_product"],
      };
    }
    if (signals.some((s) => ["recommendation_target", "recommendation_rejection"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
        confidence: 0.86,
        reasonCodes: ["taxonomy_disapproval_recommendation"],
      };
    }
    if (signals.some((s) => ["response_target", "response_criticism"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
        confidence: 0.9,
        reasonCodes: ["taxonomy_disapproval_response"],
      };
    }
  }

  if (primary === SOCIAL_INTENT_FAMILIES.SOFT_DISAGREEMENT) {
    return {
      target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
      confidence: 0.8,
      reasonCodes: ["taxonomy_disagreement_previous_answer"],
    };
  }
...
function resolveSemanticTargetCore({
  ...
  if (primary === SOCIAL_INTENT_FAMILIES.CORRECTION || primary === SOCIAL_INTENT_FAMILIES.CONTEXT_REPAIR) {
    if (signals.some((s) => ["product_target", "product_rejection"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PRODUCT,
        confidence: 0.88,
        reasonCodes: ["negative_feedback_correction_product"],
        productReference: productCtx.lastProductMention,
      };
    }
    if (productCtx.hasRecentAssistantReply) {
      return {
        target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
        confidence: 0.9,
        reasonCodes: ["negative_feedback_correction_previous_answer"],
      };
    }
    if (MIA_ADDRESS.test(normalized)) {
      return {
        target: SEMANTIC_TARGETS.MIA,
        confidence: 0.86,
        reasonCodes: ["negative_feedback_correction_mia"],
      };
    }
  }
  ...
  if (primary === SOCIAL_INTENT_FAMILIES.DISAPPROVAL) {
    if (signals.some((s) => ["recommendation_target", "recommendation_rejection"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
        confidence: 0.92,
        reasonCodes: ["negative_feedback_recommendation_rejection"],
      };
    }
    if (signals.some((s) => ["product_target", "product_rejection"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PRODUCT,
        confidence: 0.9,
        reasonCodes: ["negative_feedback_product_rejection"],
        productReference: productCtx.lastProductMention,
      };
    }
    if (signals.some((s) => ["response_target", "response_criticism"].includes(s))) {
      return {
        target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
        confidence: 0.92,
        reasonCodes: ["negative_feedback_response_criticism"],
      };
    }
  }

  if (primary === SOCIAL_INTENT_FAMILIES.SOFT_DISAGREEMENT && productCtx.hasRecentAssistantReply) {
    return {
      target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
      confidence: 0.86,
      reasonCodes: ["negative_feedback_disagreement_previous_answer"],
    };
  }
  ...
  const contextualFollowUp = recognition.contextualFollowUp || null;
  if (contextualFollowUp?.contextualCommercialAuthorized) {
    const productName =
      contextualFollowUp.resolvedProduct?.product_name ||
      contextualFollowUp.resolvedReference?.product?.product_name ||
      null;
    const comparisonProducts = contextualFollowUp.comparisonProducts || contextualFollowUp.resolvedReference?.comparisonProducts || [];
    if (productName) {
      return {
        target: SEMANTIC_TARGETS.PRODUCT,
        confidence: 0.88,
        reasonCodes: ["commercial_continuation_contract", contextualFollowUp.contextSource || "contextual_follow_up"].filter(Boolean),
    ...
    if (comparisonProducts.length >= 2) {
      return {
        target: SEMANTIC_TARGETS.PREVIOUS_ANSWER,
        confidence: 0.84,
        reasonCodes: ["comparison_continuation_contract"],
        productReference: comparisonProducts[1]?.product_name || comparisonProducts[0]?.product_name || null,
      };
    }
  }
