...
export function hasRunningCommercialDiscourse(conversationMessages = [], { minMessages = 8 } = {}) {
  ...
  let substantiveAssistant = 0;

  for (const msg of recent) {
    const content = normalizeText(String(msg?.content || ""));
    if (!content) continue;
    if (COMMERCIAL_DISCOURSE_PATTERN.test(content)) commercialSignals += 1;
    if (msg?.role === "assistant" && content.length > 28) substantiveAssistant += 1;
  }

  return commercialSignals >= 2 && substantiveAssistant >= 1;
}
