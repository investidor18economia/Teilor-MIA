...
export const COMMERCIAL_FOLLOW_UP_VERSION = "11C.V.3.1";
...
/** Referential slots handled by other follow-up types — not generic dimensions. */
const REFERENTIAL_DIMENSION_TOKENS = new Set([
  "esse",
  "essa",
  "ele",
  "ela",
...
const PRICE_FOLLOW_UP_PATTERN =
  /\b(quanto custa|qto custa|qual o pre[cç]o|qual o valor|e quanto|e o pre[cç]o|e o valor|e pre[cç]o|por quanto|t[aá] por quanto|est[aá] por quanto|quanto [eé]|qual [eé] o pre[cç]o|onde est[aá] mais barato|onde [eé] mais barato)\b|^e\s+(?:o\s+)?pre[cç]o\??$/;
...
/**
 * PATCH 5.7V.3.1 — Structural dimension follow-up (category-agnostic).
 * Matches short "e [article?] {dimension}?" without a fixed attribute catalog.
 */
export function detectGenericDimensionFollowUpQuery(message = "") {
  const q = normalizeText(message);
  if (!q) return { detected: false, dimension: null, reasonCode: null };

  const tokens = q.split(/\s+/).filter(Boolean);
  if (tokens.length > 5) return { detected: false, dimension: null, reasonCode: null };

  const match = q.match(/^e\s+(?:(?:a|o|as|os)\s+)?([a-z]{2,24})(?:\s+[a-z]{1,12})?\??$/);
  if (!match) return { detected: false, dimension: null, reasonCode: null };

  const dimension = match[1];
  if (REFERENTIAL_DIMENSION_TOKENS.has(dimension)) {
    return { detected: false, dimension: null, reasonCode: null };
  }
  if (ORDINAL_SLOT_PRIMARY.test(q) || ORDINAL_SLOT_ALTERNATE.test(q)) {
    return { detected: false, dimension: null, reasonCode: null };
  }
  if (/^(esse|essa|ele|ela|isso|aquele|aquela|outro|outra)\b/.test(q)) {
    return { detected: false, dimension: null, reasonCode: null };
  }
  ...
  return {
    detected: true,
    dimension,
    reasonCode: "commercial_dimension_follow_up_preserves_target",
  };
}

export function getActiveRecommendedEntity(sessionContext = {}) {
  if (hasProductName(sessionContext.lastBestProduct)) {
    return {
      entity: sessionContext.lastBestProduct,
      source: "lastBestProduct",
      anchorType: "single_recommendation",
    };
  }
  ...
  if (ranking.length >= 1) {
    ...
    if (hasProductName(winner)) {
      return {
        entity: winner,
        source: "lastRankingSnapshot",
        anchorType: "ranking_winner",
      };
    }
  }
  ...
  if (products.length >= 1) {
    return {
      entity: products[0],
      source: "lastProducts",
      anchorType: "product_list_primary",
    };
  }
  return { entity: null, source: "none", anchorType: null };
}
...
export function classifyCommercialFollowUpType(message = "", sessionContext = {}) {
  ...
  const genericDimension = detectGenericDimensionFollowUpQuery(message);
  if (genericDimension.detected && hasValidCommercialSessionContext(sessionContext)) {
    return COMMERCIAL_FOLLOW_UP_TYPES.ATTRIBUTE_FOLLOW_UP;
  }
...
/**
 * PATCH 5.8.1 — Infer ongoing commercial discourse from conversation history alone.
 * Used when session anchors decay in long multiturn API calls.
 */
export function hasRunningCommercialDiscourse(conversationMessages = [], { minMessages = 8 } = {}) {
  const messages = Array.isArray(conversationMessages) ? conversationMessages : [];
  if (messages.length < minMessages) return false;

  let commercialSignalsRecent = 0;
  let substantiveAssistantRecent = 0;
  let commercialSignalsFull = 0;
  let substantiveAssistantFull = 0;

  for (let i = 0; i < messages.length; i += 1) {
    const msg = messages[i];
    const content = normalizeText(String(msg?.content || ""));
    if (!content) continue;
    const isRecent = i >= messages.length - 22;
    if (COMMERCIAL_DISCOURSE_PATTERN.test(content)) {
      commercialSignalsFull += 1;
      if (isRecent) commercialSignalsRecent += 1;
    }
    if (msg?.role === "assistant" && content.length > 28) {
      substantiveAssistantFull += 1;
      if (isRecent) substantiveAssistantRecent += 1;
    }
  }

  if (commercialSignalsRecent >= 2 && substantiveAssistantRecent >= 1) return true;

  // PATCH 5.8.1 — long thread: commercial discourse may sit outside the recent filler-heavy window
  if (messages.length >= 14 && commercialSignalsFull >= 2 && substantiveAssistantFull >= 2) {
    return true;
  }
...
export function resolveContextualCommercialFollowUp({
  ...
  const genericDimension = detectGenericDimensionFollowUpQuery(message);
  const activeRecommended = getActiveRecommendedEntity(enrichedSession);
  const attributeOrComparisonFollowUp =
    followUpType === COMMERCIAL_FOLLOW_UP_TYPES.ATTRIBUTE_FOLLOW_UP ||
    followUpType === COMMERCIAL_FOLLOW_UP_TYPES.COMPARISON_FOLLOW_UP ||
    followUpType === COMMERCIAL_FOLLOW_UP_TYPES.JUSTIFICATION_FOLLOW_UP;

  const authorizedByActiveCommercialEntity =
    attributeOrComparisonFollowUp &&
    contextValid &&
    (comparisonSet.length >= 2 || hasProductName(activeRecommended?.entity));

  const singleRecommendationRestored =
    followUpType === COMMERCIAL_FOLLOW_UP_TYPES.ATTRIBUTE_FOLLOW_UP &&
    contextValid &&
    hasProductName(activeRecommended?.entity) &&
  ...
  const followUpReasonCode =
    constraintRefinement?.reasonCode ||
    (singleRecommendationRestored
      ? "single_recommendation_anchor_restored"
      : genericDimension.detected
        ? genericDimension.reasonCode
        : authorizedByActiveCommercialEntity
          ? "active_commercial_entity_supports_attribute_follow_up"
          : "contextual_commercial_follow_up");
  ...
  return {
    ...
    requestedDimension: genericDimension.detected ? genericDimension.dimension : null,
    activeRecommendedEntity: activeRecommended?.entity || null,
    activeRecommendedSource: activeRecommended?.source || null,
    resolvedReference,
