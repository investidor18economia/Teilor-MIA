...
import { createCommercialRefinementMultiExtractor } from "./miaCommercialRefinementMultiExtract.js";

export const CONSTRAINT_REFINEMENT_VERSION = "3.7.1";
...
const CATEGORY_TOKEN_PATTERN =
  /\b(celular(?:es)?|cell(?:s)?|cel(?:s)?|smartphone(?:s)?|iphone(?:s)?|notebook(?:s)?|laptop(?:s)?|tv|televis(?:ao|ão|ões)|monitor(?:es)?|fone(?:s)?|headset(?:s)?|tablet(?:s)?|cadeira(?:s)?|geladeira(?:s)?|maquina de lavar|m[aá]quina de lavar|camera(?:s)?|c[aâ]mera(?:s)?|console(?:s)?|aspirador(?:es)?|tenis|t[eê]nis|perfume(?:s)?)\b/gi;

const INITIAL_COMMERCIAL_ENTRY_CUE_PATTERN =
  /\b(quero|queria|preciso|precisamos|procuro|procurar|to procurando|t[oô] procurando|to querendo|t[oô] querendo|me ajuda|ajuda a escolher|preciso comprar|to pensando|t[oô] pensando|pensando em comprar|qual celular|qual smartphone|indica|indique|recomenda|eu queria)\b/;

const INITIAL_COMMERCIAL_PRODUCT_CUE_PATTERN =
  /\b(celular|cell|cel|smartphone|iphone|samsung|motorola|moto|xiaomi|notebook|laptop|tablet|tv|fone|headset|geladeira|maquina de lavar|m[aá]quina de lavar|cadeira|console|aspirador|tenis|t[eê]nis|perfume)\b/;

export function hasRefinementSessionAnchor(sessionContext = {}) {
  if (String(sessionContext?.lastBestProduct?.product_name || "").trim()) return true;
  if (Array.isArray(sessionContext?.lastRankingSnapshot) && sessionContext.lastRankingSnapshot.length >= 1) {
    return true;
  }
  if (Array.isArray(sessionContext?.lastProducts) && sessionContext.lastProducts.length >= 1) {
    return true;
  }
  if (sessionContext?.lastCommercialConstraints) return true;
  return false;
}

/**
 * First-turn commercial entry (informal included) must not be treated as constraint refinement.
 */
export function isInitialCommercialEntryMessage(message = "", sessionContext = {}) {
  if (hasRefinementSessionAnchor(sessionContext)) return false;
  const q = normalizeText(message);
  if (!q || !detectActiveCommercialAsk(message)) return false;
  return (
    INITIAL_COMMERCIAL_ENTRY_CUE_PATTERN.test(q) &&
    INITIAL_COMMERCIAL_PRODUCT_CUE_PATTERN.test(q)
  );
}
...
const NEGATIVE_BRAND_PATTERN =
  /\b(sem\s+(\w+)|n[aã]o\s+quero\s+(?:\w+\s+)?(\w+)|pode excluir\s+(\w+)|pode tirar\s+(\w+)|\btira\s+(\w+)|qualquer marca menos\s+(\w+))\b/;

const SOCIAL_SEM_EXCLUSION_PATTERN =
  /\b(sem\s+assunto|sem\s+tempo|sem\s+ideia|sem\s+rumo|sem\s+pressa|sem\s+vontade|sem\s+nada\s+pra\s+falar|to\s+sem\s+assunto|t[oô]\s+sem\s+assunto)\b/i;
...
const BRAND_RESTRICTION_PATTERN =
  /\b(?:quero\s+)?(?:continuar\s+)?(?:so|só)\s+(?:entre\s+)?(\w+)\s+e\s+(\w+)\b|\b(?:so|só)\s+quero\s+(\w+)\b|\bquero\s+(?:so|só)\s+(\w+)\b/;

const PRIORITY_ADD_PATTERN =
  /\b(?:(?:agora\s+)?(?:priorizar|prioriza|prioridade(?:\s+(?:em|para))?|quero\s+priorizar)\s+(bateria|autonomia|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)|(?:agora\s+)?(bateria|autonomia|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)\s+(?:é|e)\s+(?:mais importante|minha prioridade|a prioridade)|minha prioridade\s+(?:é|e)\s+(bateria|autonomia|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)|(?:pra mim|para mim|p mim)\s+(?:o )?mais importante\s+(?:é|e)\s+(bateria|autonomia|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)|(?:pra mim|para mim|p mim)\s+(bateria|autonomia|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)\s+primeiro)\b/;

const BUDGET_REDUCTION_CUE_PATTERN =
  /\b(reduzir|reduz|diminuir|abaixar)\b.{0,20}\b(or[cç]amento|teto|limite|preco|pre[cç]o)\b|\b(or[cç]amento|teto)\s+(?:menor|mais baixo)\b/;

const MIXED_CLAUSE_SPLIT_PATTERN =
  /\s*(?:,\s*)?\s+(?:mas|porém|porem|so que|só que|ao mesmo tempo|desde que|contanto que|,\s*e)\s+\s*/i;

const COMMERCIAL_QUESTION_CLAUSE_PATTERN =
  /\b(quanto custa|qual (?:é|e) melhor|qual voce recomenda|me explica|como funciona)\b/;
...
const BUDGET_INCREASE_CUE_PATTERN =
  /\b(pode\s+(?:aumentar|subir|ir|chegar)|da\s+pra\s+(?:subir|aumentar|ir|chegar|ir)|aumentar\s+(?:para|pra|ate|at[eé])|subir\s+(?:para|pra|ate|at[eé])|(?:pode\s+)?ir\s+(?:para|pra))\b/;

const HARD_BUDGET_CONSTRAINT_PATTERN =
  /\b(n[aã]o\s+pode\s+passar|n[aã]o\s+quero\s+passar|n[aã]o\s+pode\s+ultrapassar|n[aã]o\s+pode\s+passar\s+de|limite\s+r[ií]gid[oa]|teto\s+firm[eao]|continua\s+sendo\s+meu\s+limite|nao\s+pode\s+passar\s+dos)\b/;
...
export function extractPriorCommercialConstraints(sessionContext = {}) {
  ...
  const inferredBudget = (() => {
    ...
    const queryCandidates = [
      ...
      ...(Array.isArray(sessionContext.messages)
        ? sessionContext.messages.slice(-4).map((entry) => entry?.content)
        : []),
    ];
    for (const text of queryCandidates) {
      const budget = extractBudget(String(text || ""));
...
function extractHardCapBudget(message = "", q = "") {
  ...
  const match = q.match(
    /\b(?:de|dos|em|ate|at[eé]|por|(?:é|e)|agora\s+(?:é|e))\s*(?:r\$\s*)?(\d+(?:[.,]\d+)*)\s*(mil|k)?\b/i
  );
...
function tryExtractBudgetCapOrCorrectionRefinement(message = "", q = "") {
  ...
  if (/\b(meu limite|limite e)\b/.test(q)) {
    const numeric = extractBudget(message) || extractHardCapBudget(message, q);
    if (numeric != null && numeric > 0) {
      return {
        detected: true,
        refinementType: REFINEMENT_TYPES.BUDGET_REFINEMENT,
...
function tryExtractBudgetRefinement(message = "", q = "") {
  ...
  const increaseMatch =
    ...
    q.match(
      /\b(?:para|pra|ate|at[eé]|ir\s+pra)\s*(?:r\$\s*)?(\d{1,3}(?:\.\d{3})+|\d+(?:,\d+)?)\s*(mil|k)?(?=\s|$|[.!?,])/i
    );
  const hasBudgetCue =
    !!explicitMatch ||
    !!increaseMatch ||
  ...
  if ((numeric == null || !Number.isFinite(numeric)) && increaseMatch?.[1]) {
    ...
    if (
      ...
      (increaseMatch[2] || /\bmil\b/.test(q.slice(increaseMatch.index || 0, (increaseMatch.index || 0) + 24))) &&
      numeric < 1000
    ) {
      numeric *= 1000;
    }
...
const __multiRefinementExtractor = createCommercialRefinementMultiExtractor({
  REFINEMENT_TYPES,
...
export const extractCommercialRefinements = __multiRefinementExtractor.extractCommercialRefinements;
export const extractCommercialRefinement = __multiRefinementExtractor.extractCommercialRefinement;
...
export function mergePriorConstraintsWithRefinement(
  ...
  switch (refinement.refinementType) {
    ...
    case REFINEMENT_TYPES.POSITIVE_BRAND_REFINEMENT: {
      ...
      if (
        refinement.brandRestriction ||
        refinement.operation === REFINEMENT_OPERATIONS.REPLACE
      ) {
        merged.preferredBrands = uniqueList(brands);
        reasonCodes.push("brand_restriction_applied");
        break;
      }
    ...
    case REFINEMENT_TYPES.RELAX_CONSTRAINT:
      if (refinement.tighten && refinement.target === "budgetMax" && merged.budgetMax != null) {
        const previousBudget = merged.budgetMax;
        merged.budgetMax = Math.round(Number(merged.budgetMax) * 0.85);
        replacedConstraints.push({
          field: "budgetMax",
          previous: previousBudget,
          next: merged.budgetMax,
        });
        reasonCodes.push("budget_tightened_soft_cap");
      } else if (refinement.target === "budgetMax" && merged.budgetMax != null) {
        const previousBudget = merged.budgetMax;
        ...
        const anchor =
          merged.budgetMax ?? extractBudget(refinement.sourceMessage || "") ?? null;
        if (anchor != null) {
          const previousBudget = merged.budgetMax ?? anchor;
          merged.budgetMax = Math.round(Number(previousBudget) * 1.15);
          replacedConstraints.push({
            field: "budgetMax",
            previous: previousBudget,
            next: merged.budgetMax,
...
export function resolveCommercialConstraintRefinement({
  ...
  const refinements =
    refinement.refinementType === REFINEMENT_TYPES.MULTI_REFINEMENT
      ? refinement.refinements || refinement.multiRefinements || []
      : refinement.detected
        ? [refinement]
        : [];
  ...
  if (!refinement.detected || !refinements.length) {
    return {
      version: CONSTRAINT_REFINEMENT_VERSION,
      detected: false,
      requiresClarification: false,
  ...
  let mergeResult = {
    mergedConstraints: { ...priorConstraints },
    removedConstraints: [],
    replacedConstraints: [],
    conflicts: [],
    reasonCodes: [],
  ...
  for (const step of refinements) {
    if (step.topicSwitchCategory) continue;
    mergeResult = mergePriorConstraintsWithRefinement(
      mergeResult.mergedConstraints,
      step,
      { baselineProduct }
    );
    lastRefinement = step;
    if (mergeResult.requiresClarification || mergeResult.conflicts?.length) break;
  }
  ...
  return {
    ...
    reasonCode:
      refinements.length > 1 ? "multi_refinement_applied" : refresh.reasonCode,
  };
}
