...
export const CONSTRAINT_REFINEMENT_VERSION = "11B.3";
...
function refinementAckPhrase(refinement = {}) {
  switch (refinement.refinementType) {
    case REFINEMENT_TYPES.NEGATIVE_BRAND_REFINEMENT:
      return `Certo — excluindo ${refinement.value},`;
    case REFINEMENT_TYPES.POSITIVE_BRAND_REFINEMENT:
      return `Certo — priorizando ${refinement.value},`;
    case REFINEMENT_TYPES.PRICE_REFINEMENT:
      return "Certo — buscando algo mais em conta,";
    case REFINEMENT_TYPES.BUDGET_REFINEMENT:
      return `Certo — com teto de ${formatPriceDisplay(refinement.value)},`;
    case REFINEMENT_TYPES.ATTRIBUTE_REFINEMENT:
      return "Certo — com essa prioridade,";
    case REFINEMENT_TYPES.SPECIFICATION_REFINEMENT:
      return `Certo — considerando ${refinement.value},`;
    case REFINEMENT_TYPES.SIZE_REFINEMENT:
      return "Certo — ajustando o tamanho,";
    case REFINEMENT_TYPES.USE_CASE_REFINEMENT:
      return "Certo — para esse uso,";
    default:
      return "Certo —";
  }
}
...
export function buildConstraintRefinementClarificationReply(refinementResult = {}) {
  ...
  if (refinement.refinementType === REFINEMENT_TYPES.PRICE_REFINEMENT) {
    return "Mais barato que qual produto ou faixa de preço você tinha em mente?";
  }
  if (refinement.refinementType === REFINEMENT_TYPES.ATTRIBUTE_REFINEMENT) {
    return "Você quer priorizar isso em alguma recomendação que já vimos, ou em uma busca nova?";
  }
  if (refinement.refinementType === REFINEMENT_TYPES.NEGATIVE_BRAND_REFINEMENT) {
    return "Sem essa marca em qual recomendação ou faixa de preço você está pensando?";
  }
  if (refinement.refinementType === REFINEMENT_TYPES.SPECIFICATION_REFINEMENT) {
    return "Essa especificação vale para qual produto ou busca que estávamos vendo?";
  }
  return "Pode me dizer a qual recomendação anterior isso se refere?";
}
...
export function buildConstraintRefinementDeterministicReply(refinementResult = {}) {
  ...
  let reply = `${ack} eu colocaria o ${name} como a melhor opção${budgetClause}.`;
  if (priceDisplay && refinement.refinementType === REFINEMENT_TYPES.PRICE_REFINEMENT) {
    reply = `${ack} a melhor opção${budgetClause} passa a ser o ${name}, por cerca de ${priceDisplay}.`;
  }
