...
const PRIORITY_ADD_PATTERN =
  /\b(?:(?:agora\s+)?(?:priorizar|prioriza|prioridade(?:\s+(?:em|para))?|quero\s+priorizar)\s+(bateria|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)|(?:agora\s+)?(bateria|c[aâ]mera|camera|desempenho|tela|preco|pre[cç]o|jogos)\s+(?:é|e)\s+mais\s+importante)\b/;
