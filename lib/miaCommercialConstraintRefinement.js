...
export const CONSTRAINT_REFINEMENT_VERSION = "3.4b.0";
...
const USE_CASE_PATTERN = new RegExp(
  `\\b(?:na verdade\\s+)?(?:[eé] mais para|quero usar para|vou usar para|preciso para|quero para)\\s+(${USE_CASE_TERMS})\\b|\\b(para (?:minha )?m[aã]e|para jogos|para trabalho|para fotograf\\w*|para faculdade|para estudar|para estudo)\\b`,
  "i"
);
...
export function extractCommercialRefinement(message = "", sessionContext = {}) {
  ...
  if (brandAlsoMatch && isBrandLikeToken(brandAlsoMatch[1])) {
    return {
      ...
      value: normalizeBrandToken(brandAlsoMatch[1]),
      confidence: 0.91,
      requiresClarification: false,
      sourceMessage: message,
    };
  ...
  if (useCaseMatch) {
    const useCase = normalizeText(useCaseMatch[1] || useCaseMatch[2] || "");
