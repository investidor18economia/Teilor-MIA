...
export function isAnchoredExplicitNewOptionsSearchRequest(message = "") {
  const text = String(message || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
  ...
  const hasSearchVerb = /\b(buscar|busca|procurar|procura|pesquisar|pesquisa)\b/.test(text);
  const asksForNewOptions =
    /\b(opcoes|opcao|alternativas|alternativa|novas|nova)\b/.test(text) ||
    /\b(outras?\s+opcoes|outras?\s+alternativas)\b/.test(text);

  return hasSearchVerb && asksForNewOptions;
}
...
export function resolveClearNewCommercialSearchForRouting({
  ...
  if (hasAnchor) {
    ...
    if (isAnchoredExplicitNewOptionsSearchRequest(query)) {
      return true;
    }
