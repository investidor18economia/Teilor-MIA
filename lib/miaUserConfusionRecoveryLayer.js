...
export function buildUserConfusionRecoveryReply({
  ...
  if (explanationCtx?.lastConsequence) {
    const clause = formatChoiceReasonClause(anchorName, explanationCtx.lastConsequence);
    if (clause) {
      parts.push(`Em uma frase: ${clause.charAt(0).toLowerCase()}${clause.slice(1)}`);
    }
