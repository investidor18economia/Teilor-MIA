...
/** Forbidden substrings in rendered public page output */
 ...
 */
export function scanPublicMetricsForbiddenContent(htmlOrText = "") {
  const hits = [];
  for (const pattern of PUBLIC_METRICS_FORBIDDEN_PATTERNS) {
    if (pattern.test(htmlOrText)) hits.push(String(pattern));
  }
  return hits;
}
