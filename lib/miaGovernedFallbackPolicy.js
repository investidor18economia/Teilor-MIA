...
import { buildContractDrivenSocialFallback } from "./miaSocialContractVerbalization.js";

export const GOVERNED_FALLBACK_POLICY_VERSION = "5.7.1";
...
const FALLBACK_POOLS = Object.freeze({
  ...
  [FALLBACK_FAMILIES.AMBIGUOUS_SOCIAL]: [
    "Vou aceitar como elogio gratuito haha 😄",
    "Gostei desse elogio 😄",
    "Esse elogio foi pra mim ou pra algum produto? 👀",
    "Agora fiquei curiosa... estava falando de mim ou de alguma coisa?",
    "Recebi bem — você fala de mim ou de outra coisa?",
  ],
...
export function resolveFallbackFamilyForContract(contract = {}, targetResolution = {}) {
  ...
  if (routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.GREETING) {
    return FALLBACK_FAMILIES.SOCIAL;
  }
  if (routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.FAREWELL) {
    return FALLBACK_FAMILIES.SOCIAL;
  }
  if (routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.ACKNOWLEDGEMENT) {
    return FALLBACK_FAMILIES.SOCIAL;
  }
  if (routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.AMBIGUOUS_SOCIAL) {
    return FALLBACK_FAMILIES.AMBIGUOUS_SOCIAL;
  }
  ...
  if (
    ...
    primary === SOCIAL_INTENT_FAMILIES.SOFT_DISAGREEMENT ||
    contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.ACKNOWLEDGE_DISAPPROVAL
  ) {
    return FALLBACK_FAMILIES.DISAPPROVAL;
  }
  if (
    primary === SOCIAL_INTENT_FAMILIES.CORRECTION ||
    contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.REPAIR_CONTEXT
  ) {
  ...
  if (isMiaComplimentGovernedContract(contract, targetResolution)) {
    return primary === SOCIAL_INTENT_FAMILIES.PRAISE
      ? FALLBACK_FAMILIES.PRAISE
      : FALLBACK_FAMILIES.COMPLIMENT;
  }
  if (
    routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.PRODUCT_AESTHETIC_OPINION ||
    (target === SEMANTIC_TARGETS.PRODUCT && isProductAestheticFallbackPermitted(contract, targetResolution))
  ) {
    return FALLBACK_FAMILIES.PRODUCT_AESTHETIC;
  }
  if (target === SEMANTIC_TARGETS.UNKNOWN) {
    if (
      isGovernedAmbiguousSocialContract(contract, targetResolution) ||
      contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.INVITE_CLARIFICATION ||
      contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.REPAIR_CONTEXT
    ) {
      return FALLBACK_FAMILIES.AMBIGUOUS_SOCIAL;
    }
 ...
 */
function applyContractDrivenFallback(contract, family, options, failureReason) {
  const driven = buildContractDrivenSocialFallback(contract, family, options);
  if (driven?.text) {
    return {
      text: driven.text,
      family,
      functionName: driven.builder,
      reasonCodes: driven.reasonCodes || ["contract_driven", failureReason],
      contractDriven: true,
    };
  }
  return null;
...
export function selectGovernedFallback(contract = {}, options = {}) {
  ...
  if (contract.farewellMode || contract.socialDepartureMode || contract.socialFamilies?.farewell) {
    const driven = applyContractDrivenFallback(contract, FALLBACK_FAMILIES.SOCIAL, options, failureReason);
  ...
  if (
    contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.ACKNOWLEDGE_DISAPPROVAL ||
    contract.primarySocialIntent === SOCIAL_INTENT_FAMILIES.DISAPPROVAL ||
    contract.primarySocialIntent === SOCIAL_INTENT_FAMILIES.SOFT_DISAGREEMENT
  ) {
    const driven = applyContractDrivenFallback(
      contract,
      FALLBACK_FAMILIES.DISAPPROVAL,
      options,
      failureReason
    );
    if (driven) return driven;
  ...
  if (
    contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.REPAIR_CONTEXT ||
    contract.primarySocialIntent === SOCIAL_INTENT_FAMILIES.CORRECTION
  ) {
    const driven = applyContractDrivenFallback(
      contract,
      FALLBACK_FAMILIES.CONVERSATION,
  ...
  if (contract.shortReactionMode || contract.socialFamilies?.reaction) {
    const driven = applyContractDrivenFallback(contract, FALLBACK_FAMILIES.HUMOR, options, failureReason);
    if (driven) return driven;
    return {
  ...
  if (isGovernedAmbiguousSocialContract(contract, targetResolution)) {
    ...
    if (driven) return driven;
    return {
      text: pickVariant(FALLBACK_POOLS[FALLBACK_FAMILIES.AMBIGUOUS_SOCIAL], seed),
      family: FALLBACK_FAMILIES.AMBIGUOUS_SOCIAL,
      functionName: "selectGovernedFallback",
      reasonCodes: ["ambiguous_social_contract", failureReason],
    };
  }

  const family = resolveFallbackFamilyForContract(contract, targetResolution);

  if (family === FALLBACK_FAMILIES.PRODUCT_AESTHETIC && isProductAestheticFallbackPermitted(contract, targetResolution)) {
    return {
  ...
  if (isMiaComplimentGovernedContract(contract, targetResolution) && family === FALLBACK_FAMILIES.PRODUCT_AESTHETIC) {
    const complimentFamily =
      contract.primarySocialIntent === SOCIAL_INTENT_FAMILIES.PRAISE
        ? FALLBACK_FAMILIES.PRAISE
        : FALLBACK_FAMILIES.COMPLIMENT;
    return {
      text: pickVariant(FALLBACK_POOLS[complimentFamily], seed),
      family: complimentFamily,
      functionName: "selectGovernedFallback",
      reasonCodes: ["mia_compliment_blocks_product_aesthetic", failureReason],
    };
  }
  ...
  const drivenFinal = applyContractDrivenFallback(contract, family, options, failureReason);
  if (drivenFinal) return drivenFinal;
  return {
