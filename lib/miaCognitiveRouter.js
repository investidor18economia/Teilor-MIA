...
function detectsAcknowledgementSignal(q) {
  ...
  // PATCH 7.9X-H — comprehension success preserves acknowledgement response path
...
const ACK_POSITIVE_PREFIX_PATTERN =
  "ok|okay|blz|beleza|certo|show|ta|ta bom|otimo|perfeito|top|massa|fechado|fechou|combinado";
...
function detectsNaturalAcknowledgementSignal(q) {
  ...
  const shortAccept =
    /^(ok|okay|certo|beleza|blz|ta|ta bom|ta certo|tudo certo|pode ser|ah sim|justo|claro|captei|verdade)$/.test(q) ||
    /^ok entendi$/.test(q) ||
    /^ah entendi$/.test(q) ||
    /^blz entendi$/.test(q) ||
  ...
  const informalBr =
    ...
    /^beleza entao$/.test(q) ||
    /^beleza entendi$/.test(q) ||
    /^(to ligado|ta ligado|entendi|saquei)$/.test(q);
  ...
  return shortAccept || continuity || lightConfirm || informalBr || shortPositiveReaction;
}
...
export function hasAcknowledgementOpeningPrefix(message = "") {
  const q = normalize(message);
  if (!q) return false;
  return /^(ok|okay|blz|beleza|certo|show|top|fechou|fechado|ta|ta bom|suave|tranquilo|de boa|demorou|valeu|massa|perfeito|otimo|combinado)\b/.test(q);
}
...
export function isAcknowledgementFamilyQuery(message = "") {
  const q = normalize(message);
  if (detectsPureAcknowledgementSignal(q)) return true;
  if (detectsNaturalPositiveComprehensionSignal(q) && q.split(/\s+/).length <= 4) return true;
  return false;
}
...
function hasComprehensionCommercialTail(q) {
  ...
  // "que celular..." é busca — distinto de "que?" isolado.
  if (/^que\s+\S/.test(q)) return true;
...
function detectsNaturalDecisionConfirmationSignal(q) {
  ...
  return clusterA || clusterB || clusterC || clusterD || clusterE || convergenceReveal;
}
...
function detectsNaturalAntiRegretSignal(q) {
  ...
  const explicitRegretFear =
    ...
    (/\bmedo de errar\b/.test(q) && /\b(compra|escolha|decisao|nessa|nesse)\b/.test(q)) ||
...
function detectsConfidenceChallengeSignal(q) {
  ...
  if (/^(voce|vc) (sustenta|mantem|mantém|banca) (isso|essa escolha|essa recomendacao)$/.test(q)) {
    return true;
  }
...
function detectsNaturalConfidenceChallengeCore(q) {
  ...
  const sustainRecommendation =
    ...
    /\bbate(o|ria)? o martelo\b/.test(q) ||
    /\b(voce|vc)\s+(crava|garante)\b/.test(q);

  const firmnessChallenge =
    /\b(nao esta|nao ta|esta)\s+(forcando|forçando|exagerando|empurrando|puxando)\b/.test(q) ||
    /\bpegadinha\b/.test(q) ||
    (/\b(bem )?segur[oa]\b/.test(q) && /\b(recomendacao|escolha|decisao|isso)\b/.test(q));

  const personalTrustTest =
    (/\b(voce|vc)\s+(compraria|iria|optaria|pegaria|seguiria)\b/.test(q) && !/\bse fosse\b/.test(q)) ||
    (/\b(voce|vc)\s+escolheria\b/.test(q) && !/\bqual\b/.test(q)) ||
...
function detectsSocialValidationSignal(q) {
  ...
  // DELEGATION — "você compraria?" ≠ "o pessoal compra?"
  if (/\b(voce|vc)\s+(compraria|manteria|trocaria|escolheria|recomendaria)\b/.test(q)) return false;
  if (/\bo que (voce|vc) (faria|compraria|acharia)\b/.test(q)) return false;
...
function detectsNaturalSecondBestDiscoverySignal(q) {
  ...
  const runnerUpReveal =
    /\b(plano b|segunda escolha|carta na manga|substituto natural|substituto direto|reserva imediata|backup|reserva)\b/.test(q) &&
    /\b(qual|quem|seria|fica|sobra|imediato)\b/.test(q) &&
    !/\b(explorar|possibilidades|abrir opcoes|comparar|compara|versus|\bvs\b)\b/.test(q);
...
function detectsNaturalAlternativeExplorationSignal(q) {
  ...
  const hypotheticalWithoutCurrent =
...
function detectsAlternativeExplorationSignal(q) {
  ...
  if (/\be se eu nao pegar (esse|essa|ele|ela)\b/.test(q)) return true;
...
function hasWeakConversationalPrefix(prefix = "") {
  ...
  return (
    ...
    /^(entendi|saquei|faz sentido|ate que|captei|entendo)\b/.test(prefix) ||
    hasWeakDecisionConfirmationPrefix(prefix) ||
...
export function getDominantMasTailIntent(message = "") {
  ...
  if (!split?.tail || split.tail.length < 4 || !hasWeakConversationalPrefix(split.prefix)) {
    return null;
  }

  const compound = detectsConstraintAlternativeCompoundDominant(q);
  if (compound) return compound;
  if (hasAntiRegretDominantMasTail(q)) return "ANTI_REGRET";
  if (hasConfidenceChallengeDominantMasTail(q)) return "CONFIDENCE_CHALLENGE";
  if (detectsNaturalConfidenceChallengeCore(split.tail)) return "CONFIDENCE_CHALLENGE";
  if (hasSocialValidationDominantMasTail(q)) return "SOCIAL_VALIDATION";
  if (detectsNaturalSocialValidationCore(split.tail) && !hasPersonalAntiRegretDominantFrame(split.tail)) {
    return "SOCIAL_VALIDATION";
  }
  if (hasSoftDisagreementDominantMasTail(q)) return "SOFT_DISAGREEMENT";
  if (hasConstraintChangeDominantMasTail(q)) return "CONSTRAINT_CHANGE";
  if (detectsSecondBestDiscoveryCore(split.tail) || detectsSecondBestDiscoveryCore(q)) {
    return "SECOND_BEST_DISCOVERY";
  }
  if (detectsAlternativeExplorationCore(split.tail) || detectsAlternativeExplorationCore(q)) {
    return "ALTERNATIVE_EXPLORATION";
  }
...
function buildTurnSignals({
  ...
  return applyCrossFamilyCollisionPrecedence(
    ...
    isComprehension: detectsComprehensionFailureSignal(q),
    isSoftDisagreement: softDisagreement,
    isDecisionConfirmation: decisionConfirmation,
    isAntiRegret: antiRegret,
    isConfidenceChallenge: confidenceChallenge,
...
function resolveTurnTypeFromSignals(signals, q, { hasActiveAnchor, contextResolution, detectedIntent }) {
  ...
  // 2.6 ACKNOWLEDGEMENT — precede NEW_SEARCH (PATCH 7.7E)
  ...
  // 2.7 COMPREHENSION — precede NEW_SEARCH (PATCH 7.7K)
  // Falha de compreensão / pedido de esclarecimento não vira busca cold-start.
  if (signals.isComprehension) {
    if (hasActiveAnchor) {
      turnType = MIA_TURN_TYPES.EXPLANATION_REQUEST;
      confidence = 0.84;
      reasons.push("comprehension_intent_detected", "anchored_reexplanation_request");
    } else {
      turnType = MIA_TURN_TYPES.CONVERSATIONAL;
      confidence = 0.84;
      reasons.push("comprehension_intent_detected", "cold_clarification_request");
    }
    return { turnType, confidence, reasons };
  }
