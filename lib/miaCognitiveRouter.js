 ...
 *
 * Princípios (docs/core/rules/MIA_ENGINEERING_RULES.md):
...
function detectsValueQuestionSignal(q, { hasActiveAnchor }) {
  ...
  if (isSpecificProductEvaluationQuery(q)) return false;
...
function detectsCommercialQuestionSignal(q, { hasActiveAnchor, comparisonContext, rawQuery = "" }) {
  if (isSpecificProductEvaluationQuery(rawQuery || q)) return true;
...
function hasAboutMiaRoutingBlock(q, { hasActiveAnchor } = {}) {
  ...
  // PATCH 12.4 — anchored product differential/explanation beats institutional ABOUT_MIA.
  if (
    hasActiveAnchor &&
    (
      (/\b(qual|quais)\b/.test(q) &&
        /\b(diferencial|vantagem principal|ponto forte|ponto positivo|destaque)\b/.test(q)) ||
      (/\bo que\b/.test(q) && /\b(faz melhor|tem de melhor|tem de especial|se destaca)\b/.test(q)) ||
      /\bpor que esse\b/.test(q) ||
      /\b(explica|explique)\b/.test(q)
    )
  ) {
    return true;
  }
...
function hasComprehensionCommercialTail(q) {
  ...
  if (/\b(esse|essa|este|esta)\s+\w+/.test(q)) {
    const anchoredExplanationCue =
      /\b(explica|explique|por que|pq|motivo|logica|raciocinio|decisao|recomendacao|escolha|aparelho|produto)\b/.test(q) ||
      /\bpor que esse\b/.test(q);
    if (!anchoredExplanationCue) return true;
  }
...
function detectsNaturalSoftDisagreementCore(q) {
  ...
  const commercialProductPreference =
    detectsCommercialProductPreferenceChallengeCore(q);
...
/** PATCH 4A.2VF — contestação comercial com produto + "melhor"/preferência (≠ bem-estar pessoal). */
function isPersonalWellbeingImprovementQuery(q = "") {
  if (!q) return false;
  return (
    /\b(estou|to|me sinto|hoje eu|minha dor|dormi|sentindo)\b.{0,30}\bmelhor\b/.test(q) ||
    /\bmelhorou\b/.test(q) ||
    /\bagora estou melhor\b/.test(q) ||
    /\bhoje estou melhor\b/.test(q) ||
    (/\bficou melhor\b/.test(q) &&
      !/\b(achei|prefiro|escolheria|galaxy|iphone|s23|a55|moto|redmi|pixel|fe)\b/.test(q))
  );
}

function hasCommercialProductEntitySignal(q = "") {
  if (!q) return false;
  const compact = q.replace(/\s+/g, "");
  return (
    /\b(galaxy|iphone|samsung|motorola|moto|redmi|pixel|xiaomi|poco|note|rog|zenfone)\b/.test(q) ||
    /\b(s\d{1,2}|a\d{2,3}|g\d{2,3})\b/.test(q) ||
    /\bs\d{1,2}\s*fe\b/.test(q) ||
    /\bs23fe\b/.test(q) ||
    /\bs\d{1,2}fe\b/.test(compact) ||
    /(?:^|[^a-z0-9]|o)s\d{1,2}fe\b/.test(compact) ||
    /\ba\d{2,3}5g\b/.test(compact) ||
    (/\bfe\b/.test(q) && /\b(s23|galaxy|samsung)\b/.test(q))
  );
}

function detectsCommercialProductPreferenceChallengeCore(q = "") {
  if (!q || isPersonalWellbeingImprovementQuery(q)) return false;

  const comparativeProductPreference =
    (/\b(achei|acho|prefiro|escolheria|considero|vejo|penso|continu[oa]\s+achando)\b/.test(q) &&
      /\b(melhor|superior|mais interessante|mais forte|ganha|venceria)\b/.test(q)) ||
    (/\b(achei|acho|prefiro|escolheria|considero|vejo|penso|continu[oa]\s+achando)\b/.test(q) &&
      hasCommercialProductEntitySignal(q)) ||
    (/\b(pra mim|para mim|p\/ mim)\b/.test(q) &&
      (/\b(melhor|superior|mais interessante|ganha)\b/.test(q) || hasCommercialProductEntitySignal(q))) ||
    (/\b(nao concordo|discordo|nao vejo.*melhor|subestim)\b/.test(q) && hasCommercialProductEntitySignal(q)) ||
    (/\b(mas|porem|porém)\b/.test(q) &&
      (/\b(melhor|superior|prefiro|ganha)\b/.test(q) || hasCommercialProductEntitySignal(q))) ||
    (/\b(nao seria|seria)\b/.test(q) && /\b(melhor|boa escolha)\b/.test(q)) ||
    (/\b(tem certeza|voce tem certeza|vc tem certeza)\b/.test(q) &&
      hasCommercialProductEntitySignal(q) &&
      /\b(melhor|superior|parece melhor)\b/.test(q)) ||
    (/\bparece\b/.test(q) && /\bmelhor\b/.test(q) && hasCommercialProductEntitySignal(q));

  return hasCommercialProductEntitySignal(q) && comparativeProductPreference;
}

/** PATCH 4A.2VF — exported for intent/routing precedence. */
export function isCommercialProductPreferenceChallengeQuery(message = "") {
  const q = normalize(message);
  if (!q) return false;
  const dominant = getCrossFamilyDominantFamily(message);
  if (
    dominant &&
    dominant !== "SOFT_DISAGREEMENT" &&
    dominant !== "CONFIDENCE_CHALLENGE" &&
    dominant !== "OBJECTION"
  ) {
    return false;
  }
  return detectsCommercialProductPreferenceChallengeCore(q);
}
...
function detectsNaturalConstraintChangeSignal(q) {
  ...
  const priorityShift =
    ...
    /\b(pra mim|para mim|p mim)\b.*\b(o mais importante|mais importante|importa mais)\b/.test(q) &&
      _CC_ATTR.test(q) ||
    /\b(o que mais importa|mais importa)\b.*\b(pra mim|para mim|p mim)\b/.test(q) &&
      _CC_ATTR.test(q) ||
    /\bpriorizo\b/.test(q) && _CC_ATTR.test(q) ||
    /\b(dar mais peso|mais peso)\b/.test(q) && _CC_ATTR.test(q) ||
    /\bprefiro\b/.test(q) && /\b(autonomia|bateria|durabilidade|longevidade)\b/.test(q) &&
      !/\b(galaxy|iphone|samsung|motorola|moto|redmi|pixel|s\d|a\d|g\d)\b/.test(q) ||
    /\b(vem primeiro|primeiro)\b/.test(q) && _CC_ATTR.test(q) ||
    /\b(nao quero viver|procurando tomada)\b/.test(q) ||
    /\b(preciso que dure|dure o dia)\b/.test(q) ||
    /\b(acima de|antes de)\b/.test(q) && _CC_ATTR.test(q) ||
    /\b(pra mim|para mim|p mim)\b.*\b(primeiro|primeira)\b/.test(q) && _CC_ATTR.test(q) ||
    /\bmais importante\b.*\b(p mim|pra mim|para mim)\b/.test(q) && _CC_ATTR.test(q) ||
    (/\bbat\b/.test(q) && /\bmais importante\b/.test(q) && /\b(p mim|pra mim|para mim)\b/.test(q)) ||
...
function resolveTurnTypeFromSignals(signals, q, { hasActiveAnchor, contextResolution, detectedIntent }) {
  ...
  if (signals.isAcknowledgement) {
    if (hasActiveAnchor) {
      ...
      turnType = MIA_TURN_TYPES.CONVERSATIONAL;
      confidence = 0.84;
      reasons.push("acknowledgement_intent_detected", "cold_acknowledgement");
    }
    return { turnType, confidence, reasons };
  }
  ...
  if (hasActiveAnchor) {
    ...
    // 5.5 SPECIFIC PRODUCT EVALUATION — precede VALUE_QUESTION (PATCH 4A.2VF)
    if (isSpecificProductEvaluationQuery(q)) {
      turnType = MIA_TURN_TYPES.COMMERCIAL_QUESTION;
      confidence = 0.86;
      reasons.push("specific_product_evaluation_detected", "anchor_active");
      return { turnType, confidence, reasons };
    }
