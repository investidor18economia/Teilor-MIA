...
export const FILLER_GOVERNANCE_VERSION = "5.8.1";
...
function hasInterjectionMorphology(core = "") {
  ...
  if (/^(ok|okay|ta|ne|pois|blz|beleza|entendi|saquei|captei|show|massa|top|legal|verdade|claro|certo|sim|aham|uhum|pera|perai)$/.test(q)) {
    return true;
  }
  return false;
}
...
export function classifyConversationalFiller(
  ...
  const commercialThreadActive =
    hasActiveAnchor ||
    hasActiveCommercialThread(enrichedSession) ||
    hasRunningCommercialDiscourse(conversationMessages);
  ...
  if (commercialThreadActive && isMinimalNonCommercialFragment(core)) {
    const longConversation = (conversationMessages?.length || 0) >= 14;
    return {
      version: FILLER_GOVERNANCE_VERSION,
      ...
      blocksClarification: true,
      reasonCode: longConversation
        ? "long_conversation_anchor_restored"
        : "neutral_filler_preserves_active_commercial_thread",
      interactionMode: MIA_INTERACTION_MODES.SOCIAL,
    };
  }
