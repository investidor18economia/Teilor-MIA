 ...
 */
async function fetchTemporalSeriesGroup(client, rpcName, windowDays, offsetDays) {
...
function shapeTemporalGroupResponse(groupData, granularity, group) {
  ...
  if (group === "products" || group === "categories") {
...
export async function buildTemporalSeriesResponse(options = {}) {
  ...
  const cacheKey = `temporal-metrics:v${MIA_TEMPORAL_SERIES_VERSION}:d${windowDays}:o${offsetDays}:g${granularity}:s${requestedGroups.join(",")}`;
  ...
  const results = await Promise.all(
    requestedGroups.map(async (group) => {
      ...
      return { group, ...(await fetchTemporalSeriesGroup(supabase, rpcName, windowDays, offsetDays)) };
    })
  );
