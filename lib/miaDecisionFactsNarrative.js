...
import {
  buildVerbalizationPlanFromNarrativePlan,
...
export function collectDecisionFactsFromSession(sessionContext = {}, options = {}) {
  ...
  if (sessionContext.lastStructuredDecisionFacts?.semanticUnits?.length) {
    const adapted = applyLegacyDecisionFactsAdapter(
      base,
      sessionContext.lastStructuredDecisionFacts
    );
    const narrativePlan =
      sessionContext.lastNarrativePlan ||
      buildNarrativePlanFromStructuredFacts(sessionContext.lastStructuredDecisionFacts, {
        hasWinner: !!winner?.product_name,
      });
    const verbalizationPlan =
      sessionContext.lastVerbalizationPlan ||
      buildVerbalizationPlanFromNarrativePlan(narrativePlan, {
        query: sessionContext.lastQuery || "",
        winnerName: winner?.product_name || "",
        isFollowUp: !!sessionContext.lastBestProduct,
      });
    if (hasVerbalizationPlan(verbalizationPlan)) {
      const ordered = verbalizationPlanToOrderedLegacyStrings(verbalizationPlan);
      adapted.advantages = ordered.gains.length ? ordered.gains : adapted.advantages;
      adapted.sacrifices = ordered.sacrifices.length ? ordered.sacrifices : adapted.sacrifices;
      adapted.narrativePlan = narrativePlan;
      adapted.verbalizationPlan = verbalizationPlan;
      const styleGovernance =
        sessionContext.lastVerbalizationStyleGovernance ||
        buildVerbalizationStyleGovernancePayload(verbalizationPlan, {
          query: sessionContext.lastQuery || "",
          sessionContext,
        });
      if (hasVerbalizationStyleGovernance(styleGovernance)) {
        adapted.verbalizationStyleGovernance = styleGovernance;
      }
      return adapted;
    }
...
export function decisionFactsNarrativeToTrace(facts = null) {
  ...
  return {
    ...
    narrativePlan: narrativePlanToTrace(facts.narrativePlan),
    verbalizationPlan: verbalizationPlanToTrace(facts.verbalizationPlan),
  };
}
