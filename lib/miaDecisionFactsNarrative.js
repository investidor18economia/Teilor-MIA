...
export const DECISION_FACTS_NARRATIVE_VERSION = "3.5a.0";

const SHALLOW_REPLY_EXACT =
  /^(faz sentido pelo que voc[eê] trouxe|esse ponto pesa na decis[aã]o|entendo o contexto|entendi|certo|ok|boa|legal|show)\.?$/i;
...
export function isShallowCommercialReply(text = "") {
  ...
  if (!t) return true;
  if (SHALLOW_REPLY_EXACT.test(t)) return true;
  if (t.length <= 28 && /^(faz sentido|esse ponto pesa|entendi|certo)\b/i.test(t)) return true;
  return false;
}
 ...
 */
export function collectDecisionFactsFromSession(sessionContext = {}) {
...
export function collectRefinementDecisionFacts(refinementResult = {}, sessionContext = {}) {
  ...
  return {
    ...
    changeSummary: describeRefinementTypeChange(refinement, prior, merged),
 ...
 */
export function buildCommercialRefinementNarrative(facts = {}) {
  const name = shortProductName(facts.newWinnerName);
  if (!name) return "";

  const transition = "Agora mudou um detalhe importante na busca.";
  const change = facts.changeSummary
    ? `Como ${facts.changeSummary}, eu reavaliei as opções que já tínhamos em contexto.`
    : "Com esse refinamento, eu reavaliei as opções que já tínhamos em contexto.";

  let decision;
  if (facts.winnerChanged) {
    decision = `Eu mudaria a recomendação para o ${name} — ${whyOutcome(facts)}.`;
  } else {
    decision = `Mantenho o ${name} na frente — ${whyOutcome(facts)}.`;
  }

  const continuity = whatStaysSame(facts);
  const tradeoff =
    facts.tradeoff && !decision.includes(facts.tradeoff.slice(0, 24))
      ? facts.tradeoff
      : facts.sacrifices?.length
        ? `Tradeoff: ${facts.sacrifices[0]}.`
        : "";
...
export function buildCommercialTransitionAck({ message = "", decisionFacts = null, depth = "brief" } = {}) {
  ...
  if (facts.refinementType || facts.changeSummary) {
    ...
    return minimal ? `Como ${lead},` : `Como ${lead}, eu reavaliei a recomendação anterior.`;
  }

  if (facts.hasCommercialContext && facts.primaryAxisLabel) {
    return minimal
      ? `Pensando em ${facts.primaryAxisLabel},`
      : `Pensando no foco em ${facts.primaryAxisLabel}, isso muda como eu comparo as opções.`;
  }

  if (facts.hasCommercialContext && facts.winner?.product_name) {
    return minimal
      ? "Com o que você trouxe,"
      : "Com o que você trouxe agora, eu ajusto a leitura da recomendação anterior.";
  }

  if (msg.length >= 8 && facts.hasCommercialContext) {
    return minimal ? "Com esse contexto," : "Com esse contexto, eu ajusto a leitura da decisão.";
  }
...
export function selectCommercialAwareAck(input = {}) {
  ...
  return minimal ? "Certo —" : "Certo — vou te mostrar a consequência disso na recomendação.";
}
