...
export const SOCIAL_PERCEPTION_VERSION = "11C";
...
const TOPIC_ANCHOR_GROUPS = Object.freeze([
  ...
  {
    key: "melhora",
    pattern: /\b(melhorou|melhorando|melhoria|tranquil\w*|aliviad\w*|alivio|alívio)\w*/i,
  },
  { key: "agradecimento", pattern: /\b(valeu|obrigad\w*|tmj|brigad\w*)\w*/i },
  { key: "estetica", pattern: /\b(bonit\w*|lind\w*|elegante|premium|discret\w*|design|visual|estetic\w*)\w*/i },
  { key: "pesquisa_cansativa", pattern: /\b(cansad\w*.{0,24}pesquis\w*|pesquisar demais|comparar tant\w* op[cç][õo]es)\w*/i },
  { key: "celular_velho", pattern: /\b(celular.{0,20}velho|smartphone.{0,20}velho|aparelho.{0,20}velho|trocar.{0,20}celular)\w*/i },
  { key: "marca_negativa", pattern: /\b(n[aã]o gosto (?:de )?(?:samsung|iphone|apple|motorola|xiaomi))\b/i },
]);
...
export function extractContentAnchors(message = "", options = {}) {
  const normalized = normalizeText(message);
  if (!normalized) return [];

  const resolvedTarget = options.resolvedTarget || options.contract?.resolvedSemanticTarget || null;
  const skipAestheticForMia = resolvedTarget === SEMANTIC_TARGETS.MIA;

  const anchors = new Set();
  for (const group of TOPIC_ANCHOR_GROUPS) {
    if (group.key === "estetica" && skipAestheticForMia) {
      continue;
    }
...
function readMessageContext(recognition = {}, message = "") {
  ...
  return {
    ...
    isCasualComment:
      ...
      /\b(acho|achei|gost\w*|curt\w*)\b/i.test(normalized) ||
      tokenCount(message) >= 3,
    isEntityOpinion:
      /\b(acho|achei|gost\w*|curt\w*)\b.{0,40}\b(bonit\w*|lind\w*|fei\w*|design|visual|elegante|premium)\w*/i.test(
        normalized
      ) ||
      /\b(bonit\w*|lind\w*)\b.{0,40}\b(galaxy|iphone|esse|essa|ele|ela|modelo)\w*/i.test(
        normalized
      ),
    isFarewell:
...
function resolveMustReferenceUserContent(message = "", anchors = [], recognition = {}, contract = {}) {
  if (contract.skipMustReferenceUserContent) {
    return false;
  }
  const ctx = readMessageContext(recognition, message);
  ...
  if (recognition.primarySocialIntent === "compliment" || recognition.primarySocialIntent === "praise") {
    return false;
  }
  if (ctx.isCasualComment || ctx.isEntityOpinion) {
    return true;
  }
...
export function enrichContractWithSocialPerception(
  ...
  const contentAnchors = extractContentAnchors(userMessage, {
    resolvedTarget: workingContract.resolvedSemanticTarget,
...
function extractEntityMention(message = "") {
  const normalized = normalizeText(message);
  const productTokens = normalized.match(
    /\b(galaxy|iphone|samsung|motorola|xiaomi|redmi|dell|lenovo|notebook|celular)\b/
  );
  if (productTokens?.[1]) return productTokens[1];
  const generic = normalized.match(/\b(esse|essa)\b/);
  return generic?.[1] || "";
}

function buildEntityOpinionFallback(message = "", depth = RESPONSE_DEPTH.BRIEF, contract = {}) {
  if (!isEntityOpinionFallbackAllowed(contract, contract.semanticTargetResolution)) {
    return null;
  }
  const entity = extractEntityMention(message) || contract.productReference;
  const normalized = normalizeText(message);
  if (/\b(bonit\w*|lind\w*|elegante|premium|discret\w*)\w*/i.test(normalized)) {
    if (entity && !/^(esse|essa)$/.test(entity)) {
      ...
      return `O design do ${label} realmente é mais discreto e premium. Foi o visual que mais chamou sua atenção nele?`;
    }
    if (depth === RESPONSE_DEPTH.MINIMAL) {
      return "O visual dele realmente chama atenção.";
    }
    return "Ele tem um design bem marcante. Foi o visual que mais pesou para você?";
  }
  if (/\bfei\w*/i.test(normalized)) {
...
export function validateSocialResponsePerception(reply = "", contract = {}) {
  ...
  if (contract.mustReferenceUserContent === false || contract.skipMustReferenceUserContent) {
    // governed social families allow concise replies
  } else if (contract.mustReferenceUserContent) {
    if (
  ...
  if (
    ...
    /^(entendo|imagino|que bom|faz sentido|pois e|pois é)[.!]?$/.test(normalized)
  ) {
    violations.push("generic_response_violation");
  }
...
export function buildSpecificGovernedFallback(contract = {}, { period = "" } = {}) {
  if (isCommercialFallbackBlocked(contract)) {
    return selectGovernedFallback(contract, {
      failureReason: "legacy_specific_governed_redirect",
      period,
    }).text;
  }
  ...
  if (anchors.includes("pesquisa_cansativa")) {
    return depth === RESPONSE_DEPTH.MINIMAL
      ? "Comparar opções cansa mesmo."
      : "Comparar tantas opções cansa mesmo — faz sentido querer simplificar.";
  }
  ...
  const ctx = readMessageContext(contract, message);

  if (ctx.isEntityOpinion || anchors.includes("estetica")) {
    const opinionReply = buildEntityOpinionFallback(message, depth, contract);
    if (opinionReply) return opinionReply;
  }

  if (anchors.includes("celular_velho")) {
    return depth === RESPONSE_DEPTH.MINIMAL
      ? "Quando o aparelho envelhece, tudo parece mais lento."
      : "Quando o celular já está no limite, até tarefas simples parecem mais pesadas.";
  }

  if (anchors.includes("marca_negativa")) {
    return "Preferência de marca conta bastante na hora de escolher.";
  }
  ...
  if (isCommercialFallbackBlocked(contract)) {
    return selectGovernedFallback(contract, {
      failureReason: "legacy_specific_governed_tail",
      period,
    }).text;
  }

  if (ctx.isCasualComment && tokenCount(message) >= 4) {
    return selectHumanAck({
      anchors,
      depth,
      message,
      recentOpeners: contract.repetitionSignals?.recentResponseOpeners || [],
      contract,
    });
  }
