...
export const SOCIAL_PERCEPTION_VERSION = "11B.3";
...
function buildEntityOpinionFallback(message = "", depth = RESPONSE_DEPTH.BRIEF) {
  ...
  if (/\bfei\w*/i.test(normalized)) {
    return "Entendo — estética pesa diferente para cada pessoa.";
  }
  return null;
}
...
export function buildSpecificGovernedFallback(contract = {}, { period = "" } = {}) {
  ...
  if (ctx.isCasualComment && tokenCount(message) >= 4) {
    return depth === RESPONSE_DEPTH.MINIMAL
      ? "Entendo."
      : "Faz sentido pelo contexto que você trouxe.";
  }

  return depth === RESPONSE_DEPTH.MINIMAL ? "Entendo." : "Compreendo.";
}
