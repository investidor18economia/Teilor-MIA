...
function buildEntityOpinionFallback(message = "", depth = RESPONSE_DEPTH.BRIEF) {
  const entity = extractEntityMention(message);
  const normalized = normalizeText(message);
...
export function buildSpecificGovernedFallback(contract = {}, { period = "" } = {}) {
  ...
  if (ctx.isEntityOpinion || anchors.includes("estetica")) {
    const opinionReply = buildEntityOpinionFallback(message, depth);
