...
export const CONVERSATIONAL_IDENTITY_PRESENCE_VERSION = "5.8.8";
...
function classifyIdentityQuerySupplement(message = "") {
  ...
  if (/\b(voc[eê]\s+[eé]\s+(?:humana|pessoa|gente|um\s+ser\s+humano))\b/.test(q)) {
    return IDENTITY_QUERY_KIND.REAL;
  }
  if (/\b(llm|alimenta)\b/.test(q)) return IDENTITY_QUERY_KIND.MODEL_TECH;
  ...
  if (/\bopen\s*ai\b/.test(q) && q.length <= 20) return IDENTITY_QUERY_KIND.MODEL_TECH;
  return null;
}

function resolveIdentityQueryKind(message = "", contract = {}, recognition = {}) {
  if (contract.identityQueryKind) return contract.identityQueryKind;
  const classified = classifyIdentityQuery(message || contract.resolvedQuery || "");
  if (classified !== IDENTITY_QUERY_KIND.GENERAL) return classified;
  const supplement = classifyIdentityQuerySupplement(message || contract.resolvedQuery || "");
  if (supplement) return supplement;
  const mode = recognition.interactionMode || contract.interactionMode;
  if (recognition.primaryIntent === "about_mia" || mode === MIA_INTERACTION_MODES.IDENTITY) {
...
export function resolveConversationalIdentityPresence({
  ...
  const identityQueryKind = resolveIdentityQueryKind(message, contract, recognition);
...
export function applyConversationalIdentityPresenceGovernance(text = "", contract = {}) {
  ...
  if (contract.personalityGovernanceBypass && contract.identityQueryKind) {
    return { reply: raw, replaced: false, violations: [] };
  }
