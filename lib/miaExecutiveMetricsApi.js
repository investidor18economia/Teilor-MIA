...
async function fetchMetricGroup(client, rpcName, windowDays, offsetDays = 0) {
  ...
  if (error && offsetDays > 0 && /p_offset_days|does not exist|could not find/i.test(String(error.message || error))) {
    return {
      ok: false,
      data: null,
  ...
  if (error && offsetDays > 0) {
    ({ data, error } = await client.rpc(rpcName, { p_days: windowDays }));
  }
  return {
    ok: !error,
    data: data ?? null,
