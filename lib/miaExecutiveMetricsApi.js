 ...
 */
async function fetchMetricGroup(client, rpcName, windowDays) {
  const started = Date.now();
  const { data, error } = await client.rpc(rpcName, { p_days: windowDays });
  return {
    ok: !error,
    data: data ?? null,
...
export async function buildExecutiveMetricsResponse(options = {}) {
  ...
  const cacheKey = `executive-metrics:v${MIA_EXECUTIVE_METRICS_VERSION}:d${windowDays}`;
  ...
  const results = await Promise.all(
    rpcCategories.map(async (category) => {
      ...
      return { category, ...(await fetchMetricGroup(supabase, rpcName, windowDays)) };
    })
  );
