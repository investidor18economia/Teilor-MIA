...
const CONVERSATION_REQUEST_MARKERS =
  /\b(s[oó]\s+queria\s+convers|so\s+queria\s+convers|queria\s+convers|bater\s+papo|trocar\s+ideia|falar\s+um\s+pouco|conversar\s+um\s+pouco)\b/;
...
export function isComplimentDirectedAtMia(message = "") {
  ...
  if (FLIRT_MARKERS.test(q) && (MIA_DIRECT_ADDRESS.test(q) || tokenCount(q) <= 3)) return true;
