...
export const SOCIAL_INTENT_TAXONOMY_VERSION = "4.1I";
...
const FRUSTRATION_MARKERS =
  /\b(n[aã]o\s+(?:ta|tá|est[aá])\s+ajud|nao\s+ajud|n[aã]o\s+entend|nao\s+entend|viajou|doidez|doideza|t[aá]\s+doid\w*|que\s+assistente|inutil|inútil|p[eé]ssim[ao]|horr[ií]vel|nada\s+a\s+ver|para\s+de\s+enrol)\b/;

const DISAPPROVAL_MARKERS =
  /\b(n[aã]o\s+gost\w*|n[aã]o\s+curt\w*|achei\s+ruim|ficou\s+p[eé]ssim\w*|n[aã]o\s+me\s+convenc\w*|nao\s+me\s+convenc\w*|prefiro\s+outr\w*|muda\s+isso|esse\s+n[aã]o\s+serve|nao\s+quero\s+esse|n[aã]o\s+quero\s+esse)\b/;
...
const CORRECTION_MARKERS =
  /\b(n[aã]o\s+entend\w*|voce\s+nao\s+entend|você\s+não\s+entend|entendeu\s+errad|entendeu\s+nada|deixou\s+passar|viajou|errad[ao]|confundiu|nao\s+foi\s+isso|não\s+foi\s+isso)\b/;
...
const FAMILY_PRIORITY = Object.freeze({
  ...
  [SOCIAL_INTENT_FAMILIES.DISAPPROVAL]: 38,
});
...
export function detectSocialIntentFamilies(message = "", context = {}) {
  ...
  if (PROFANITY_OR_INSULT.test(q)) {
    const isInsult = /\b(burr|idiot|inutil|inútil|lix|fajut|rob[oô]|otari|imbecil|assistente\s+inutil)\b/.test(q);
    matches.push(
      buildFamilyMatch(
  ...
  // Frustration
  if (FRUSTRATION_MARKERS.test(q)) {
    matches.push(
      buildFamilyMatch(SOCIAL_INTENT_FAMILIES.FRUSTRATION, 0.84, ["frustration_signal"], ["frustration"])
    );
  }

  // Disapproval / rejection (before correction — "não foi isso" stays correction)
  ...
  // Correction / context repair / conversation recovery
  if (CORRECTION_MARKERS.test(q)) {
    matches.push(
      buildFamilyMatch(SOCIAL_INTENT_FAMILIES.CORRECTION, 0.87, ["correction_signal"], ["correction"])
    );
    matches.push(
      buildFamilyMatch(
  ...
  // Soft disagreement
  if (existing.softDisagreement || cognitive.isSoftDisagreement) {
    matches.push(
      buildFamilyMatch(
