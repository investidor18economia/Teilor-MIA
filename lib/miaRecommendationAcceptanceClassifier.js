...
const FOLLOW_UP_SIGNAL_MAP = Object.freeze({
  ...
  [COMMERCIAL_FOLLOW_UP_TYPES.RUNNER_UP_FOLLOW_UP]: {
    signal_type: MIA_ACCEPTANCE_SIGNAL_TYPES.COMPARISON_REQUESTED,
    signal_strength: MIA_ACCEPTANCE_SIGNAL_STRENGTHS.MEDIUM,
    signal_target: MIA_ACCEPTANCE_SIGNAL_TARGETS.RUNNER_UP,
  },
});
...
export function classifyAcceptanceSignalFromFollowUp(followUpType = "", input = {}) {
  ...
  const runnerUpFamily = input.runnerUpProductFamilyHash ?? null;
  const signalTarget =
    followUpType === COMMERCIAL_FOLLOW_UP_TYPES.RUNNER_UP_FOLLOW_UP
      ? MIA_ACCEPTANCE_SIGNAL_TARGETS.RUNNER_UP
      : mapped.signal_target;
  ...
  return {
    ...
    signal_observed: true,
    product_family_hash:
      signalTarget === MIA_ACCEPTANCE_SIGNAL_TARGETS.RUNNER_UP
        ? runnerUpFamily || winnerFamily
        : winnerFamily,
...
function resolveSignalTargetFromClientRow(row, decisionContext = {}, fallback = "UNKNOWN") {
  ...
  const winnerFamily =
    ...
    null;
  const runnerUpFamily =
    decisionContext.runnerUpProductFamilyHash ||
    decisionContext.runner_up_product_family ||
    decisionContext.runner_up_product_family_hash ||
    null;
