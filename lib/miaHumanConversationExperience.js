...
import {
  ...
  enrichContractWithFactValidation,
...
import {
  ...
  personalityGovernanceToTrace,
} from "./miaPersonalityGovernance.js";
import {
  enrichContractWithSocialConversationContinuity,
  socialConversationContinuityToTrace,
} from "./miaSocialConversationContinuity.js";
import {
  enrichContractWithConversationalRhythm,
  applyConversationalRhythmGovernance,
  conversationalRhythmToTrace,
} from "./miaConversationalRhythmGovernance.js";
import {
  enrichContractWithSocialHumanization,
  applySocialHumanizationGovernance,
  socialHumanizationToTrace,
} from "./miaSocialHumanizationGovernance.js";
import {
  enrichContractWithHumanWarmthPresence,
  applyHumanWarmthPresenceGovernance,
  humanWarmthPresenceToTrace,
} from "./miaHumanWarmthPresenceGovernance.js";
import {
  enrichContractWithStructuralExpression,
  applyStructuralExpressionGovernance,
  structuralExpressionToTrace,
} from "./miaStructuralExpressionGovernance.js";
import {
  enrichContractWithConversationalIdentityPresence,
  applyConversationalIdentityPresenceGovernance,
  conversationalIdentityPresenceToTrace,
} from "./miaConversationalIdentityPresenceGovernance.js";
import {
  enrichContractWithConversationalIntent,
  detectConversationalIntentViolations,
  applyConversationalIntentGovernance,
} from "./miaConversationalIntentGovernance.js";
import {
...
import { buildUniversalContractFromHumanFinalization } from "./miaUniversalConversationResponseContract.js";

export const HUMAN_EXPERIENCE_VERSION = "5.8.8.3";
...
const PRODUCT_RECOMMENDATION_BLEED_PATTERN =
  /\b(minha escolha|eu iria no|recomend\w*\s+(?:o|a|um|uma)|iphone|galaxy|samsung|notebook gamer|lenovo|asus|monitor gamer)\b/i;
...
export function shouldForceSocialExperienceEgress(contract = {}) {
  return !!(
    contract?.socialDepartureMode ||
    contract?.socialContinuityBehavior === "social_closing" ||
    (contract?.farewellMode && contract?.socialContinuityBypass)
  );
}
...
function resolveResponseDepth(recognition = {}, contract = {}) {
  ...
  if (length <= 3) return RESPONSE_DEPTH.BRIEF;
...
function resolveFollowUpPolicy(recognition = {}, contract = {}) {
  ...
  if (
    recognition.primarySocialIntent === "disapproval" ||
    recognition.primarySocialIntent === "soft_disagreement" ||
    recognition.expectedHumanBehavior === "acknowledge_disapproval"
  ) {
    return FOLLOW_UP_POLICY.CLARIFYING_REQUIRED;
  }
...
export function enrichBehaviorContractWithHumanExperience(
  ...
  enriched = enrichContractWithFactValidation(enriched, {
    recognition: rec,
    sessionContext: sessionContext || authority?.sessionContext || {},
    conversationMessages,
    message: message || rec.resolvedQuery || "",
  ...
  enriched = enrichContractWithPersonalityGovernance(enriched, {
    recognition: rec,
    message: message || rec.resolvedQuery || "",
    conversationMessages,
    sessionContext: sessionContext || authority?.sessionContext || {},
  ...
  enriched = enrichContractWithSocialConversationContinuity(enriched, {
    recognition: rec,
    message: message || rec.resolvedQuery || "",
    conversationMessages,
    sessionContext: sessionContext || authority?.sessionContext || {},
  ...
  if (enriched.socialDepartureMode || enriched.farewellMode) {
    enriched = {
      ...enriched,
      commerceReentryPolicy: COMMERCE_REENTRY_POLICY.FORBIDDEN,
      followUpPolicy: FOLLOW_UP_POLICY.NONE,
      conversationContinuation: "complete",
      responseBehavior: {
        ...(enriched.responseBehavior || {}),
        redirectToCommerce: false,
        askFollowUp: false,
        forbidden: [
          ...new Set([
            ...(enriched.responseBehavior?.forbidden || []),
            "forced_commerce_redirect",
            "end_with_shopping_offer",
            "institutional_pitch",
            "product_recommendation_on_departure",
          ]),
        ],
      },
    };
  ...
  enriched = enrichContractWithConversationalRhythm(enriched, {
    recognition: rec,
    message: message || rec.resolvedQuery || "",
    conversationMessages,
    sessionContext: sessionContext || authority?.sessionContext || {},
  ...
  enriched = enrichContractWithSocialHumanization(enriched, {
    recognition: rec,
    message: message || rec.resolvedQuery || "",
    conversationMessages,
  });

  enriched = enrichContractWithHumanWarmthPresence(enriched, {
    recognition: rec,
    conversationMessages,
  });

  enriched = enrichContractWithStructuralExpression(enriched, {
    conversationMessages,
    sessionContext: sessionContext || authority?.sessionContext || {},
  });

  enriched = enrichContractWithConversationalIdentityPresence(enriched, {
    recognition: rec,
    message: message || rec.resolvedQuery || "",
    conversationMessages,
  });

  enriched = enrichContractWithConversationalIntent(enriched, {
    message: message || rec.resolvedQuery || "",
    recognition: rec,
  });
...
export function validateHumanConversationResponse(
  ...
  if (
    isGovernedAmbiguousSocialContract(contract, contract.semanticTargetResolution) &&
    isNeutralClarificationText(text)
  ) {
    violations.push("neutral_clarification_on_ambiguous_social");
  }

  if (
    isGovernedAmbiguousSocialContract(contract, contract.semanticTargetResolution) &&
    /\b(produto|aparelho|visual dele|visual dela|tem um visual|design do)\b/i.test(normalized)
  ) {
    violations.push("product_assumption_on_ambiguous_social");
  }
  ...
  if (
    ...
    violations.push("mia_thanks_on_ambiguous_social");
  }
  ...
  if (
    contract.factValidation?.blocksAutoConfirmation &&
    detectUnvalidatedClaimConfirmation(text, contract).detected
  ) {
    violations.push("unvalidated_user_claim_confirmation");
  }
  ...
  if (
    (contract.socialDepartureMode || contract.farewellMode) &&
    PRODUCT_RECOMMENDATION_BLEED_PATTERN.test(normalized)
  ) {
    violations.push("commercial_bleed_on_departure");
  }
...
export function finalizeHumanConversationReply(
  ...
  const factGate = applyFactValidationGovernance(rawLlmResponse, contract);
  let text = factGate.replaced ? factGate.reply : rawLlmResponse;
  let factValidationApplied = factGate.replaced;

  const personalityGate = applyPersonalityGovernance(text, contract);
  if (personalityGate.replaced) {
    text = personalityGate.reply;
  }

  const rhythmGate = applyConversationalRhythmGovernance(text, contract);
  if (rhythmGate.replaced) {
    text = rhythmGate.reply;
  }

  const humanizationGate = applySocialHumanizationGovernance(text, contract);
  if (humanizationGate.replaced) {
    text = humanizationGate.reply;
  }

  const warmthPresenceGate = applyHumanWarmthPresenceGovernance(text, contract);
  if (warmthPresenceGate.replaced) {
    text = warmthPresenceGate.reply;
  }

  const structuralGate = applyStructuralExpressionGovernance(text, contract);
  if (structuralGate.replaced) {
    text = structuralGate.reply;
  }

  const identityPresenceGate = applyConversationalIdentityPresenceGovernance(text, contract);
  if (identityPresenceGate.replaced) {
    text = identityPresenceGate.reply;
  }

  const intentGate = applyConversationalIntentGovernance(text, contract);
  if (intentGate.replaced) {
    text = intentGate.reply;
  }

  const intentViolations = detectConversationalIntentViolations(text, contract);
  const gateViolationCount =
    (humanizationGate.violations?.length || 0) +
    (warmthPresenceGate.violations?.length || 0) +
    (identityPresenceGate.violations?.length || 0) +
    (personalityGate.violations?.length || 0) +
    (intentGate.violations?.length || 0) +
    intentViolations.length;

  if (
    !factGate.replaced &&
    !personalityGate.replaced &&
    !rhythmGate.replaced &&
    !humanizationGate.replaced &&
    !warmthPresenceGate.replaced &&
    !structuralGate.replaced &&
    !identityPresenceGate.replaced
  ) {
    text = stripUnauthorizedCommercialMentions(rawLlmResponse, contract);
  ...
  const universalContract = buildUniversalContractFromHumanFinalization(
    contract,
    {
      rawLlmResponse,
      response: text,
...
export function humanExperienceToTrace(contract = null, validation = null) {
  ...
  return {
    ...
    perception: socialPerceptionToTrace(contract, validation?.perception || null),
    personalityGovernance: personalityGovernanceToTrace(contract),
    socialConversationContinuity: socialConversationContinuityToTrace(contract),
    conversationalRhythm: conversationalRhythmToTrace(contract),
    socialHumanization: socialHumanizationToTrace(contract),
    humanWarmthPresence: humanWarmthPresenceToTrace(contract),
    structuralExpression: structuralExpressionToTrace(contract),
    conversationalIdentityPresence: conversationalIdentityPresenceToTrace(contract),
  };
}
