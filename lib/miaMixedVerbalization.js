...
import { selectHumanAck } from "./miaConversationPolish.js";
import { collectDecisionFactsFromSession } from "./miaDecisionFactsNarrative.js";

export const MIXED_VERBALIZATION_VERSION = "11C";
...
export function buildMixedResponseContext(
  ...
  return {
    ...
    sessionContext,
    decisionFacts: sessionContext ? collectDecisionFactsFromSession(sessionContext) : null,
  };
}
...
function buildStructuralHumanFallbackLine(contract = {}, humanSnapshot = null) {
  ...
  const depth =
    mv.humanAcknowledgementDepth === HUMAN_ACKNOWLEDGEMENT_DEPTH.MINIMAL ? "minimal" : "brief";
  const sessionContext = contract.sessionContext || null;
  const decisionFacts =
    contract.decisionFacts ||
    (sessionContext ? collectDecisionFactsFromSession(sessionContext) : null);

  return selectHumanAck({
    anchors,
    polarity: polarity === HUMAN_POLARITY.POSITIVE ? "positive" : "neutral",
    depth,
    message: contract.resolvedQuery || contract.rawUserMessage || "",
