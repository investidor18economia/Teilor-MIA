...
export function buildFirstAnswerStructuredReply({
  ...
  const planOrdered = hasNarrativePlan(narrativePlan)
  ...
  const primaryGain =
    narrativePlan?.primaryNarrative?.legacyText || pickPrimaryGain(safeGains, winner);
  const openingGain =
...
export function applyFirstAnswerResponseContract(input = {}) {
  ...
  const { gains, sacrifices, narrativePlan } = extractGainsAndSacrificesFromProduct(tradeoffSource);
