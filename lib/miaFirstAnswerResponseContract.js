...
import { surfaceRewriteFragment, rewriteConsequenceForSpeech } from "./miaVerbalizationStyleGovernor.js";
import {
...
} from "./miaMixedVerbalization.js";

export const FIRST_ANSWER_RESPONSE_CONTRACT_VERSION = "11C";
...
export function isFalseSacrificeText(text = "") {
  ...
  if (/um dos pontos fortes|pontos fortes é|experi[eê]ncia forte na faixa/i.test(body)) return true;
  return false;
}

export function sanitizeSacrificeItems(items = []) {
  const cleaned = (Array.isArray(items) ? items : [])
    .map((entry) => sanitizeInternalLabelText(cleanText(entry)))
    .filter(Boolean)
...
export function extractGainsAndSacrificesFromProduct(product = {}) {
  const verbalizationPlan =
    product?.presentation?.tradeoff?.verbalizationPlan ||
    product?.presentation?.verbalizationPlan ||
    null;
  const narrativePlan =
    product?.presentation?.tradeoff?.narrativePlan ||
    product?.presentation?.narrativePlan ||
    null;

  if (hasVerbalizationPlan(verbalizationPlan)) {
    const ordered = verbalizationPlanToOrderedLegacyStrings(verbalizationPlan);
    return {
      gains: ordered.gains.slice(0, 3),
      sacrifices: ordered.sacrifices.slice(0, 2),
      structuredDecisionFacts:
        product?.presentation?.tradeoff?.structuredDecisionFacts ||
        product?.presentation?.structuredDecisionFacts ||
        null,
      narrativePlan,
      verbalizationPlan,
    };
  ...
  if (hasNarrativePlan(narrativePlan)) {
    const ordered = narrativePlanToOrderedLegacyStrings(narrativePlan);
    return {
      gains: ordered.gains.slice(0, 3),
      sacrifices: ordered.sacrifices.slice(0, 2),
      structuredDecisionFacts:
  ...
  const presentationStructured =
    product?.presentation?.tradeoff?.structuredDecisionFacts ||
    product?.presentation?.structuredDecisionFacts ||
    null;

  if (presentationStructured?.legacy) {
    return {
      gains: cleanList(presentationStructured.legacy.advantages, 3),
      sacrifices: cleanList(presentationStructured.legacy.sacrifices, 2),
      structuredDecisionFacts: presentationStructured,
    };
  }
...
function pickConsequenceParagraph(gains = [], query = "", ledger = null) {
  const gain = pickUnusedGain(
    sanitizeGainItems(gains).map((g) => g.replace(/\.$/, "")),
    ledger,
    0
  );
  const spoken = gain
    ? rewriteConsequenceForSpeech(gain)
    : "";
  if (!spoken) {
    return "No conjunto, o equilíbrio entre ganhos e concessões ainda fecha bem para o que você pediu — vale olhar o uso real, não só o anúncio.";
  }
  if (/modelo que você citou|foco principal/i.test(spoken)) {
    return `Escolher o modelo certo evita cair em alternativa parecida que não era o que você pediu.`;
  }
  if (ledger) claimSemanticSlot(ledger, spoken, "consequence_paragraph", { allowNearDuplicate: true, threshold: 0.8 });
  const body = spoken.charAt(0).toUpperCase() + spoken.slice(1).replace(/\.$/, "");
  return `${body} — isso tende a aparecer no uso real, não só no anúncio.`;
}
...
export function matchesStrictFirstAnswerContract(reply = "", winnerName = "") {
  ...
  if (!matchesPolishedFirstAnswerOpening(body)) return false;
  if (!/O que voc[eê] ganha/i.test(body)) return false;
  if (!/O que voc[eê] abre m[aã]o/i.test(body)) return false;
  if (!/Mesmo (?:com|considerando(?: que)?|sabendo que|reconhecendo que).+eu manteria o/i.test(body)) {
    return false;
  }
  ...
  if ((body.match(/Mesmo (?:com|considerando|sabendo|reconhecendo)/gi) || []).length > 1) return false;
...
export function buildFirstAnswerStructuredReply({
  ...
  narrativePlan = null,
  verbalizationPlan = null,
} = {}) {
  const winner = cleanText(winnerName);
  if (!winner) return "";

  const verbalOrdered = hasVerbalizationPlan(verbalizationPlan)
    ? verbalizationPlanToOrderedLegacyStrings(verbalizationPlan)
    : null;
  const planOrdered = verbalOrdered?.gains?.length
    ? verbalOrdered
    : hasNarrativePlan(narrativePlan)
      ? narrativePlanToOrderedLegacyStrings(narrativePlan)
      : null;
  const safeGains = sanitizeGainItems(planOrdered?.gains?.length ? planOrdered.gains : gains).map(
    (entry, index) =>
      surfaceRewriteFragment(entry, {
        effectKey:
          verbalizationPlan?.supportingMessages?.[index]?.effectKey ||
          verbalizationPlan?.mainMessage?.effectKey ||
          narrativePlan?.supportingArguments?.[index]?.effectKey ||
          narrativePlan?.primaryNarrative?.effectKey ||
          "",
      })
  );
  const safeSacrifices = sanitizeSacrificeItems(
    planOrdered?.sacrifices?.length ? planOrdered.sacrifices : sacrifices
  );

  const ledger = createCompositionLedger();
  const primaryGain =
    verbalizationPlan?.mainMessage?.text ||
    narrativePlan?.primaryNarrative?.legacyText ||
    pickPrimaryGain(safeGains, winner);
  const openingGain = rewriteConsequenceForSpeech(
    primaryGain.charAt(0).toLowerCase() + primaryGain.slice(1).replace(/\.$/, "")
  );
  claimSemanticSlot(ledger, openingGain, "opening_gain");

  const opening = buildFirstAnswerOpening({
    winner,
    gainPhrase: openingGain,
    seed: query || winner,
  });

  const guardedComparative = guardComparativeParagraph(comparativeParagraph, openingGain);
  if (guardedComparative) claimSemanticSlot(ledger, guardedComparative, "runner_up", { allowNearDuplicate: true, threshold: 0.75 });

  const consequence = pickConsequenceParagraph(safeGains, query, ledger);

  const listedGains = dedupeGainBullets(safeGains, ledger);
  ...
  const closingReason =
    pickUnusedGain(safeGains, ledger, 0) ||
    pickDominantReason(listedGains, safeSacrifices).replace(/\.$/, "");
  const tradeoffForClosing =
    safeSacrifices.find((entry) => entry !== NEUTRAL_SACRIFICE) || safeSacrifices[0] || NEUTRAL_SACRIFICE;
  const closing = buildMesmoComClosing({
    winner,
    tradeoff: tradeoffForClosing,
  ...
  return [
    ...
    normalizeSingleLineSection(guardedComparative),
...
export function hasFirstAnswerStructure(reply = "") {
  ...
  return (
    ...
    /Eu iria no|Minha escolha|ficou no topo|recomendo|Neste cen[aá]rio|A escolha mais equilibrada|Entre as op[cç][õo]es/i.test(body)
  );
}
 ...
 *   isFollowUp?: boolean,
 *   mixedVerbalization?: Record<string, unknown>|null,
...
export function applyFirstAnswerResponseContract(input = {}) {
  ...
  const preliminaryWinnerName =
    cleanText(
      input.winnerProduct?.product_name ||
        input.winnerProduct?.trustedSpecs?.official_name ||
        input.prices?.[0]?.product_name ||
        ""
    ) || "";

  let reply = sanitizeFirstAnswerReplyText(input.reply || "");
  const mixedContract = input.mixedVerbalization || null;
  const mixedSplit = mixedContract?.humanAcknowledgementRequired
    ? splitMixedReplySections(reply, { winnerName: preliminaryWinnerName })
    : { humanPrefix: "", commercialBody: reply, splitIndex: -1 };
  let preservedHumanPrefix = mixedSplit.humanPrefix || "";
  if (mixedContract && preservedHumanPrefix) {
    reply = mixedSplit.commercialBody || reply;
  }
  ...
  const { gains, sacrifices, narrativePlan, verbalizationPlan } =
    extractGainsAndSacrificesFromProduct(tradeoffSource);
  ...
  if (mixedContract?.humanAcknowledgementRequired && (preservedHumanPrefix || rebuilt)) {
    reply = mergeMixedReplySections(preservedHumanPrefix, reply, {
      mixedVerbalization: mixedContract,
