...
function pickConsequenceParagraph(gains = [], query = "") {
  const gain = rewriteConsequenceForSpeech(pickPrimaryGain(gains));
  ...
  const body = gain.charAt(0).toUpperCase() + gain.slice(1).replace(/\.$/, "");
...
export function buildFirstAnswerStructuredReply({
  ...
  const dominantReason = rewriteConsequenceForSpeech(
    pickDominantReason(safeGains, safeSacrifices).replace(/\.$/, "")
  );
  const closing = `Mesmo com ${pickTradeoffSummary(safeSacrifices)}, eu manteria o ${winner} — ${dominantReason.charAt(0).toLowerCase()}${dominantReason.slice(1).replace(/\.$/, "")}.`;
