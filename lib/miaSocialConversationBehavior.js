...
import {
  enrichContractWithSocialVerbalizationBridge,
  socialVerbalizationBridgeToInstructions,
  socialVerbalizationBridgeToTrace,
} from "./miaSocialVerbalizationBridge.js";
import { personalityGovernanceToVerbalizationInstructions } from "./miaPersonalityGovernance.js";
import { socialConversationContinuityToVerbalizationInstructions } from "./miaSocialConversationContinuity.js";
import { conversationalRhythmToVerbalizationInstructions } from "./miaConversationalRhythmGovernance.js";
import { socialHumanizationToVerbalizationInstructions } from "./miaSocialHumanizationGovernance.js";
import { humanWarmthPresenceToVerbalizationInstructions } from "./miaHumanWarmthPresenceGovernance.js";
import { structuralExpressionToVerbalizationInstructions } from "./miaStructuralExpressionGovernance.js";
import { conversationalIdentityPresenceToVerbalizationInstructions } from "./miaConversationalIdentityPresenceGovernance.js";
import { conversationalIntentToVerbalizationInstructions } from "./miaConversationalIntentGovernance.js";
...
export function buildSocialConversationBehaviorContract(recognition = {}, options = {}) {
  ...
  const baseContract = {
    ...
    conversationObjective:
      recognition.conversationObjective || recognition.socialClassification?.conversationObjective || null,
    conversationDirection: recognition.conversationDirection || null,
    expectedHumanBehavior: recognition.expectedHumanBehavior || null,
    emotionalState: recognition.emotionalState || null,
    primaryIntent: recognition.primaryIntent || null,
    primarySocialIntent: recognition.primarySocialIntent || null,
    secondarySocialIntent: recognition.secondarySocialIntent || null,
    socialIntentConfidence: recognition.socialIntentConfidence ?? null,
  ...
  const enriched = enrichBehaviorContractWithHumanExperience(baseContract, {
    recognition,
    authority: options.authority || null,
    message: options.message || recognition.resolvedQuery || "",
    conversationMessages: options.conversationMessages || [],
    sessionContext: options.sessionContext || options.authority?.sessionContext || null,
  });

  return enrichContractWithSocialVerbalizationBridge(enriched);
}

export function buildFullHumanConversationInstructions(contract = {}) {
  const bridgeInstructions = socialVerbalizationBridgeToInstructions(contract);
  ...
  return [
    ...
    contract.personalityGovernanceVersion
      ? personalityGovernanceToVerbalizationInstructions(contract)
      : "",
    contract.socialConversationContinuityVersion
      ? socialConversationContinuityToVerbalizationInstructions(contract)
      : "",
    contract.conversationalRhythmVersion
      ? conversationalRhythmToVerbalizationInstructions(contract)
      : "",
    contract.socialHumanizationVersion
      ? socialHumanizationToVerbalizationInstructions(contract)
      : "",
    contract.humanWarmthPresenceVersion
      ? humanWarmthPresenceToVerbalizationInstructions(contract)
      : "",
    contract.structuralExpressionVersion
      ? structuralExpressionToVerbalizationInstructions(contract)
      : "",
    contract.conversationalIdentityPresenceVersion
      ? conversationalIdentityPresenceToVerbalizationInstructions(contract)
      : "",
    contract.conversationalIntentVersion
      ? conversationalIntentToVerbalizationInstructions(contract)
      : "",
...
export { socialVerbalizationBridgeToTrace };
