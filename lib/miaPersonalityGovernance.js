...
export const PERSONALITY_GOVERNANCE_VERSION = "5.8.2";
...
const RECIPROCAL_MARKERS =
  /\b(e\s+voc[eê]|e\s+contigo|e\s+a[ií]|como\s+voc[eê]\s+t[aá]|como\s+(foi|t[aá])\s+(seu|o)\s+dia|dormiu\s+bem)\b/i;
...
export function buildGovernedIdentityReply(contract = {}) {
  ...
  return pickHumanizedVariant(variants, seedFromContract(contract, `identity-${kind}`));
}
...
export function buildPersonalityGovernedGreetingReply(contract = {}, mirrorBuilder) {
  ...
  const key = warmthKey(contract);
  const hook = pickHumanizedVariant(
