...
export const PERSONALITY_GOVERNANCE_VERSION = "5.8.8.3";
...
const IDENTITY_MEMORY =
  /\b(voc[eê]\s+lembra|tem\s+mem[oó]ria|lembra\s+das\s+coisas|guarda\s+isso|lembra\s+de\s+mim|voc[eê]\s+lembra\s+de\s+mim|guarda\s+(?:meus\s+)?dados|grava\s+(?:meus\s+)?dados)\b/i;
const IDENTITY_MODEL =
  /\b(chat\s*gpt|open\s*ai|qual\s+modelo|gpt[\s-]?4|claude|usa\s+chatgpt|qual\s+ia\s+voc[eê]\s+usa|trocar\s+de\s+modelo|pode\s+trocar\s+de\s+modelo)\b/i;
const IDENTITY_AI_NATURE =
  /\b(voc[eê]\s+[eé]\s+(?:(?:s[oó]|apenas|mesmo|realmente)\s+)?(?:uma\s+)?ia|voc[eê]\s+[eé]\s+(?:(?:s[oó]|apenas|mesmo|realmente)\s+)?chatgpt|voc[eê]\s+[eé]\s+(?:(?:s[oó]|apenas|mesmo|realmente)\s+)?(?:um\s+)?rob[oô]|intelig[eê]ncia\s+artificial)\b/i;
...
const GENERIC_STAY_SOCIAL_INVITE =
  /\b(fico por aqui|estou acompanhando|pode falar|me conta mais|claro — me conta|o que voce quer conversar|manda ver,? t[oô] aqui|t[oô] aqui no papo)\b/i;

const MIA_IDENTITY_SURFACE_MARKER =
...
export function classifyIdentityQuerySupplement(message = "") {
  const q = normalizeText(message);
  if (/\b(teilor|da teilor)\b/.test(q) && /\b(voc[eê]|mia|[eé]|quem|o que)\b/.test(q)) {
    return IDENTITY_QUERY_KIND.MIA_BRAND;
  }
  if (/\b(especialidade|especializ\w*)\b/.test(q)) return IDENTITY_QUERY_KIND.CAPABILITY;
  if (/\b(guarda|grava|salva|registra)\b/.test(q) && /\b(falo|digo|mensagen|convers\w*|dados|privacidade)\b/.test(q)) {
    return IDENTITY_QUERY_KIND.MEMORY;
  }
  if (/\bassistente virtual\b/.test(q)) return IDENTITY_QUERY_KIND.AI_NATURE;
  if (/\blimites?\b/.test(q)) return IDENTITY_QUERY_KIND.GENERAL;
  if (
    /\b(voc[eê]\s+[eé]\s+(?:(?:s[oó]|apenas|mesmo|realmente)\s+)?(?:humana|pessoa|gente|um\s+ser\s+humano|uma\s+ia|um\s+rob[oô]))\b/.test(
      q
    )
  ) {
    return IDENTITY_QUERY_KIND.AI_NATURE;
  }
  if (/\b(voc[eê]\s+[eé]\s+(?:real|gente))\b/.test(q)) return IDENTITY_QUERY_KIND.REAL;
  if (/\b(finge\s+ser|finjo\s+ser|simula\s+ser)\s+(humana|pessoa|gente|real)\b/.test(q)) {
    return IDENTITY_QUERY_KIND.AI_NATURE;
  }
  if (/\b(trocar|troca)\b/.test(q) && /\bmodelo\b/.test(q)) {
    return IDENTITY_QUERY_KIND.MODEL_TECH;
  }
  if (/\bo\s+que\s+[eé]\s+a\s+teilor\b/.test(q) || /\bquem\s+[eé]\s+a\s+teilor\b/.test(q)) {
    return IDENTITY_QUERY_KIND.MIA_BRAND;
  }
  if (/\b(llm|alimenta|stack tecnol[oó]gico|arquitetura)\b/.test(q)) {
    return IDENTITY_QUERY_KIND.MODEL_TECH;
  }
  if (/\b(trein\w*)\b/.test(q) && /\b(mensagen\w*|convers\w*|dados|falo)\b/.test(q)) {
    return IDENTITY_QUERY_KIND.LEARNING;
  }
  if (/\b(me\s+)?(fala|conta)\b/.test(q) && /\b(sobre\s+)?voc[eê]\b/.test(q)) {
    return IDENTITY_QUERY_KIND.WHO;
  }
  if (/\bconta\s+quem\b/.test(q)) return IDENTITY_QUERY_KIND.WHO;
  if (/\bopen\s*ai\b/.test(q) && q.length <= 24) return IDENTITY_QUERY_KIND.MODEL_TECH;
  if (/\bwhat model\b/.test(q)) return IDENTITY_QUERY_KIND.MODEL_TECH;
  if (/\b(transpar[eê]ncia|privacidade)\b/.test(q) && q.length <= 40) {
    return IDENTITY_QUERY_KIND.GENERAL;
  }
  return null;
}

export function resolveIdentityQueryKind(
  message = "",
  { recognition = {}, contract = {} } = {}
) {
  if (contract.identityQueryKind) return contract.identityQueryKind;
  const userMessage = message || contract.resolvedQuery || "";
  const classified = classifyIdentityQuery(userMessage);
  if (classified !== IDENTITY_QUERY_KIND.GENERAL) return classified;
  const supplement = classifyIdentityQuerySupplement(userMessage);
...
export function isGenericStaySocialInvite(text = "") {
  const normalized = normalizeText(text);
  if (!normalized || MIA_IDENTITY_SURFACE_MARKER.test(normalized)) return false;
  return GENERIC_STAY_SOCIAL_INVITE.test(normalized) || GENERIC_STAY_SOCIAL.test(normalized);
}

export function propagateIdentityQueryContractFields(contract = {}, identityQueryKind = null) {
  if (!identityQueryKind) return contract;
  return {
    ...contract,
    identityQueryKind,
    identityMode: contract.identityMode || true,
...
export function resolveCentralPersonalityPolicy({
  ...
  const identityKind = resolveIdentityQueryKind(userMessage, {
    recognition,
    contract,
  });
...
export function enrichContractWithPersonalityGovernance(
  ...
  const enriched = propagateIdentityQueryContractFields(
    {
      ...contract,
      ...
      personalityGovernanceBypass:
        bypassLlmVerbalization && !contract.factValidation?.bypassLlmVerbalization,
    },
    centralPersonalityPolicy.identityQueryKind
  );
...
export function personalityGovernanceToVerbalizationInstructions(contract = {}) {
  ...
  const lines = [
    ...
    "- Participar emocionalmente de forma leve quando o contexto permitir — presença, não apenas informação.",
    "- Nunca infantil, coach, robótica, fria, forçada, exagerada, institucional ou marketing.",
  ...
  const templates = {
    ...
    [IDENTITY_QUERY_KIND.MEMORY]: {
      warm_balanced: [
        ...
        `Cuido dos dados com responsabilidade — a ${MIA_IDENTITY.name} da ${MIA_IDENTITY.brand} usa o contexto desta conversa para te ajudar, sem inventar memória permanente.`,
      ],
      warm_light: [
    ...
    [IDENTITY_QUERY_KIND.MODEL_TECH]: {
      warm_balanced: [
        ...
        `A arquitetura da ${MIA_IDENTITY.name} é independente do modelo — eu conduzo a experiência; a tecnologia verbaliza de forma natural.`,
      ],
      warm_reserved: [
    ...
    [IDENTITY_QUERY_KIND.AI_NATURE]: {
      warm_balanced: [
        ...
        `Não finjo ser humana — sou a ${MIA_IDENTITY.name}, da ${MIA_IDENTITY.brand}, e levo a conversa a sério com leveza.`,
      ],
    },
    [IDENTITY_QUERY_KIND.LEARNING]: {
      warm_balanced: [
        ...
        `Não treino com suas mensagens de forma permanente — a ${MIA_IDENTITY.name} acompanha o papo atual com responsabilidade.`,
      ],
      warm_reserved: [
...
export function buildPersonalityGovernedStaySocialReply(contract = {}) {
  if (contract.identityQueryKind || contract.expectedHumanBehavior === EXPECTED_HUMAN_BEHAVIORS.ANSWER_META) {
    return buildGovernedIdentityReply(contract);
  }
...
export function detectPersonalityViolations(reply = "", contract = {}) {
  ...
  if (
    ...
    (GENERIC_STAY_SOCIAL.test(normalized) || isGenericStaySocialInvite(text))
  ) {
