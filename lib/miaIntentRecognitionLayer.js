...
import { resolveCorrectionContinuity } from "./miaCorrectionContinuityGovernance.js";
import {
...
function resolveInteractionMode({
  ...
  const correctionContinuity = resolveCorrectionContinuity(message, {
    conversationMessages,
    sessionContext,
  });
  if (correctionContinuity.blocksClarification) {
    return {
      interactionMode: MIA_INTERACTION_MODES.SOCIAL,
      confidence: 0.84,
      ambiguity: 0.2,
      requiresClarification: false,
      preserveCommerceContext: !!correctionContinuity.preservePreviousTarget,
      reasons: correctionContinuity.reasonCodes?.length
        ? correctionContinuity.reasonCodes
        : ["correction_continuity_blocks_clarification"],
      correctionContinuity,
    };
  }

  const earlyFillerResolution = resolveConversationalFillerInCommercialContext({
    message,
    sessionContext,
    conversationMessages,
    hasActiveAnchor,
  });
  if (earlyFillerResolution.blocksClarification) {
    return {
      interactionMode: earlyFillerResolution.interactionMode || MIA_INTERACTION_MODES.SOCIAL,
      confidence: 0.77,
      ambiguity: 0.18,
      requiresClarification: false,
      preserveCommerceContext: !!earlyFillerResolution.preserveCommercialAnchor,
      reasons: [earlyFillerResolution.reasonCode || "filler_governance_blocks_clarification"],
      fillerGovernance: earlyFillerResolution,
    };
  }
  ...
  if ((shortAmbiguous || turnAmbiguous) && continuityRelevance >= 0.35) {
    ...
    if (fillerResolution.blocksClarification) {
      return {
        interactionMode: fillerResolution.interactionMode || MIA_INTERACTION_MODES.SOCIAL,
        confidence: 0.76,
        ambiguity: 0.2,
        requiresClarification: false,
        preserveCommerceContext: !!fillerResolution.preserveCommercialAnchor,
        reasons: [fillerResolution.reasonCode || "neutral_filler_preserves_active_commercial_thread"],
...
export function recognizeMiaIntent(input = {}) {
  ...
  return {
    ...
    correctionContinuity: modeResolution.correctionContinuity || null,
    fillerGovernance: modeResolution.fillerGovernance || null,
  };
}
