 ...
 * conversation_id = anonymous chat thread identifier (one or more per session).
 ...
 * @param {{ conversationId?: string|null|false }} [options]
 */
export async function trackMiaEvent(eventName, payload = {}, options = {}) {
  try {
    ...
    await fetch("/api/analytics/track", {
      ...
      body: JSON.stringify(
        buildAnalyticsTrackPayload(eventName, sessionId, payload, visitorId, resolvedConversationId)
      ),
    });
  } catch (err) {
...
export async function trackMiaQuestionSent(
  queryText = "",
  { userId = null, hasImage = false, conversationId = null } = {}
) {
  const eventOptions =
...
export async function trackMiaSessionStarted() {
  ...
  await trackMiaEvent("session_started", buildMiaSessionStartedPayload(), { conversationId: false });
}
