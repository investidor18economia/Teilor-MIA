...
} from "./miaPracticalConsequenceEngine.js";
import {
  applyContextualPriorityToStructuredFacts,
  buildContextualPriorityModel,
  contextualPriorityToTrace,
} from "./miaContextualPriorityEngine.js";
import {
  applyDomainKnowledgeAdapter,
  domainKnowledgeToTrace,
} from "./miaDomainKnowledgeAdapter.js";

export const CONTEXTUAL_DECISION_SYNTHESIS_VERSION = "4A.9.0";
...
/**
 * PATCH 4A.7 — enrich semantic units with governed practical spec translations.
 * @param {{ gainUnits?: Array, sacrificeUnits?: Array, trustedSpecs?: Record<string, unknown>|null, context?: Record<string, unknown> }} input
 */
export function enrichSemanticUnitsWithPracticalConsequences(input = {}) {
  const specs = input.trustedSpecs || null;
  if (!specs || typeof specs !== "object") {
    return {
      gainUnits: input.gainUnits || [],
      sacrificeUnits: input.sacrificeUnits || [],
      practicalConsequences: [],
      practicalConsequenceTrace: null,
    };
  }

  const context = input.context || {};
  const translated = input.mergedTranslatedKnowledge
    ? input.mergedTranslatedKnowledge
    : translateDataLayerFieldsToConsequences(specs);
  const practical = buildPracticalConsequences({
    trustedSpecs: specs,
    translatedKnowledge: translated,
    primaryAxis: context.primaryAxis || "",
    productName: context.productName || null,
    category: context.category || null,
  });

  const practicalUnits = practicalConsequencesToSemanticUnits(practical.consequences, context);
  let gainUnits = Array.isArray(input.gainUnits) ? [...input.gainUnits] : [];
  let sacrificeUnits = Array.isArray(input.sacrificeUnits) ? [...input.sacrificeUnits] : [];

  for (const unit of practicalUnits) {
    if (unit.decisionRole === SEMANTIC_DECISION_ROLE.TRADEOFF) {
      sacrificeUnits.push(unit);
    } else {
      gainUnits.push(unit);
    }
  }

  gainUnits = dedupeSemanticUnitsByEffect(gainUnits);
  sacrificeUnits = dedupeSemanticUnitsByEffect(sacrificeUnits);

  return {
    gainUnits,
    sacrificeUnits,
    practicalConsequences: practical.consequences,
    practicalConsequenceTrace: practicalConsequencesToTrace(practical),
    practicalConsequenceSkipped: practical.skipped,
  };
}
...
export function synthesizeContextualDecisionFacts(input = {}) {
  ...
  if (existing?.semanticUnits?.length) {
    ...
    if (validation.valid && existing.legacy?.isPrimaryTruth !== true) {
      let gainUnits = existing.semanticUnits.filter((unit) => unit.decisionRole !== "tradeoff");
      let sacrificeUnits =
        existing.tradeoffs?.map((entry) => entry.unit).filter(Boolean) || [];
      let practicalConsequences = [];
      let practicalConsequenceTrace = null;

      if (input.trustedSpecs && !input.skipPracticalConsequences) {
        const enriched = enrichSemanticUnitsWithPracticalConsequences({
          gainUnits,
          sacrificeUnits,
          trustedSpecs: input.trustedSpecs,
          mergedTranslatedKnowledge: input.mergedTranslatedKnowledge || null,
          context,
        });
        gainUnits = enriched.gainUnits;
        sacrificeUnits = enriched.sacrificeUnits;
        practicalConsequences = enriched.practicalConsequences;
        practicalConsequenceTrace = enriched.practicalConsequenceTrace;
      }

      const structuredDecisionFacts =
        practicalConsequences.length > 0
          ? buildStructuredDecisionFacts({
              gainUnits,
              sacrificeUnits,
...
export function buildContextualDecisionSynthesisPayload(input = {}) {
  const synthesis = synthesizeContextualDecisionFacts({
    ...input,
    skipPracticalConsequences: true,
  });
  let structured = synthesis.structuredDecisionFacts;
  let gainUnits = synthesis.gainUnits || [];
  let sacrificeUnits = synthesis.sacrificeUnits || [];

  const priorityQuery =
    input.userQuery ||
    input.query ||
    input.sessionContext?.lastQuery ||
    "";

  const priorityModel = buildContextualPriorityModel({
    query: priorityQuery,
    category: input.category || input.sessionContext?.lastCategory || "",
  ...
  if (structured && priorityModel?.criteria?.length) {
    priorityApplied = applyContextualPriorityToStructuredFacts(structured, priorityModel, {
      productName: input.productName || "",
      category: input.category || "",
      primaryAxis: input.primaryAxis || "",
    });
    structured = priorityApplied.structuredDecisionFacts || structured;
    gainUnits = structured?.semanticUnits?.filter((unit) => unit.decisionRole !== "tradeoff") || gainUnits;
    sacrificeUnits =
      structured?.tradeoffs?.map((entry) => entry.unit).filter(Boolean) || sacrificeUnits;
  }

  const domainKnowledge = applyDomainKnowledgeAdapter({
    category: input.category || input.sessionContext?.lastCategory || "",
    productName: input.productName || "",
  ...
  let practicalConsequences = [];
  let practicalConsequenceTrace = null;
  if (input.trustedSpecs) {
    const enriched = enrichSemanticUnitsWithPracticalConsequences({
      gainUnits,
      sacrificeUnits,
      trustedSpecs: domainKnowledge.enrichedTrustedSpecs || input.trustedSpecs,
      mergedTranslatedKnowledge: domainKnowledge.mergedTranslatedKnowledge,
      context: {
        productName: input.productName || null,
        category: input.category || null,
        primaryAxis: input.primaryAxis || "",
    ...
    if (gainUnits.length || sacrificeUnits.length) {
      structured = buildStructuredDecisionFacts({
        gainUnits,
        sacrificeUnits,
        productName: input.productName || "",
        category: input.category || "",
      ...
      if (structured?.legacy) {
        structured.legacy.isPrimaryTruth = false;
      }
    }
  }
  ...
  const verbalization = buildSemanticVerbalizationPayload(narrativePlan, {
    ...
    sessionContext: input.sessionContext || null,
    contextualPriorityModel: priorityModel,
  });
  ...
  return {
    ...
    domainKnowledgeModel: {
      ...
      neutral: domainKnowledge.neutral,
      limitation: domainKnowledge.limitation,
      itemCount: domainKnowledge.items?.length || 0,
      matchedRules: domainKnowledge.matchedRules || [],
      items: domainKnowledge.items || [],
    },
    domainKnowledgeTrace: domainKnowledgeToTrace(domainKnowledge),
    contextualPriorityModel: priorityModel,
    contextualPriorityTrace: contextualPriorityToTrace(priorityModel),
    contextualPriorityApplied: priorityApplied?.reordered === true,
    sessionFields,
...
export function contextualSynthesisToTrace(payload = null) {
  ...
  return {
    ...
    practicalConsequenceEngine: payload.practicalConsequenceTrace || null,
    contextualPriorityEngine: payload.contextualPriorityTrace || null,
    domainKnowledgeAdapter: payload.domainKnowledgeTrace || null,
  };
}
