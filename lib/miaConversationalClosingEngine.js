...
import { setPresentationClosing } from "./miaSpecialistPresentationContract.js";
import {
...
import {
  ...
  isAuthorityDerivedClosing,
} from "./miaAuthorityBridgeLayer.js";
import { isContractGovernedClosing } from "./miaAuthorityClosingContract.js";
import { polishIntentDiscoveryFallback } from "./miaConversationPolish.js";

export const CONVERSATIONAL_CLOSING_ENGINE_VERSION = "9.2S.1";
...
function buildIntentDiscoveryClosing(input = {}, signals = {}) {
  ...
  if (labels.length < 2) {
    return polishIntentDiscoveryFallback(labels);
  }
...
export function applyConversationalClosing(input = {}) {
  ...
  if ((!closing || overlap >= OVERLAP_REJECT_THRESHOLD) && input.closingAuthority) {
    const authorityClosing = verbalizeClosingFromAuthority(input.closingAuthority, signals);
    if (authorityClosing) {
      const authorityOverlap = closingConceptOverlap(authorityClosing, blocks, context);
      const preferAuthority =
        (isAuthorityDerivedClosing(input.closingAuthority) ||
          isContractGovernedClosing(input.closingAuthority)) &&
        overlap >= OVERLAP_REJECT_THRESHOLD;
      if (authorityOverlap < OVERLAP_REJECT_THRESHOLD || preferAuthority) {
        closing = authorityClosing;
        overlap = authorityOverlap;
        mode = "AUTHORITY_BRIDGE_CLOSE";
      }
    }
  }
  ...
  if (
    mode !== "AUTHORITY_BRIDGE_CLOSE" &&
    (!closing || overlap >= OVERLAP_REJECT_THRESHOLD)
  ) {
  ...
  return {
    ...
    signals,
    closing,
    closingAuthority: input.closingAuthority || null,
    authorityDerived: mode === "AUTHORITY_BRIDGE_CLOSE",
    overlap,
    error: null,
  };
}
...
export function finalizeReplyWithConversationalClosing(input = {}) {
  ...
  if (presentation && applied.closing) {
    setPresentationClosing(presentation, applied.closing);
  }
  ...
  return {
    ...
    presentation,
    closingAuthority: applied.closingAuthority || input.closingAuthority || null,
    authorityBridge: input.authorityBridge || null,
  };
}
