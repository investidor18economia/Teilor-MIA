...
export function isLikelyCommercialFollowUp(query = "") {
  ...
  if (isSpecificProductEvaluationQuery(query)) {
    return false;
  }
...
export function shouldResetCommercialOfferContext(input = {}) {
  ...
  if (isSpecificProductEvaluationQuery(currentQuery)) {
    const queryIdentity = resolveProductIdentityFromQuery(currentQuery);
    const prevName = normalizeCommercialText(previousOffer.product_name || "");
    const queryName = normalizeCommercialText(queryIdentity.officialName || "");
    if (
      queryName &&
      prevName &&
      queryName !== prevName &&
      !prevName.includes(queryName) &&
      !queryName.includes(prevName)
    ) {
      return {
        shouldReset: true,
        confidence: "high",
        reason: "specific_product_evaluation_switch",
        currentCore: currentCore.core || queryName,
        previousCore: previousCore.core || prevName,
      };
    }
  }
...
export function pickCommercialPresentationProduct(
  ...
  if (isSpecificProductEvaluationQuery(currentQuery)) {
    const aligned = list.find((product) =>
      productOfferMatchesQueryIdentity(product, currentQuery)
    );
