...
export const SEMANTIC_AUTHORITY_VERSION = "4.1I.3.V.2.2";
...
/**
 * True when the turn is a short social evaluation without enough context to infer target.
 */
...
export function isGovernedAmbiguousSocialContract(contract = {}, targetResolution = {}) {
  ...
  if (!isSocialEvaluativeIntent(contract)) {
    const isShortSocialTurn =
      contract.messageLength != null && contract.messageLength <= 4;
    if (
      !(isShortSocialTurn && target === SEMANTIC_TARGETS.UNKNOWN &&
        !hasSufficientSocialTargetContext(conversationMessages, productCtx))
    ) {
      return false;
    }
...
export function resolveGovernedSocialRoutingKey(contract = {}, targetResolution = {}) {
  ...
  if (isGovernedAmbiguousSocialContract(contract, targetResolution)) {
    return GOVERNED_SOCIAL_ROUTING_KEYS.AMBIGUOUS_SOCIAL;
  }
...
export function enrichContractWithSemanticAuthority(
  ...
  return {
    ...
    ambiguousSocialContract: isGovernedAmbiguousSocialContract(contract, resolution),
