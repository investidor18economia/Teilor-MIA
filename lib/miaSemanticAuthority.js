/**
 * PATCH 4.1I.3 / 5.4 — Semantic Authority & Precedence
 *
...
import { SEMANTIC_TARGETS, hasSufficientSocialTargetContext } from "./miaSemanticTargetResolution.js";
import {
  SEMANTIC_PRECEDENCE_VERSION,
  applySemanticPrecedence,
  shouldAllowAmbiguousSocial,
  resolvePrecedenceRoutingKey,
  semanticPrecedenceToTrace,
} from "./miaSemanticPrecedence.js";

export const SEMANTIC_AUTHORITY_VERSION = "5.4.0";
...
const GOVERNED_CLARIFICATION_BLOCKING_KEYS = new Set([
  GOVERNED_SOCIAL_ROUTING_KEYS.GREETING,
  GOVERNED_SOCIAL_ROUTING_KEYS.FAREWELL,
  GOVERNED_SOCIAL_ROUTING_KEYS.ACKNOWLEDGEMENT,
...
const SPECIFIC_GOVERNED_ROUTING_KEYS = new Set([
  GOVERNED_SOCIAL_ROUTING_KEYS.GREETING,
  GOVERNED_SOCIAL_ROUTING_KEYS.FAREWELL,
  GOVERNED_SOCIAL_ROUTING_KEYS.ACKNOWLEDGEMENT,
  GOVERNED_SOCIAL_ROUTING_KEYS.MIA_COMPLIMENT,
...
export { isSocialEvaluativeIntent } from "./miaSemanticPrecedence.js";

/**
 * Governed ambiguous social contract — target cannot be inferred; do not assume MIA/product/prior answer.
 * Delegates to PATCH 5.4 central precedence — specific families never lose to ambiguous.
 */
export function isGovernedAmbiguousSocialContract(contract = {}, targetResolution = {}) {
  if (contract.governedSocialRoutingKey === GOVERNED_SOCIAL_ROUTING_KEYS.AMBIGUOUS_SOCIAL) {
    return shouldAllowAmbiguousSocial(contract, targetResolution);
  }
  return shouldAllowAmbiguousSocial(contract, targetResolution);
}
 ...
 * Delegates to PATCH 5.4 semantic precedence.
 */
export function resolveGovernedSocialRoutingKey(contract = {}, targetResolution = {}) {
  return resolvePrecedenceRoutingKey(contract, targetResolution);
}
...
export function adaptLegacyPrimaryIntent(contract = {}, targetResolution = {}) {
  ...
  const legacyMap = {
    [GOVERNED_SOCIAL_ROUTING_KEYS.GREETING]: "greeting",
    [GOVERNED_SOCIAL_ROUTING_KEYS.FAREWELL]: "social_conversation",
    [GOVERNED_SOCIAL_ROUTING_KEYS.ACKNOWLEDGEMENT]: "acknowledgement",
    ...
    [GOVERNED_SOCIAL_ROUTING_KEYS.AMBIGUOUS_SOCIAL]: "social_conversation",
  };
...
export function isEntityOpinionFallbackAllowed(contract = {}, targetResolution = {}) {
  if (!isProductAestheticFallbackPermitted(contract, targetResolution)) {
    return false;
  }
...
/**
 * True when governed contract belongs to the MIA compliment/praise family.
 */
export function isMiaComplimentGovernedContract(contract = {}, targetResolution = {}) {
  const routingKey = contract.governedSocialRoutingKey;
  if (
    routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.MIA_COMPLIMENT ||
    routingKey === GOVERNED_SOCIAL_ROUTING_KEYS.MIA_PRAISE
  ) {
    return true;
  }
  ...
  if (
    signals.some((s) =>
      ["mia_target", "compliment_to_mia", "praise_to_mia"].includes(s)
    ) &&
    [SOCIAL_INTENT_FAMILIES.COMPLIMENT, SOCIAL_INTENT_FAMILIES.PRAISE].includes(primary) &&
    target === SEMANTIC_TARGETS.MIA
  ) {
...
/**
 * Structural invariant — product aesthetic fallback is incompatible with mia_compliment contracts.
 */
export function isProductAestheticFallbackPermitted(contract = {}, targetResolution = {}) {
  if (isGovernedAmbiguousSocialContract(contract, targetResolution)) {
    return false;
  }
  if (isMiaComplimentGovernedContract(contract, targetResolution)) {
    return false;
  }
  ...
  const target = targetResolution?.target || contract.resolvedSemanticTarget;
  if (target === SEMANTIC_TARGETS.MIA) {
    return false;
  }

  return isEntityOpinionFallbackAllowedForTarget(contract, targetResolution);
}

function isEntityOpinionFallbackAllowedForTarget(contract = {}, targetResolution = {}) {
  const target = targetResolution?.target || contract.resolvedSemanticTarget;
  if (target === SEMANTIC_TARGETS.MIA) return false;
  if (target === SEMANTIC_TARGETS.CONVERSATION) return false;
  if (target === SEMANTIC_TARGETS.PREVIOUS_ANSWER) return false;
...
export function isAcceptableGovernedSocialReply(text = "", contract = {}, targetResolution = {}) {
  ...
  if (isGovernedAmbiguousSocialContract(contract, targetResolution)) {
    ...
    if (isLegacyEntityOpinionText(reply)) return false;
    if (isNeutralClarificationText(reply)) return false;
    if (
      /\b(obrigad\w*|valeu pelo elogio|fico feliz|que gentil)\b/i.test(reply) &&
      !/\?/.test(reply)
    ) {
      return false;
    }
    const tokens = reply.split(/\s+/).filter(Boolean).length;
    return tokens <= 40 && !COMMERCIAL_REDIRECT_PATTERN.test(reply);
  }
  ...
  if (
    ...
    if (/^(de\s+nada\.?!?|por\s+nada\.?!?|disponha\.?!?)\s*$/i.test(reply)) {
      return false;
    }
    return true;
  }
  ...
  if (
    ...
    contract.expectedHumanBehavior === "acknowledge_gratitude" ||
    contract.socialHumanizationBehavior === "gratitude_with_presence"
  ) {
    if (/^(de\s+nada\.?!?|por\s+nada\.?!?|disponha\.?!?)\s*$/i.test(reply)) {
      return false;
    }
  ...
  if (contract.identityQueryKind && !/\b(mia|teilor|assistente)\b/i.test(reply)) {
    return false;
  }
...
export function enrichContractWithSemanticAuthority(
  ...
  const mergedContract = { ...contract, ...(recognition || {}) };
  const precedenceDecision = applySemanticPrecedence(mergedContract, resolution);

  const legacyAdapter = adaptLegacyPrimaryIntent(mergedContract, resolution);

  return {
    ...contract,
    semanticAuthorityVersion: SEMANTIC_AUTHORITY_VERSION,
    semanticPrecedenceVersion: SEMANTIC_PRECEDENCE_VERSION,
    semanticPrecedence: precedenceDecision,
    ...
    governedSocialRoutingKey:
      precedenceDecision.winningRoutingKey || legacyAdapter.governedSocialRoutingKey,
    ...
    ambiguousSocialContract: precedenceDecision.ambiguousAllowed,
...
export function semanticAuthorityToTrace(contract = null) {
  ...
  return {
    ...
    semanticPrecedenceVersion: contract.semanticPrecedenceVersion,
    semanticPrecedence: semanticPrecedenceToTrace(contract.semanticPrecedence),
    ...
    ambiguousSocialContract: contract.ambiguousSocialContract ?? null,
