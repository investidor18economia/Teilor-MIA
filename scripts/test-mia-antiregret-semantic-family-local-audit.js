/**
 * PATCH 7.8E — Anti-Regret Semantic Family Local Audit
 *
 * Audits ANTI_REGRET family without production changes.
 * Two contexts: cold session (no anchor) and anchored session (winner preserved).
 * Includes commercial guard cases (must not be treated as pure anti-regret).
 *
 * Usage: node scripts/test-mia-antiregret-semantic-family-local-audit.js
 */
...
const PURE_ANTI_REGRET = [
  "vou me arrepender?",
  "posso comprar tranquilo?",
  "é uma compra segura?",
  "e uma compra segura?",
  "não vou me arrepender depois?",
  "nao vou me arrepender depois?",
  "dá pra comprar sem medo?",
  "da pra comprar sem medo?",
  "é uma escolha tranquila?",
  "e uma escolha tranquila?",
  "posso ficar sossegado?",
  "não tem muito risco?",
  "nao tem muito risco?",
  "é uma compra confiável?",
  "e uma compra confiavel?",
  "é difícil se arrepender dessa escolha?",
  "e dificil se arrepender dessa escolha?",
  "vou me arrepender depois?",
  "posso comprar sem medo?",
  "da pra ficar tranquilo?",
];

/** PATCH 7.9X-D — natural-language ANTI_REGRET (regret-avoidance / emotional safety). */
const ANTI_REGRET_SEMANTIC_EXPANSION_CASES = [
...
const COMMERCIAL_GUARD_CASES = [
  { input: "vou me arrepender ou tem outro melhor?", dominantIntent: "alternative_exploration" },
  { input: "posso comprar tranquilo ou comparar com samsung?", dominantIntent: "comparison" },
  { input: "e se eu gastar menos?", dominantIntent: "constraint_change" },
  { input: "é seguro ou espero promoção?", dominantIntent: "promotion" },
];

const IDEAL_COLD_ANTI_REGRET_TURN_TYPES = new Set([
  MIA_TURN_TYPES.CONVERSATIONAL,
]);

const PARTIAL_ANCHORED_ANTI_REGRET_TURN_TYPES = new Set([
...
/** Audit-side family detector — documents expected ANTI_REGRET intent. */
function isPureAntiRegretFamilyQuery(message = "") {
  ...
  if (/^(da|dá) pra comprar sem medo$/.test(q)) return true;
  ...
  if (/^(da|dá) pra ficar tranquilo$/.test(q)) return true;
...
function mapPartialCoverage(cognitiveTurn) {
  ...
  if (hr?.detected && hr.subtype === "purchase_anxiety") {
  ...
  if (cognitiveTurn.turnType === MIA_TURN_TYPES.EXPLANATION_REQUEST) {
    partial.push("EXPLANATION_REQUEST");
  }
...
function isPartialRouterAntiRegret(cognitiveTurn, hasActiveAnchor) {
  if (!hasActiveAnchor) return false;

  const partial = mapPartialCoverage(cognitiveTurn);
  if (partial.includes("PURCHASE_ANXIETY")) return true;
  if (partial.includes("CONCERN/OBJECTION") && cognitiveTurn.turnType === MIA_TURN_TYPES.OBJECTION) {
    return true;
  }
  return PARTIAL_ANCHORED_ANTI_REGRET_TURN_TYPES.has(cognitiveTurn.turnType);
}
...
function simulatePipeline(message, hasActiveAnchor, options = {}) {
  ...
  return {
    ...
    routerHasDedicatedAntiRegretFamily: !!cognitiveTurn.signals?.isAntiRegret,
    routerPartialCoverage: isPartialRouterAntiRegret(cognitiveTurn, hasActiveAnchor),
    signals: {
      isAntiRegretFamilyAudit: isPureAntiRegretFamilyQuery(message),
      purchaseAnxiety: cognitiveTurn.signals?.hesitationReaction?.subtype === "purchase_anxiety",
...
function simulateResponsePath({
  ...
  if (openedNewSearch) {
    return {
      ...
      wouldReassureAnchoredDecision: false,
    };
  }

  // PATCH 7.8H — anti_regret_flow (mirror handler)
  ...
  if (
    hasActiveAnchor &&
    isPartialRouterAntiRegret(cognitiveTurn, hasActiveAnchor) &&
    routingDecision.shouldPreserveAnchor
  ) {
    return {
      responsePathFinal: "context_objection_partial",
      finalResponsePreview:
        "Resposta parcial via OBJECTION/FOLLOW_UP — sem fluxo anti_regret dedicado.",
      genericFallbackDetected: false,
      wouldReassureAnchoredDecision: cognitiveTurn.turnType === MIA_TURN_TYPES.OBJECTION,
    };
  }
...
function classifyPureAntiRegretFailures(spec, pipeline) {
  const failures = [];
  const turnType = pipeline.cognitiveTurn.turnType;
  const { hasActiveAnchor, message } = spec;

  if (!isPureAntiRegretFamilyQuery(message)) {
    failures.push({
      layer: "Audit expectation",
      detail: "input is not in audit pure ANTI_REGRET family list",
    });
    return failures;
  }
  ...
  if (turnType === MIA_TURN_TYPES.NEW_SEARCH) {
    failures.push({
      layer: "Router",
      detail: "pure anti-regret classified as NEW_SEARCH",
    });
  }

  if (
    !hasActiveAnchor &&
    !IDEAL_COLD_ANTI_REGRET_TURN_TYPES.has(turnType)
  ) {
    failures.push({
      layer: "Router",
  ...
  if (
    hasActiveAnchor &&
    !isPartialRouterAntiRegret(pipeline.cognitiveTurn, hasActiveAnchor)
  ) {
    failures.push({
      layer: "Router",
      detail: `expected OBJECTION/purchase_anxiety or partial anchored path, got ${turnType}`,
    });
  }
  ...
  if (hasActiveAnchor && pipeline.winnerChanged) {
    failures.push({
      ...
      detail: "allowReplaceWinner or anchor loss on anchored anti-regret",
    });
  }
  ...
  if (
    ...
    failures.push({
      layer: "Routing",
      detail: "allowReplaceWinner=true on anchored anti-regret",
    });
  }
  ...
  if (
    ...
    failures.push({
      layer: "Audit expectation",
      detail: "cold anti-regret should reassure honestly or ask context, not institutional welcome",
    });
  }
  ...
  if (
    hasActiveAnchor &&
    isPartialRouterAntiRegret(pipeline.cognitiveTurn, hasActiveAnchor) &&
    !pipeline.responsePath.wouldReassureAnchoredDecision &&
    pipeline.responsePath.responsePathFinal !== "context_objection_partial" &&
    pipeline.responsePath.responsePathFinal !== "anti_regret_flow"
  ) {
    failures.push({
      layer: "Response path",
      detail: "anchored anti-regret did not reach dedicated or objection reassurance path",
    });
  }

  if (
    pipeline.responsePath.responsePathFinal !== "anti_regret_flow" &&
    isPureAntiRegretFamilyQuery(message)
  ) {
    failures.push({
      layer: "Response path",
      detail: "pure anti-regret did not reach anti_regret_flow",
    });
  }
  ...
  if (pipeline.cognitiveTurn.signals?.isDecisionConfirmation) {
    failures.push({
      layer: "Router",
      detail: "anti-regret query misclassified as DECISION_CONFIRMATION",
    });
  }
...
function classifyGuardFailures(spec, pipeline) {
  ...
  if (isPureAntiRegretFamilyQuery(spec.input)) {
    failures.push({
      layer: "Router",
      detail: "classified as pure anti-regret despite commercial tail",
    });
  }
...
function evaluatePureCase(message, hasActiveAnchor) {
  ...
  const failures = classifyPureAntiRegretFailures(
    { message, hasActiveAnchor },
    pipeline
  );

  return {
    kind: "pure_anti_regret",
    input: message,
    family: "ANTI_REGRET",
...
function classifyRouterOnlyAntiRegretFailures(spec, pipeline) {
  const failures = [];
  const { hasActiveAnchor, message } = spec;
  const turnType = pipeline.cognitiveTurn.turnType;

  if (!isAntiRegretFamilyQuery(message)) {
    failures.push({
      layer: "Router",
      detail: "semantic expansion phrase not recognized as ANTI_REGRET",
    });
  }
  ...
  if (!pipeline.cognitiveTurn.signals?.isAntiRegret) {
    failures.push({
      layer: "Router",
      detail: "signals.isAntiRegret missing on expansion phrase",
    });
  }
  ...
  if (
    ...
    failures.push({
      layer: "Router",
      detail: `expected CONVERSATIONAL or OBJECTION on cold expansion, got ${turnType}`,
    });
  }
  ...
  if (
    hasActiveAnchor &&
    turnType !== MIA_TURN_TYPES.OBJECTION &&
    !PARTIAL_ANCHORED_ANTI_REGRET_TURN_TYPES.has(turnType)
  ) {
    failures.push({
      layer: "Router",
      detail: `expected OBJECTION or compatible anchored path, got ${turnType}`,
    });
  }
...
function evaluateExpansionCase(message, hasActiveAnchor) {
  const pipeline = simulatePipeline(message, hasActiveAnchor);
  const failures = classifyRouterOnlyAntiRegretFailures(
    { message, hasActiveAnchor },
    pipeline
  );
  ...
  return {
    ...
    actualTurnType: pipeline.cognitiveTurn.turnType,
    signals: {
      isAntiRegret: !!pipeline.cognitiveTurn.signals?.isAntiRegret,
...
console.log("\nPATCH 7.8E/7.9X-D — Anti-Regret Semantic Family Local Audit\n");
...
const pureRecords = [];
for (const message of PURE_ANTI_REGRET) {
...
for (const message of ANTI_REGRET_SEMANTIC_EXPANSION_CASES) {
...
console.log("Dedicated ANTI_REGRET family in Router: YES (PATCH 7.8F — signals.isAntiRegret)");
console.log(
  "Response path for pure ANTI_REGRET: YES (PATCH 7.8H — anti_regret_flow)"
);
console.log(
  `Partial signals on pure audit phrases — cold: ${partialCold.length}/${pureTotal / 2}, anchored: ${partialAnchored.length}/${pureTotal / 2}`
);

console.log("\n── Pure anti-regret cases ──\n");
...
console.log("\n── Semantic expansion cases (PATCH 7.9X-D, Router-only) ──\n");
for (const r of expansionRecords) {
  const partial = r.signals.partialCoverage.length
    ? `[${r.signals.partialCoverage.join(",")}]`
    : "[]";
  console.log(
    `  ${r.passed ? "✓" : "✗"} [${r.context}] "${r.input}" → ${r.actualTurnType} | partial=${partial} | signal=${r.signals.isAntiRegret} | cc=${r.signals.isConstraintChange} | ${r.primaryFailureLayer}`
  );
}
...
console.log(`Cold working: ${coldWorking.length}/${PURE_ANTI_REGRET.length}`);
console.log(`Anchored working: ${anchoredWorking.length}/${PURE_ANTI_REGRET.length}`);
...
const nextPatch =
  layerCounts.Router && layerCounts.Routing
    ? "7.8F-Router → 7.8G-Routing → 7.8H-Response path"
    : layerCounts.Router
      ? "7.8F-Router — add ANTI_REGRET family (cold + anchored); precede NEW_SEARCH"
      : layerCounts.Routing
        ? "7.8G-Routing — anti_regret hold must not fall through to default search"
        : layerCounts["Response path"] || layerCounts["Final response"]
          ? "7.8H-Response path — anti_regret_flow wiring"
          : "7.8F-Router — ANTI_REGRET semantic family";
...
console.log(
  "3. Dedicated ANTI_REGRET in Router: YES — signals.isAntiRegret + isAntiRegretFamilyQuery (PATCH 7.8F/7.9X-D natural expansion)"
);
console.log(
  `4. Phrases working when cold: ${coldWorking.length}/${PURE_ANTI_REGRET.length}`
);
console.log(
  `4b. Phrases working when anchored: ${anchoredWorking.length}/${PURE_ANTI_REGRET.length}`
);
console.log(
  `4c. Phrases fully working (both contexts): ${workingPure.length}/${PURE_ANTI_REGRET.length}`
);
console.log(
  `5. Phrases with failures: ${failingPure.length}/${PURE_ANTI_REGRET.length} (original); expansion router failures ${expansionRouterFailures}/${expansionTotal}`
);
console.log(
...
console.log(
  "13–18. Regressions: run GREETING/ACK/COMPREHENSION/SOFT_DISAGREEMENT/DECISION_CONFIRMATION closure + 7.6V scripts separately after audit"
);
...
console.log(
  `\nAudit script approval: ${familyClosed ? "ANTI_REGRET FULLY_CLOSED" : "GAPS REMAIN"}\n`
);
console.log(
  `PATCH 7.8E/7.8H/7.9X-D audit COMPLETE — ${familyClosed ? "ALL PASS (40/40 pure + 16/16 expansion + 4/4 guards)" : "GAPS FOUND"}\n`
);
