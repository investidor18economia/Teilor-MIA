#!/usr/bin/env node
/**
 * PATCH 7.2 — Error analytics production validation.
 */
...
const QUERIES = [
  { file: "patch-72-query1-error-overview.sql", label: "Q1 overview", minRows: 1 },
  { file: "patch-72-query2-error-dimensions.sql", label: "Q2 dimensions", minRows: 0 },
  { file: "patch-72-query3-recovery-correlation.sql", label: "Q3 recovery", minRows: 0 },
  { file: "patch-72-query4-evolution-gaps-panel.sql", label: "Q4 evolution", minRows: 1 },
];
...
try {
  ...
  for (const q of QUERIES) {
    ...
    if (q.file.includes("query1")) {
      const cap = rows.find((r) => r.metrica === "total_eventos_mia_error_event");
      ok("Q1 capacity row present", !!cap);
      console.log("\nQuery 1 — error overview sample:");
      console.log(JSON.stringify(rows.slice(0, 10), null, 2));
      if (cap?.valor_absoluto === 0) {
        ok("Q1 zero events pre-deploy expected", cap.limitacao === "sem_eventos_apos_deploy_patch_72");
      }
    }
  }

  const q1 = runLinkedSql(join(ROOT, "docs/analytics/sql/patch-72-query1-error-overview.sql"));
  const total = q1.find((r) => r.metrica === "total_eventos_mia_error_event")?.valor_absoluto ?? 0;
  ok("mia_error_event events counted", total >= 0, `total=${total}`);
} catch (err) {
