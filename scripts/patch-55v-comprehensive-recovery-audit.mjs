#!/usr/bin/env node
/**
 * PATCH 5.5V — Comprehensive Universal Recovery Audit (production UI + API)
 * Audit-only — no code changes. Run: node scripts/patch-55v-comprehensive-recovery-audit.mjs
 */
...
const OUT = join(ROOT, "docs/conversational/audits/phase-5/evidence/patch-55v");
...
function semanticFingerprint(text = "") {
  ...
  if (/celular|notebook|galaxy|iphone|recomend|orçamento|compar|monitor|preço/i.test(n)) return "commercial";
  if (/tchau|até logo|até mais|flw|falou/i.test(n)) return "farewell";
  return "other_social";
}
...
async function runStability(page, msg, times = 10, id = "STAB") {
  const runs = [];
...
// ─── Main ───
log("PATCH 5.5V comprehensive recovery audit starting");
...
writeFileSync(
  ...
  JSON.stringify(
    {
      expectedFunctionalCommit: "bc3290f",
      expectedEvidenceCommit: "f3191a3",
...
log("Running stability battery (10x each)...");
const stabilityTargets = ["Oi", "Opa", "Linda", "Show", "Quero um celular até 2000", "Compare iPhone 13 com Galaxy A55", "Valeu", "Não entendi"];
const stability = {};
for (const msg of stabilityTargets) {
  stability[msg] = await runStability(page, msg, 10, `STAB_${msg.slice(0, 12).replace(/\W/g, "_")}`);
}
...
writeFileSync(join(OUT, "STABILITY_10X.json"), JSON.stringify(stability, null, 2));
...
log(`DONE totalTurns=${totalTurns} parity=${summary.parityApproved}/${summary.totalScenarios} commercialDegraded=${summary.commercialDegraded}`);
console.log(JSON.stringify({ totalTurns, summary, localTests: localTests.pass }, null, 2));
