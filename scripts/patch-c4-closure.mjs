#!/usr/bin/env node
/**
 * PATCH C.4 — Official closure orchestrator.
 */
...
steps.push(run("node scripts/test-mia-analytics-patch-c4-executive-trends.js", "C.4 trends audit"));
...
if (process.env.MIA_ADMIN_API_KEY) {
  steps.push(run("node scripts/patch-c4-browser-validation.mjs", "C.4 browser regression"));
  steps.push(run("node scripts/patch-b9-production-validation.mjs", "Production validation"));
}

const trendsEvidence = readJson("docs/analytics/PATCH_C_4_TRENDS_EVIDENCE.json");
const browserEvidence = readJson("docs/analytics/PATCH_C_4_BROWSER_EVIDENCE.json");
...
const allPass =
  ...
  trendsEvidence.status === "APPROVED" &&
  docExists &&
...
const closure = {
  patch: "C.4",
  title: "PATCH C.4 — Official Closure",
  phase: "C — MIA como Analista da Empresa",
  status: allPass && gitSync.pass ? "CLOSED" : "BLOCKED",
  patch_c4_status: allPass && gitSync.pass ? "OFFICIALLY_CLOSED" : "BLOCKED_PENDING_VALIDATION",
  ...
  scope: "Deterministic Executive Trend Generator — lib only, no Cockpit UI",
  steps,
  evidence: {
    trends: trendsEvidence.status ?? "PENDING",
    browser: browserEvidence.status ?? "SKIPPED",
    documentation: docExists,
  },
  files_created: [
    "lib/miaExecutiveTrendCatalog.js",
    "lib/miaExecutiveTrendRules.js",
    "lib/miaExecutiveTrendBuilder.js",
    "scripts/test-mia-analytics-patch-c4-executive-trends.js",
    "scripts/patch-c4-browser-validation.mjs",
    "scripts/patch-c4-closure.mjs",
  ],
  files_modified: [
  ...
  next_patch: "C.5 — Narrative assembly (not started)",
};

writeFileSync(join(ROOT, "docs/analytics/PATCH_C_4_CLOSURE_EVIDENCE.json"), JSON.stringify(closure, null, 2));

console.log(`\nPATCH C.4 status: ${closure.patch_c4_status}\n`);
