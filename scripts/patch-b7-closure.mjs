#!/usr/bin/env node
/**
 * PATCH B.7 — Official closure orchestrator.
 */
...
steps.push(run("node scripts/test-mia-analytics-patch-b7-executive-summary.js", "B.7 summary audit"));
...
if (process.env.MIA_ADMIN_API_KEY) {
  steps.push(run("node scripts/patch-b7-browser-validation.mjs", "Browser validation"));
...
const b7Evidence = readJson("docs/analytics/PATCH_B_7_EXECUTIVE_SUMMARY_EVIDENCE.json");
const browserEvidence = readJson("docs/analytics/PATCH_B_7_BROWSER_EVIDENCE.json");
const docExists = existsSync(join(ROOT, "docs/analytics/FOUNDER_EXECUTIVE_DASHBOARD.md"));
const docHasB7 =
  docExists && readFileSync(join(ROOT, "docs/analytics/FOUNDER_EXECUTIVE_DASHBOARD.md"), "utf8").includes("B.7");

const requiredSteps = steps.filter((s) => !s.label?.includes("Browser") && !s.label?.includes("Production"));
const allPass =
  requiredSteps.every((s) => s.pass) &&
  b7Evidence.status === "APPROVED" &&
...
const closure = {
  patch: "B.7",
  title: "PATCH B.7 — Official Closure",
  phase: "B — Dashboard Executivo",
  status: allPass && gitSync.pass ? "CLOSED" : "BLOCKED",
  patch_b7_status: allPass && gitSync.pass ? "OFFICIALLY_CLOSED" : "BLOCKED_PENDING_VALIDATION",
  ...
  baseline_b6_preserved: true,
  steps,
  evidence: {
    b7_summary: b7Evidence.status ?? "PENDING",
    browser: browserEvidence.status ?? "SKIPPED",
    dashboard_doc: docExists && docHasB7,
  },
  next_patch: "B.8 — Polimento Executivo",
};

writeFileSync(join(ROOT, "docs/analytics/PATCH_B_7_CLOSURE_EVIDENCE.json"), JSON.stringify(closure, null, 2));

console.log(`\nPATCH B.7 status: ${closure.patch_b7_status}\n`);
