#!/usr/bin/env node
/**
 * PATCH B.3 — Official closure orchestrator.
 */
...
steps.push(run("node scripts/test-mia-analytics-patch-b3-executive-growth.js", "B.3 growth audit"));
...
if (process.env.MIA_ADMIN_API_KEY) {
  steps.push(run("node scripts/patch-b3-browser-validation.mjs", "Browser validation"));
...
const b3Evidence = readJson("docs/analytics/PATCH_B_3_EXECUTIVE_GROWTH_EVIDENCE.json");
const browserEvidence = readJson("docs/analytics/PATCH_B_3_BROWSER_EVIDENCE.json");
const docExists = existsSync(join(ROOT, "docs/analytics/FOUNDER_EXECUTIVE_DASHBOARD.md"));
const docHasB3 = docExists && readFileSync(join(ROOT, "docs/analytics/FOUNDER_EXECUTIVE_DASHBOARD.md"), "utf8").includes("B.3");

const requiredSteps = steps.filter((s) => !s.label?.includes("Browser") && !s.label?.includes("Production"));
const allPass =
  requiredSteps.every((s) => s.pass) &&
  b3Evidence.status === "APPROVED" &&
  docExists &&
  docHasB3;

const closure = {
  patch: "B.3",
  title: "PATCH B.3 — Official Closure",
  phase: "B — Dashboard Executivo",
  status: allPass && gitSync.pass ? "CLOSED" : "BLOCKED",
  patch_b3_status: allPass && gitSync.pass ? "OFFICIALLY_CLOSED" : "BLOCKED_PENDING_VALIDATION",
  ...
  steps,
  evidence: {
    b3_growth: b3Evidence.status ?? "PENDING",
    browser: browserEvidence.status ?? "SKIPPED",
    dashboard_doc: docExists && docHasB3,
  },
  next_patch: "B.4 — (próximo módulo executivo)",
};

writeFileSync(join(ROOT, "docs/analytics/PATCH_B_3_CLOSURE_EVIDENCE.json"), JSON.stringify(closure, null, 2));

console.log(`\nPATCH B.3 status: ${closure.patch_b3_status}\n`);
