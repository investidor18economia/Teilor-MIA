#!/usr/bin/env node
/**
 * PATCH B.5 — Official closure orchestrator.
 */
...
steps.push(run("node scripts/test-mia-analytics-patch-b5-executive-commercial-performance.js", "B.5 commercial audit"));
...
if (process.env.MIA_ADMIN_API_KEY) {
  steps.push(run("node scripts/patch-b5-browser-validation.mjs", "Browser validation"));
...
const b5Evidence = readJson("docs/analytics/PATCH_B_5_EXECUTIVE_COMMERCIAL_PERFORMANCE_EVIDENCE.json");
const browserEvidence = readJson("docs/analytics/PATCH_B_5_BROWSER_EVIDENCE.json");
const docExists = existsSync(join(ROOT, "docs/analytics/FOUNDER_EXECUTIVE_DASHBOARD.md"));
const docHasB5 =
  docExists && readFileSync(join(ROOT, "docs/analytics/FOUNDER_EXECUTIVE_DASHBOARD.md"), "utf8").includes("B.5");
...
const closure = {
  patch: "B.5",
  title: "PATCH B.5 — Official Closure",
  phase: "B — Dashboard Executivo",
  status: allPass && gitSync.pass ? "CLOSED" : "BLOCKED",
  patch_b5_status: allPass && gitSync.pass ? "OFFICIALLY_CLOSED" : "BLOCKED_PENDING_VALIDATION",
  ...
  steps,
  evidence: {
    b5_commercial: b5Evidence.status ?? "PENDING",
    browser: browserEvidence.status ?? "SKIPPED",
    dashboard_doc: docExists && docHasB5,
  },
  next_patch: "B.6 — Indicadores Operacionais",
};

writeFileSync(join(ROOT, "docs/analytics/PATCH_B_5_CLOSURE_EVIDENCE.json"), JSON.stringify(closure, null, 2));

console.log(`\nPATCH B.5 status: ${closure.patch_b5_status}\n`);
