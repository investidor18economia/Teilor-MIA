#!/usr/bin/env node
/**
 * PATCH 4.1I.3.V.2.1 — Post-deploy production validation (complete gates)
 */
...
const EVIDENCE = join(ROOT, "docs/conversational/audits/phase-4/evidence/patch-41i3v21");
...
function classifyLindaIsolated(reply) {
  ...
  const reasons = [];
  if (PRODUCT_FRAME.test(reply)) reasons.push("product_frame");
  if (MIA_THANKS.test(reply)) reasons.push("mia_thanks_without_context");
  if (CLARIFICATION.test(reply)) reasons.push("unnecessary_clarification");
...
function classifyMiaComplimentVariant(reply) {
  const tech = classifyTechnical(reply);
  if (tech.technical) return { classification: "FALHA_TECNICA", reasons: [tech.type], semantic: false };
  const reasons = [];
...
async function main() {
  appendFileSync(LOG, `\n--- PATCH 4.1I.3.V.2.1 POST-DEPLOY ${new Date().toISOString()} ---\n`);
  ...
  for (let i = 0; i < MIA_COMPLIMENT_VARIANTS.length; i++) {
    ...
    const r = await runMultiTurn(browser, `MIA_VAR_${i + 1}`, msgs, (reply) => classifyMiaComplimentVariant(reply));
  ...
  const summary = {
    ...
    rootCause: "Stale session lastBestProduct promoted to product context in resolveSemanticTarget",
    fixStage: "miaSemanticTargetResolution + miaSemanticAuthority + miaGovernedFallbackPolicy",
