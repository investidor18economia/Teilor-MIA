/**
 * PATCH 9.2U — Human Friction Modeling Layer Audit
 *
 * Usage:
 *   node scripts/test-mia-human-friction-modeling-audit.js
 */
...
import {
  ...
  calculateFrictionRelevance,
  FRICTION_CLASSES,
  isFrictionTraceable,
  classifyFrictionOrigin,
  HUMAN_FRICTION_MODELING_VERSION,
...
function classifyFrictionMetric(friction = null) {
  if (!friction) return "placeholder";
  const origin = classifyFrictionOrigin(friction);
...
console.log("\nPATCH 9.2U — Human Friction Modeling Layer Audit\n");
console.log(`Friction layer: ${HUMAN_FRICTION_MODELING_VERSION}`);
console.log(`Friction classes: ${FRICTION_CLASSES.join(", ")}`);
...
assert("friction model ok", frictionModel.ok);
assert("frictions traceable", frictionModel.frictions.every((entry) => isFrictionTraceable(entry)));
assert(
  "full chain consequence→sensation→experience→friction",
  frictionModel.frictions.every(
    (entry) => entry.sourceConsequence && entry.sensation && entry.sourceExperience && entry.friction
  )
);

console.log("\n── Friction Relevance Engine ──");
const hzFriction =
  frictionModel.frictions.find((entry) => /fluidez|gesto|60|hz/i.test(entry.friction)) ||
  frictionModel.frictions[0];
const casualRelevance = calculateFrictionRelevance(hzFriction, {
  primaryAxis: "battery",
  query: "celular bateria",
});
const gamerRelevance = calculateFrictionRelevance(hzFriction, {
  primaryAxis: "screen",
  query: "celular gamer fluidez 120hz",
  querySignals: { technical: true },
});
console.log(`  60Hz friction — usuário comum (bateria): ${casualRelevance.toFixed(2)}`);
console.log(`  60Hz friction — prioridade tela/gamer: ${gamerRelevance.toFixed(2)}`);
assert("relevance varia por contexto", gamerRelevance > casualRelevance);
assert("gamer/screen relevance alta", gamerRelevance >= 0.55);

console.log("\n── Context awareness (prioridade câmera vs bateria) ──");
const cameraFrictionModel = buildHumanFrictionModel({
  winner: "iPhone 13",
  sensations: bridge.sensations,
  experiences: experienceModel.experiences,
...
const batteryFrictionModel = buildHumanFrictionModel({
  winner: "iPhone 13",
  sensations: bridge.sensations,
  experiences: experienceModel.experiences,
...
assert(
  "camera e bateria geram frictions distintas",
  (cameraFrictionModel.frictions[0]?.frictionClass || "") !==
    (batteryFrictionModel.frictions[0]?.frictionClass || "") ||
    cameraFrictionModel.frictions[0]?.contextualRelevance !==
      batteryFrictionModel.frictions[0]?.contextualRelevance
);
...
for (const scenario of SCENARIOS) {
  ...
  const model = result.specialist.humanFrictionModel;
  const top = model?.frictions?.[0] || null;
  const metric = classifyFrictionMetric(top);
  frictionMetrics.push(metric);

  console.log(`  frictions: ${model?.frictions?.length || 0}`);
  console.log(`  top: ${top?.frictionClass || "(none)"} — ${top?.friction?.slice(0, 70) || ""}`);
  console.log(`  relevance: ${top?.contextualRelevance?.toFixed(2) || "n/a"}`);
  console.log(`  metric: ${metric}`);

  assert(`${scenario.id}: specialist ok`, result.specialist.ok);
  assert(`${scenario.id}: humanFrictionModel wired`, !!model?.ok);
  assert(`${scenario.id}: frictions non-empty`, (model?.frictions?.length || 0) > 0);
  assert(`${scenario.id}: all traceable`, model?.frictions?.every((entry) => isFrictionTraceable(entry)));
  if (top) {
...
const realPct =
  (frictionMetrics.filter((m) => m === "real" || m === "derived").length / frictionMetrics.length) * 100;
const placeholderPct =
  (frictionMetrics.filter((m) => m === "placeholder").length / frictionMetrics.length) * 100;

console.log("\n── Métricas de aceite ──");
console.log(`  Friction real/derived: ${realPct.toFixed(1)}% (meta > 80%)`);
console.log(`  Friction placeholder: ${placeholderPct.toFixed(1)}% (meta < 10%)`);
assert("friction real > 80%", realPct > 80, `${realPct.toFixed(1)}%`);
assert("friction placeholder < 10%", placeholderPct < 10, `${placeholderPct.toFixed(1)}%`);
...
console.log("Antes (9.2T): modelava experiência, não atrito explícito");
console.log("Depois friction:", after.specialist.humanFrictionModel?.frictions?.[0]?.friction || "(none)");
console.log("Depois relevance:", after.specialist.humanFrictionModel?.frictions?.[0]?.contextualRelevance);
assert("after: friction model populated", (after.specialist.humanFrictionModel?.frictions?.length || 0) > 0);
